CREATE OR REPLACE FORCE VIEW V_SEACCOUNTRISK AS
(
                select se.floorprice,se.MRCRLIMIT,se.MRCLAMT,se.MRCRLIMITMAX,se.codeid,se.afacctno,se.acctno, se.trade, se.receiving,se.mrpriceloan,se.mrratioloan,se.mrpricerate,se.mrratiorate,nvl(BUYQTTY,0) BUYQTTY,nvl(od.EXECQTTY,0) EXECQTTY, nvl(STS.MAMT,0) mamt
                from
                (select se.floorprice,se.MRCRLIMIT,se.MRCLAMT,se.MRCRLIMITMAX,se.aftype, se.codeid,se.afacctno,se.acctno, se.trade, se.receiving,nvl(rsk.mrpriceloan,0) mrpriceloan, nvl(rsk.mrratioloan,0) mrratioloan,nvl(rsk.mrpricerate,0) mrpricerate, nvl(rsk.mrratiorate,0) mrratiorate
                    from
                    (select sb.floorprice,af.MRCRLIMIT,af.MRCLAMT,af.MRCRLIMITMAX,af.actype aftype, se.codeid,se.afacctno,se.acctno, se.trade, se.receiving
