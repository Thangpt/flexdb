CREATE OR REPLACE FORCE VIEW VW_MR9004_FOR_LOG AS
select se.roomchk, symbol , se.codeid,
trade , mortage,receiving ,t0receiving receivingt2,TOTALRECEIVING ,execqtty SELLMATCHQTTY,buyqtty BUYQTTY,TOTALBUYQTTY,
...
from
(select prgrp.autoid, se.roomchk, cf.custodycd, se.codeid, af.actype, af.mriratio,af.trfbuyrate, af.trfbuyext, se.afacctno,se.acctno, se.trade, se.mortage ,
    NVL(sts.receiving,0) receiving,nvl(sts.t0receiving,0) t0receiving,nvl(sts.totalreceiving,0) totalreceiving,nvl(BUYQTTY,0) BUYQTTY,
    nvl(TOTALBUYQTTY,0) TOTALBUYQTTY,nvl(od.EXECQTTY,0) EXECQTTY, nvl(STS.MAMT,0) mamt,
