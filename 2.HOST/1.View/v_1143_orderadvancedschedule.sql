CREATE OR REPLACE FORCE VIEW V_1143_ORDERADVANCEDSCHEDULE AS
(
...
FROM (
    SELECT  1 ISMORTAGE,STSCHD.AFACCTNO, AMT, QTTY, FULLNAME, FAMT,
            ...
            /*(CASE WHEN STSCHD.DFNINTCD='000' THEN 1 ELSE 0 END)*STSCHD.DFALLFEE+
            (CASE WHEN STSCHD.DFNINTCD='000' THEN 0 ELSE 1 END)*(SP_SBS_CAL_INTOVDDUE(STSCHD.DFPRINTFRQ3, STSCHD.DFrate3, STSCHD.DFrlsdate, TO_DATE(SYSVAR.VARVALUE,'DD/MM/RRRR')) +
            SP_SBS_CAL_INTDUE(STSCHD.DFPRINTFRQ1, STSCHD.DFrate1, STSCHD.DFPRINTFRQ2, STSCHD.DFrate2,
            STSCHD.DFPRINTFRQ3, STSCHD.DFrate3, STSCHD.DFrlsdate, TO_DATE(SYSVAR.VARVALUE,'DD/MM/RRRR')))*STSCHD.ODAMT INTAMT, --TONG LAI PHAI TRA TINH THEO SL CHUNG KHOAN DUOC GIAI TOA*/
    ...
    (SELECT greatest(max(round((sts.qtty-sts.aqtty + nvl(v.rlsqtty,0)) * nvl(v.amt,0)/(nvl(v.remainqtty,1) + nvl(v.rlsqtty,1))
      ...
      MAX(v.DESCRIPTION) DFDESCRIPTION, MAX(MST.DFACCTNO) DFACCTNO,
        ...
        FROM STSCHD STS,ODMAST MST,AFMAST AF,SBSECURITIES SEC, AFTYPE TYP, SYSVAR SYS,v_getDealInfo v
