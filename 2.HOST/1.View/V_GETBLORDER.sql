...
SELECT F.TRADERID, BL.BLACCTNO, CF.CUSTODYCD, AF.ACCTNO AFACCTNO, MST.ORDERID, MST.EXECTYPE,
            ...
            --CASE WHEN MST.ORDERID = F.ORGACCTNO THEN F.ACCTNO ELSE MST.ORDERID END FOACCTNO, 
            f.feedbackmsg ERR_DESC, SB.TRADEPLACE TRADEPLACEVL,
                MST.TXTIME
            FROM CFMAST CF, AFMAST AF, (SELECT * FROM OOD UNION SELECT * FROM OODHIST) OOD,
                ...
                FROM
                    (SELECT OD1.*,OD2.EDSTATUS EDITSTATUS, fo.foorderid
                    FROM ODMAST OD1,(SELECT * FROM ODMAST WHERE EDSTATUS IN ('C','A')) OD2, newfo_ordermap fo
                    WHERE OD1.ORDERID=OD2.REFORDERID(+) AND SUBSTR(OD1.EXECTYPE,1,1) <> 'C'
                        AND SUBSTR(OD1.EXECTYPE,1,1) <> 'A' AND od1.orderid = fo.boorderid
                        AND OD1.VIA = 'L'
                    ) MST
                ) MST,
                SBSECURITIES SB, ALLCODE CD1,ALLCODE CD2, 
                ALLCODE CD4, ALLCODE CD7, ALLCODE CD10,BL_ODMAST F, BL_TRADEREF BL
            WHERE MST.ORSTATUS <> '7' AND AF.ACCTNO=MST.AFACCTNO
                AND MST.ORDERID=OOD.ORGORDERID(+)
                AND BL.AFACCTNO(+) = f.afACCTNO AND BL.TRADERID(+) = F.TRADERID AND BL.STATUS(+) = 'A'
                ...
                AND (MST.foorderid = f.hftorderid OR mst.foorderid IN (SELECT hftOrderId FROM bl_maporder blm WHERE blm.blorderid = f.blorderid))
)
;
