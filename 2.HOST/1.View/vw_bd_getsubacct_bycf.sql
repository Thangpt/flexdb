CREATE OR REPLACE FORCE VIEW VW_BD_GETSUBACCT_BYCF AS
SELECT CF.CUSTID CFCUSTID, AF.ACCTNO, CF.CUSTODYCD || '.' || AF.ACCTNO || '.' || TYP.TYPENAME || '.' || CF.FULLNAME OWNERNAME,
'YYYYYYYYYY' LINKAUTH, A0.CDCONTENT LNKTYPE, AF.ACTYPE, CI.COREBANK , AF.CAREBY, NVL(MR.MRTYPE,'T') MRTYPE, NVL(MR.ISPPUSED,0) ISPPUSED,
CF.FULLNAME CFFULLNAME, CF.CUSTODYCD CFCUSTODYCD, CF.EMAIL CFEMAIL, CF.USERNAME, CF.IDCODE,
RTRIM(CD.cdcontent) || ': ' || CF.idcode || ',' || CF.IDDATE || ', ' || CF.IDPLACE || ', ' || CF.address CFEXT, CF.CAREBY CFCAREBY
FROM CFMAST CF, AFMAST AF, CIMAST CI, ALLCODE CD, ALLCODE A0, AFTYPE TYP, MRTYPE MR
WHERE CF.CUSTID=AF.CUSTID AND CI.AFACCTNO=AF.ACCTNO AND AF.ACTYPE=TYP.ACTYPE
