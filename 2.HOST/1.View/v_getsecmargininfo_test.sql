CREATE OR REPLACE FORCE VIEW V_GETSECMARGININFO_TEST AS
(
    select se.afacctno ,
                ...
                (select se.codeid, af.actype, af.mriratio, se.afacctno,se.acctno, se.trade , nvl(sts.receiving,0) receiving,nvl(sts.t0receiving,0) t0receiving,nvl(sts.totalreceiving,0) totalreceiving,nvl(BUYQTTY,0) BUYQTTY,nvl(TOTALBUYQTTY,0) TOTALBUYQTTY,nvl(od.EXECQTTY,0) EXECQTTY, nvl(STS.MAMT,0) mamt, nvl(afpr.prinused,0) prinused, nvl(afpr.sy_prinused,0) sy_prinused
                            ...
                            from (
                                SELECT (case when od.exectype IN ('NB','BC')
                                            then (case when (af.trfbuyext * af.trfbuyrate) > 0 then 0 else REMAINQTTY end)
                                                    + (case when (select NVL((trfbuyext * trfbuyrate * (amt - trfexeamt)), 0) islatetransfer from stschd sts_trf where sts_trf.duetype = 'SM' and sts_trf.deltd <> 'Y' and od.orderid = sts_trf.orgorderid) > 0 then 0 else EXECQTTY - DFQTTY end)
                                            else 0 end) BUYQTTY,
                                        ...
                                        (case when od.exectype IN ('NS','MS') and od.stsstatus <> 'C'
                                            then EXECQTTY - (select nvl(sum(execqtty), 0) dfexecqtty from odmapext dfex where dfex.type = 'D' and od.orderid = dfex.orderid) else 0 end) EXECQTTY,AFACCTNO, CODEID
                                FROM odmast od, afmast af
                                   where od.afacctno = af.acctno
                                   and od.txdate =(select CURRDATE from sysvar_CURRDATE)
                                   AND od.deltd <> 'Y'
                                   --AND od.errod = 'N'
                    ...
                    (SELECT STS.CODEID,STS.AFACCTNO,
                            ...
                            SUM(CASE WHEN DUETYPE ='RS' and nvl(sts_trf.islatetransfer,0) = 0 AND STS.TXDATE <> sy.CURRDATE THEN QTTY-AQTTY ELSE 0 END) RECEIVING,
                            SUM(CASE WHEN DUETYPE ='RS' AND (nvl(sts_trf.islatetransfer,0) = 0 or sts_trf.trfbuydt = sy.CURRDATE) AND STS.TXDATE <> sy.CURRDATE THEN QTTY-AQTTY ELSE 0 END) T0RECEIVING,
                            SUM(CASE WHEN DUETYPE ='RS' AND STS.TXDATE <> sy.CURRDATE THEN QTTY-AQTTY ELSE 0 END) TOTALRECEIVING
