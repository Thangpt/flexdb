CREATE OR REPLACE VIEW V_CA3351 AS
(SELECT CA.AUTOID, CA.BALANCE, SUBSTR(CA.CAMASTID,1,4) || '.' || SUBSTR(CA.CAMASTID,5,6) || '.' || SUBSTR(CA.CAMASTID,11,6) CAMASTID, CA.AFACCTNO,A0.CDCONTENT CATYPE, tosym.CODEID, CA.EXCODEID, CA.QTTY, ROUND(CA.AMT) AMT,
       ROUND(CASE WHEN camast.catype IN ('017','020','023','027') THEN nvl(se.blocked,0) ELSE 0 END ) BLOCKEDQTTY,
       ROUND(CASE WHEN camast.catype IN ('017','020','023','027') THEN nvl(se.trade,0) ELSE 0 END ) AQTTY,ROUND(CA.DFQTTY) DFQTTY,
       ROUND(CA.AAMT) AAMT, tosym.SYMBOL,ISWFT,(CASE WHEN AF.ISFCT='Y' THEN PITRATESE ELSE 0 END) PITRATE,CASE WHEN NVL(CAMAST.ISWFT,'N')='Y' THEN (SELECT CODEID FROM SBSECURITIES WHERE REFCODEID=nvl(CAMAST.TOCODEID,CAMAST.CODEID))  ELSE TOCODEID END TOCODEID , A1.CDCONTENT STATUS,
...
AND CA.DELTD ='N' AND CA.STATUS IN ('S','G','H','J') AND CA.ISSE ='N' and CAMAST.STATUS IN ('I','G','H','J')
