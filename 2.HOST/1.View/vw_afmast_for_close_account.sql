CREATE OR REPLACE FORCE VIEW VW_AFMAST_FOR_CLOSE_ACCOUNT AS
SELECT CF.CUSTODYCD, CF.FULLNAME, CF.IDCODE, AF.ACCTNO AFACCTNO, TYP.TYPENAME, A0.CDCONTENT STATUS,
              ...
              NVL(SE.TRADE_QTTY,0)TRADE_QTTY,nvl(sedtl.qtty,0) trade_limit_qtty,
              NVL(SE.LOAN_QTTY,0) LOAN_QTTY, nvl(ST.RS_QTTY,0) RS_QTTY, nvl(ST.SS_QTTY,0) SS_QTTY,
              nvl(ST.RM_AMT,0) RM_AMT, nvl(ST.SM_AMT,0) SM_AMT, AF.MRCRLIMITMAX, AF.MRCRLIMIT, (usl.acclimit) AS T0AMT,
              ...
              nvl(COUNT_DAYS_EXTEND(AF.ACCTNO),0) EXTENDDAYS,nvl(sts.depoamt,0) depoamt,
              NVL(st.RS_QTTY+st.SS_QTTY,0) WAIT_QTTY,
              NVL(seretl.qtty,0) SERETAIL_QTTY,
              NVL(sestand.qtty,0) standing, NVL(DF.DF_QTTY,0) DF_QTTY,CI.EMKAMT,
       ...
       FROM ALLCODE A0, CFMAST CF, AFTYPE TYP, AFMAST AF, MRTYPE,
           ...
           (
             ...
             FROM       (
              ...
              ( -- CK  cam co VSD
              SELECT afacctno,ABS(qtty-sendqtty) qtty
              FROM
                    (SELECT SUM(qtty) qtty , SUM(sendqtty)sendqtty, afacctno
                    FROM
                        (SELECT  (CASE WHEN status='S' THEN qtty ELSE 0 END) sendqtty,
                        (CASE WHEN status NOT IN ( 'S','I') THEN qtty ELSE 0 END ) qtty,
                        substr(acctno,0,10) afacctno
                        from semastdtl
                        WHERE deltd='N' AND qttytype ='011'
          ...
          --lenh Repo
          (--LAY DANH SACSH LENH REPO CHO LENH LAN 2
