create or replace force view vw_ln5541 as
select cf.custodycd,cf.address, cf.idcode license, af.acctno afacctno, cf.fullname, nvl(sec.marginratio,0) marginratio, nvl(sec.marginrate,0) marginrate,
ROUND(greatest((af.mriratio/100 - sec.marginratio/100) * (sec.semaxtotalcallass + GREATEST(balance + ROUND(nvl(avladvance,0)) - ROUND(nvl(ln.marginamt,0),0))),ROUND(nvl(marginovdamt,0))-balance-ROUND(nvl(avladvance,0)))) rtnamt,
af.mriratio, af.mrmratio, af.mrlratio, sec.semaxtotalcallass setotalcallass, af.mrcrlimit, af.mrcrlimitmax, aft.lntype

from cfmast cf, afmast af, cimast ci, aftype aft, v_getsecmarginratio sec,
...
and sec.marginratio<=af.mriratio
