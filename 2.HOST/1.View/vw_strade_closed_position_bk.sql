CREATE OR REPLACE FORCE VIEW VW_STRADE_CLOSED_POSITION_BK AS
...
where exectype in ('NS','MS') AND a.txdate <> b.varvalue AND EXISTS (SELECT afacctno FROM RESULT WHERE a.afacctno = acctno)
AND a.txdate >= '31-may-2010'
--AND a.afacctno = '0001103952'
group by afacctno, codeid
union all
select a.afacctno, codeid, sum(execqtty) TOTALSELLQTTY from vw_odmast_all a, (SELECT to_date(varvalue, 'dd/mm/yyyy') varvalue FROM sysvar WHERE varname = 'CURRDATE') b
where exectype in ('NS','MS') AND a.txdate <> b.varvalue and not EXISTS (SELECT afacctno FROM RESULT WHERE a.afacctno = acctno)
AND a.txdate >= '22-oct-2010'
--AND a.afacctno = '0001103952'
group by afacctno, codeid
