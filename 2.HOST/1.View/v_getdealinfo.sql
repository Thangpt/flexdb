create or replace force view v_getdealinfo as
(
select df.ACCTNO,df.AFACCTNO,df.LNACCTNO,to_char(cf.FULLNAME) FULLNAME,CF.ADDRESS ADDRESS,df.dealtype,
        CF.IDCODE IDCODE,CF.CUSTODYCD CUSTODYCD, DF.FLAGTRIGGER, af.email, df.triggerdate, nvl(df.tlid,'0000') tlid, sb.currprice,
        ...
        ln.FEEINTPAID,ln.INTFLOATAMT,ln.FEEFLOATAMT,
            ...
            when df.RRTYPE='B' then df.custbank
                ...
                ac.cdcontent CALLTYPE,df.DFQTTY,df.BQTTY,df.RCVQTTY,df.CARCVQTTY,df.BLOCKQTTY,DF.CACASHQTTY,df.RLSQTTY,
