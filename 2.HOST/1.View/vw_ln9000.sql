CREATE OR REPLACE FORCE VIEW VW_LN9000 AS
(
    ...
    t."BANKNAME",t."BANKSHORTNAME", t.prinamt + t.intamt + t.feeamt totalamt,
        ...
        SELECT CF.CUSTODYCD, cf.acctno AFACCTNO, CF.FULLNAME,cf.careby, lnt.lnacctno,ln.autoid,
                        ...
                        CASE WHEN NVL(LNT.OVDPAIDAMT,0) = 0 THEN ln.custbank ELSE 'BVSC' END custbank,
                        CASE WHEN NVL(LNT.OVDPAIDAMT,0) = 0 THEN LN.BANKNAME ELSE 'BVSC' END BANKNAME,
                        LN.BANKSHORTNAME
        ...
        FROM
         ...
         (
             SELECT lns.autoid,ln.acctno, ln.trfacctno, lns.rlsdate, lns.overduedate, lns.rate2 LOANRATE,
                 ...
                 NVL(CF.FULLNAME,'BVSC') BANKNAME ,NVL(CF.shortname,'BVSC') BANKSHORTNAME, ftype
