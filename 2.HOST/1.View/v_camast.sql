CREATE OR REPLACE FORCE VIEW V_CAMAST AS
(
...
(CASE WHEN EXRATE IS NOT NULL THEN EXRATE ELSE (CASE WHEN RIGHTOFFRATE IS NOT NULL
    ...
    nvl(schd2.QTTY,0) CAQTTY, CAMAST.ISINCODE, VNCODE
      ...
      (select sum(case when schd.isci= 'Y' then schd.amt else 0 end) amt,
         ...
         sum(round(mst.pitrate *
                       ( CASE WHEN
                         ...
                         THEN 1 ELSE 0 END)
                        *(case when ( schd.isci= 'Y' and aft.vat='Y') 
                               then (case when  mst.catype='016' then schd.intamt
                                     --1.6.1.3: thue CW
					                           WHEN mst.CATYPE in ('028')
                                           then LEAST(round(mst.pitrate * schd.BALANCE * mst.EXPRICE / (to_number(SUBSTR(SYM.EXERCISERATIO,0,INSTR(SYM.EXERCISERATIO,'/') - 1)) / to_number( SUBSTR(SYM.EXERCISERATIO,INSTR(SYM.EXERCISERATIO,'/')+1,LENGTH(SYM.EXERCISERATIO))))/100), schd.AMT)
                                      else schd.amt end)
                           else 0 end) /100
            ))taxamt,
      sum((case when schd.isci= 'Y' then schd.amt else 0 end) - ROUND(mst.pitrate
                         * ( CASE WHEN
                              (CASE WHEN schd.pitratemethod='##' THEN mst.pitratemethod ELSE schd.pitratemethod END) ='SC' THEN 1 ELSE 0 END)
                             *(case when (schd.isci= 'Y' and aft.vat='Y') 
                                    then (case when  mst.catype='016' then schd.intamt 
                                         --1.6.1.3: thue CW
					                           WHEN mst.CATYPE in ('028')
         ...
         schd.camastid
          from caschd schd,camast mst,afmast af, aftype aft, SBSECURITIES SYM
          where schd.deltd='N'
          and mst.deltd='N'
