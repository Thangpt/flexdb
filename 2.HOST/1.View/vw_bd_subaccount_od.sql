CREATE OR REPLACE FORCE VIEW VW_BD_SUBACCOUNT_OD AS
SELECT "CUSTODYCD","TXDATE","REFORDERID","AFACCTNO","ACCTNO","ORGACCTNO","EXECTYPE","REFACCTNO","PRICETYPE","DESC_EXECTYPE","SYMBOL","QUANTITY","PRICE","FEEDBACKMSG","QUOTEPRICE","DESC_BOOK","STATUS","DESC_STATUS","DESC_TIMETYPE","DESC_MATCHTYPE","DESC_NORK","DESC_PRICETYPE","EXECQTTY","EXECAMT","AVEXECPRICE","REMAINQTTY","TXTIME","CANCELQTTY","ADJUSTQTTY","BOOK","VIA","VIACD","CANCELSTATUS","AMENDSTATUS","CURRSECSSION","ODSECSSION","MAKER","CODEID","BRATIO" 
FROM 
    (
    -- OD 
    ...
    WHERE MST.ORSTATUS <> '7' AND CF.ACCTNO=MST.AFACCTNO
        ...
        AND CD1.cdtype ='OD' AND CD1.CDNAME='ORSTATUS' AND CD1.CDVAL= (CASE WHEN MST.REMAINQTTY <> 0 AND MST.EDITSTATUS='C' THEN 'C' WHEN MST.REMAINQTTY <> 0 AND MST.EDITSTATUS='A' THEN 'A' WHEN MST.EDITSTATUS IS NULL AND MST.CANCELQTTY <> 0 THEN '5' WHEN MST.REMAINQTTY = 0 AND MST.CANCELQTTY <> 0 AND MST.EDITSTATUS='C' THEN '3' ELSE MST.ORSTATUS END)
    ...
    WHERE  REFOD.ORSTATUS <> '7' AND MST.DELTD='N' AND CF.ACCTNO=MST.AFACCTNO AND MST.STATUS = 'A' AND MST.acctno = mst.orgacctno
    ...
    WHERE MST.ACCTNO=ROOT.FOACCTNO(+) AND MST.STATUS<>'A' AND substr(MST.EXECTYPE,1,1) <> 'C' AND substr(MST.EXECTYPE,1,1) <> 'A'  AND MST.DELTD='N' AND CF.ACCTNO=MST.AFACCTNO AND SYINFO.SYMBOL=MST.SYMBOL
        AND CD1.cdtype ='FO' AND CD1.CDNAME='STATUS' AND CD1.CDVAL=MST.status
        AND SYS.SYSNAME='CONTROLCODE'  AND CFMAST.CUSTID=CF.CUSTID
        AND CD2.cdtype ='FO' AND CD2.CDNAME='EXECTYPE' AND CD2.CDVAL=MST.EXECTYPE
        AND CD3.cdtype ='FO' AND CD3.CDNAME='BOOK' AND CD3.CDVAL=MST.BOOK
