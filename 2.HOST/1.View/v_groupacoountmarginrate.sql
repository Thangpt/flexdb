CREATE OR REPLACE FORCE VIEW V_GROUPACOOUNTMARGINRATE AS
(
...
ADVLIMIT,MRCRLIMITMAX,MRCRLIMIT,MRCLAMT,MRIRATE,MRMRATE,MRLRATE,DUEAMT,OVAMT,PP,
...
(SELECT mst.fullname, aftype actype, mst.acctno,mst.custid, mst.afacctno, mst.ccycd, mst.lastdate,
      ...
      FROM
      (select mst.*,cd1.cdcontent desc_status,af.advanceline,af.mrirate,af.mrmrate,af.mrlrate,cf.fullname,af.MRCRLIMIT,af.MRCLAMT,af.MRCRLIMITMAX,af.actype aftype,
            CF.email,CF.mobile,AF.fax1 phone1,CF.address,af.groupleader
...
select cf.fullname, af.actype, af.acctno,cf.custid, mst.afacctno, mst.ccycd, mst.lastdate,
            cf.email,cf.mobile,af.fax1 phone1,cf.address,cd1.cdcontent desc_status,
...
(SELECT SUM(CASE WHEN mst.ACCTNO <> mst.groupleader THEN 0 ELSE mst.MRIRATE END) MSTMRIRATE,
        SUM(CASE WHEN mst.ACCTNO <> mst.groupleader THEN 0 ELSE mst.ADVANCELINE END) MSTADVANCELINE ,
           ...
           sum(nvl(mst.mrcrlimitmax,0)) mrcrlimitmax,sum(mst.MRCRLIMIT) MRCRLIMIT,sum(mst.MRCLAMT) MRCLAMT,
      ...
      FROM
      (select mst.*,af.groupleader,af.advanceline,af.mrirate,af.mrmrate,af.mrlrate,af.MRCRLIMIT,af.MRCLAMT,af.MRCRLIMITMAX
