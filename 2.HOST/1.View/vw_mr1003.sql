CREATE OR REPLACE FORCE VIEW VW_MR1003 AS
select custodycd,actype,acctno,fullname,
    ...
    realsellamt, asssellamt, mrirate, MRMRATE, MRLRATE, CALLDATE, CALLTIME,PHONE1, EMAIL,MR1003TYPE ,RTNAMOUNTREF , OVDAMOUNTREF,
    ...
    , NYOVDAMT, MPOVDAMT, navaccount, secoutstanding, BRID,fullnamere,mobilesms,emailibr,EMAILBRID,EMAILPCC,EMAILNV, grpname
FROM
(
...
af.brid,NVL(re.fullname,0) fullnamere,NVL(re.mobilesms,0) mobilesms,FN_GET_EMAILMG(af.acctno,'EMAIL') emailibr,FN_GET_EMAILMG(af.acctno,'EMAILCN') EMAILBRID,FN_GET_EMAILMG(af.acctno,'EMAILPCC') EMAILPCC,FN_GET_EMAILMG(af.acctno,'EMAILNV') EMAILNV
, tl.grpname
...
and (
    ...
    or (ci.ovamt -nvl(ln.NYOVDAMT,0)>1 and ln.liqday >0)
    --chaunh
