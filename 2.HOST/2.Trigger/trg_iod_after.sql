...
begin
  ...
  l_header := l_side || :newval.symbol || ' ';
      ...
      insert into SMSMATCHED
        (autoid, CUSTODYCD, AFACCTNO, TXDATE, HEADER, MATCHQTTY, MATCHPRICE, ORDERID, ORDERQTTY, TOTALQTTY, CREATEDATE)
      values
  ...
  --1.8.2.5: thue quyen
  select isfo_order INTO v_isfo FROM odmast WHERE orderid=:newval.orgorderid AND txdate = :newval.txdate;
  ...
  if :newval.bors='S' and inserting then
    ...
    IF v_isfo = 'N' THEN
        for rec in (
            SELECT * FROM (SELECT se.*, decode(af.acctno,l_afacctno,0,1) lord
                                     FROM sepitlog se, afmast af
                                    WHERE se.afacctno = af.acctno
                                      AND af.custid = v_custid
                                      AND se.codeid = :newval.codeid
        ...
        loop
             IF v_recSTSCHD < rec.QTTY-rec.MAPQTTY then
                ...
                UPDATE SEPITLOG SET MAPQTTY= MAPQTTY + v_recSTSCHD
                    WHERE AUTOID = rec.autoid; /*AFACCTNO=rec.afacctno AND CODEID=rec.codeid AND TXDATE= rec.txdate AND TXNUM=rec.txnum;*/

                INSERT INTO SEPITALLOCATE (CAMASTID,AFACCTNO,CODEID,PITRATE,QTTY,PRICE,ARIGHT,ORGORDERID,TXNUM,TXDATE,CARATE,SEPITLOG_ID) 
                VALUES(rec.CAMASTID, rec.AFACCTNO, rec.CODEID, rec.PITRATE,v_recSTSCHD, v_parvalue, v_recSTSCHD * rec.CARATE * v_parvalue * rec.PITRATE/100,
                ...
                INSERT INTO SEPITALLOCATE (CAMASTID,AFACCTNO,CODEID,PITRATE,QTTY,PRICE,ARIGHT,ORGORDERID,TXNUM,TXDATE,CARATE,SEPITLOG_ID) 
                VALUES(rec.CAMASTID, rec.AFACCTNO, rec.CODEID, rec.PITRATE,rec.qtty - rec.mapqtty, v_parvalue, (rec.qtty - rec.mapqtty) * rec.CARATE * v_parvalue * rec.PITRATE/100,
                        :newval.orgorderid, :newval.TXNUM,:newval.TXDATE,rec.CARATE,rec.AUTOID);
