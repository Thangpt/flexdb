CREATE OR REPLACE TRIGGER "TRG_OTRIGHT_AFTER"
  after insert or update on otright
...
begin
  ...
  if :newval.DELTD = 'N' then
    select custodycd, :newval.afacctno account, fullname, cf.email
    ...
    select af.email
      into l_emailowner
      from afmast af
     where af.acctno = :newval.afacctno;
    ...
    l_datasource := 'select ''' || l_custodycd || ''' custodycode, ''' ||
                    ...
                    ''' fullname, CollectionToString(cast(collect(a.cdcontent) as collection),''</li><li>'') rights
                       ...
                       where a.cdval = otr.otmncode and instr(otright,''Y'')>0
                         and otr.deltd = ''N''
                         and afacctno = ''' ||
                    :newval.afacctno || ''' and authcustid = ''' ||
                    :newval.authcustid || '''';
