CREATE OR REPLACE TRIGGER TRG_ORDERSYS_HA_AFTER
 AFTER 
    ...
    if fopks_api.fn_is_ho_active then
     ...
     if trim(:oldval.sysdesc) <> trim(:newval.sysdesc) then
            ...
            if :newval.sysdesc ='CONNECTION_ESTABLISHED' then
                ...
                l_datasourcesms:= ' Select ''' || 'START HNX GATEWAY' || ''' TASK , ''' || v_strBeginTime|| ''' Begintime FROM dual' ;
                l_datasourcesms2:= ' Select ''' || ' TASK : START HNX GATEWAY - TIME : ' || v_strBeginTime|| ''' detail FROM dual' ;
                for rec in
                (
                ...
                l_datasourcesms:= ' Select ''' || 'STOP HNX GATEWAY' || ''' TASK , ''' || v_strBeginTime|| ''' Begintime FROM dual' ;
                l_datasourcesms2:= ' Select ''' || ' TASK : STOP HNX GATEWAY - TIME : ' || v_strBeginTime|| ''' detail FROM dual' ;
                for rec in
                (
                    Select cdval Email from allcode where CDTYPE ='SY' and CDNAME ='EMAIL'
                )
