...
SELECT
case
     WHEN SP_FORMAT_REGRP_MAPCODE(ma_nhom) LIKE '000018.000017%' THEN 'A. T?ng tr? s?'
     WHEN SP_FORMAT_REGRP_MAPCODE(ma_nhom) LIKE '000018.000019%' THEN 'B. T?ng chi nhánh'
		 WHEN ma_nhom is NOT NULL AND brid LIKE '000%' THEN 'A. T?ng tr? s?'
		 WHEN ma_nhom is NOT NULL AND brid LIKE '010%' THEN 'B. T?ng chi nhánh'
     WHEN  ma_nhom is NULL AND brid LIKE '000%' THEN 'A. T?ng tr? s?'
     WHEN  ma_nhom is NULL AND brid LIKE '010%' THEN 'B. T?ng chi nhánh'
     WHEN brid is NULL AND  afacctno LIKE '000%' THEN 'A. T?ng tr? s?'
     WHEN  brid is NULL AND  afacctno LIKE '010%' THEN 'B. T?ng chi nhánh'
     ELSE 'sid'
END nhom_cha,
case
    WHEN SP_FORMAT_REGRP_MAPCODE(ma_nhom) = '000018.000017' THEN 'A. T?ng tr? s?'
    WHEN SP_FORMAT_REGRP_MAPCODE(ma_nhom) = '000018.000019' THEN 'B. T?ng chi nhánh'
    WHEN SP_FORMAT_REGRP_MAPCODE(ma_nhom) LIKE '000018.000017.000001.000014%' THEN '4. Ð?i lý Sông Lam'
    WHEN SP_FORMAT_REGRP_MAPCODE(ma_nhom) LIKE '000018.000017.000001%' THEN '1. Giao d?ch tr? s?'

    WHEN SP_FORMAT_REGRP_MAPCODE(ma_nhom) LIKE '000018.000017.000021%' THEN '3. Khách hàng t? ch?c'
    WHEN SP_FORMAT_REGRP_MAPCODE(ma_nhom) LIKE '000018.000017.000020%' THEN '5. Phòng giao d?ch M? Ðình'
    WHEN SP_FORMAT_REGRP_MAPCODE(ma_nhom) LIKE '000018.000017.000005%' THEN '6. Phòng giao d?ch 94 Bà Tri?u'
    WHEN SP_FORMAT_REGRP_MAPCODE(ma_nhom) LIKE '000018.000019.000016%' THEN '2. Khách hàng TCCN'
    WHEN SP_FORMAT_REGRP_MAPCODE(ma_nhom) LIKE '000018.000019.000023%' THEN '1. Giao d?ch CN'
    WHEN SP_FORMAT_REGRP_MAPCODE(ma_nhom) LIKE '000018.000019.000012%' THEN '1. Giao d?ch CN'
    WHEN ma_nhom is NULL AND brid LIKE '000%' THEN '9.TK t? do va MG t? do'
    WHEN ma_nhom is NULL AND brid LIKE '010%' THEN '3.TK t? do va MG t? do'
    WHEN brid is NULL AND  afacctno LIKE '000%' THEN '9.TK t? do va MG t? do'
    WHEN brid is NULL AND  afacctno LIKE '010%' THEN '3.TK t? do va MG t? do'
    ELSE '8. Khách hàng khác'
END nhom_con,
case WHEN fullname IS NOT NULL THEN fullname
     WHEN brid is NOT NULL THEN 'MG t? do'
     WHEN brid is NULL THEN 'TK t? do'
...
FROM
    (
		...
		(
				...
				FROM vw_odmast_all od, sbsecurities sb, cfmast cf, afmast af
...
SELECT 'A. T?ng tr? s?' nhom_cha,'7. T? doanh' nhom_con,'T? doanh' fullname,'T? doanh' ma_nhom,
