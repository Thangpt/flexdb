-- Start of DDL Script for Procedure HOSTMSTRADE.SE0061
-- Generated 11/04/2017 2:34:05 PM from HOSTMSTRADE@FLEX_111
...
FROM (
     select af.acctno afacctno,  --txdate,
        ...
        CASE WHEN txdate = v_ToDate THEN v_ToDate + 1
             WHEN txdate <> v_ToDate AND txdate + days >=  v_ToDate THEN v_ToDate + 1
            ELSE txdate + days END to_date,
        --days,
       CASE WHEN sb.sectype IN ('003' ,'006' ,'222') THEN 'TP'
            WHEN sb.sectype IN ('001', '002',  '008','011') THEN 'CP & CCQ' END sectype,
    ...
    group by af.acctno, CASE WHEN txdate < v_FrDate and txdate + days > v_FrDate THEN v_FrDate
            ...
            CASE WHEN sb.sectype IN ('003' ,'006' ,'222') THEN 'TP'
            WHEN sb.sectype IN ('001', '002',  '008','011') THEN 'CP & CCQ' END, cf.fullname, cf.custodycd
    )a
...
-- End of DDL Script for Procedure HOSTMSTRADE.SE0061
