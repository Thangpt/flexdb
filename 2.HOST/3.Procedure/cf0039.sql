  ...
  IF (BRGID  <> 'ALL')
   ...
   THEN*/
    OPEN PV_REFCURSOR
        FOR
        SELECT
          (CASE WHEN length(AUTH1) > 0 AND length(AUTH2) > 0 AND length(AUTH3) > 0 AND length(AUTH4) > 0 AND length(AUTH5) > 0  THEN '0'
                --else
                when length(AUTH1) > 0 OR length(AUTH2) > 0 OR length(AUTH3) > 0 OR length(AUTH4) > 0 OR length(AUTH5) > 0 then
                    (trim(AUTH1
   ...
   FROM(
        SELECT
         ...
         (CASE WHEN CFK.AUT1='1'and CFK.AUT2='2' and CFK.AUT1='1'and CFK.AUT3='3'
               ...
               then '0' end) AUTH6,
         (CASE WHEN CFK.AUT1='1'or CFK.AUT2='2' or CFK.AUT6='6' then '12' end) AUTH7,
        ...
        FROM ( SELECT
             ...
             CF.FULLNAME ,AF.ACCTNO ,CF.CUSTODYCD  ,CF.IDCODE ,CF.IDDATE , CF.ADDRESS,
                   CFA.FULLNAME  FULLNAMEAUTH ,CFMA.IDCODE LICENSENO ,CFA.VALDATE ,CFMA.ADDRESS ADDRESSAUT,
             ...
             FROM CFMAST  CF, AFMAST AF , CFAUTH CFA, CFMAST CFMA
             WHERE  CF.CUSTID =AF.CUSTID
             AND AF.ACCTNO =CFA.ACCTNO
             AND CFA.CUSTID = CFMA.CUSTID
             ...
             AND NVL(CF.CUSTODYCD,'-') like V_STRCUSTODYCD
             ORDER BY AF.acctno ,CF.SHORTNAME)CFK)
             ;
  /* ELSE
      OPEN PV_REFCURSOR
      FOR
SELECT
          (CASE WHEN length(AUTH1) > 0 AND length(AUTH2) > 0 AND length(AUTH3) > 0
           AND length(AUTH4) > 0 AND length(AUTH5) > 0 AND length(AUTH6) > 0 THEN '0'
           ELSE
            (trim(AUTH1
            || case when length(AUTH1) > 0 and length(AUTH2) > 0 then ',' else '' end
            || AUTH2
   ...
   FROM(
        SELECT
         (CASE WHEN CF.AUT4='4'or CF.AUT5='5' then '1' end) AUTH1,
         (CASE WHEN CF.AUT3='3' then '2' end) AUTH2,
         (CASE WHEN CF.AUT9='9' then '3' end) AUTH3,
         (CASE WHEN CF.AUT10='10' then '4' end)AUTH4,
         (CASE WHEN CF.AUT11='11' then '5' end) AUTH5,
         ...
         (CASE WHEN CF.AUT1='1'or CF.AUT2='2' or CF.AUT6='6' then '' end) AUTH7,
               ...
               CF.FULLNAMEAUTH  FULLNAMEAUTH ,CF.LICENSENO ,CF.VALDATE , TO_DATE(CFK.EXPDATE, 'DD/MM/RRRR') EXPDATE, CF.ADDRESSAUT,CF.LOAI_UQ
        FROM ( SELECT
             ( CASE WHEN SUBSTR(CFA.LINKAUTH,1,1) ='Y' THEN '1'END)AUT1,
             ( CASE WHEN SUBSTR(CFA.LINKAUTH,2,1) ='Y' THEN '2'END)AUT2,
             ( CASE WHEN SUBSTR(CFA.LINKAUTH,3,1) ='Y' THEN '3'END)AUT3,
             ...
             ( CASE WHEN SUBSTR(CFA.LINKAUTH,11,1) ='Y' THEN '11'END)AUT11,
             CF.FULLNAME ,AF.ACCTNO ,CF.CUSTODYCD  ,CF.IDCODE ,CF.IDDATE , CF.ADDRESS,
                   CFA.FULLNAME  FULLNAMEAUTH ,CFA.LICENSENO ,CFA.VALDATE ,CFA.ADDRESS ADDRESSAUT,
                   (CASE when to_number(to_char(CFA.EXPDATE,'YYYY'))<=3000 then '1' else '2' end) LOAI_UQ,
                   (CASE when to_number(to_char(CFA.EXPDATE,'YYYY'))<=3000 then to_char(CFA.EXPDATE) else '' end) EXPDATE
