   ...
   END IF;
    ...
    FOR
        SELECT OD.BRID, MAX(OD.BRNAME) BRNAME, OD.TXDATE, OD.SECTYPE, OD.TRADEPLACE TRADEPLACEVL, MAX(A1.CDCONTENT) TRADEPLACE,
            ...
            SUM(CASE WHEN OD.EXECTYPE = 'B' AND OD.CUSTATCOM = 'Y' THEN OD.EXECQTTY ELSE 0 END) B_QTTY_CUSCOM,
            SUM(CASE WHEN OD.EXECTYPE = 'S' AND OD.CUSTATCOM = 'Y' THEN OD.EXECAMT ELSE 0 END) S_AMT_CUSCOM,
            SUM(CASE WHEN OD.EXECTYPE = 'S' AND OD.CUSTATCOM = 'Y' THEN OD.EXECQTTY ELSE 0 END) S_QTTY_CUSCOM,
            SUM(CASE WHEN OD.EXECTYPE = 'B' AND OD.CUSTATCOM = 'N' THEN OD.EXECAMT ELSE 0 END) B_AMT_NONCOM,
            SUM(CASE WHEN OD.EXECTYPE = 'B' AND OD.CUSTATCOM = 'N' THEN OD.EXECQTTY ELSE 0 END) B_QTTY_NONCOM,
            SUM(CASE WHEN OD.EXECTYPE = 'S' AND OD.CUSTATCOM = 'N' THEN OD.EXECAMT ELSE 0 END) S_AMT_NONCOM,
            SUM(CASE WHEN OD.EXECTYPE = 'S' AND OD.CUSTATCOM = 'N' THEN OD.EXECQTTY ELSE 0 END) S_QTTY_NONCOM,
        ...
        (
                ...
                FROM
                            (   SELECT OD.TXDATE, SUBSTR(OD.EXECTYPE,2,1) EXECTYPE, OD.TRADEPLACE, CF.CUSTATCOM, BR.BRID,
                                BR.BRNAME BRNAME,
                                 CASE WHEN SB.SECTYPE IN ('001','011') THEN 'CP'
                                    WHEN SB.SECTYPE IN ('008') THEN 'CCQ'
                                    WHEN SB.SECTYPE IN ('003','006') THEN 'TP' ELSE 'OT' END SECTYPE,
                                OD.EXECAMT EXECAMT,
                                OD.EXECQTTY EXECQTTY,
                               ...
                               (CASE WHEN SB.SECTYPE IN ('001','008','011') AND OD.TRADEPLACE IN ('001','002') THEN TO_NUMBER(SYS1.VARVALUE)
                                        WHEN SB.SECTYPE IN ('001','008','011') AND OD.TRADEPLACE IN ('005') THEN TO_NUMBER(SYS2.VARVALUE)
