-- Start of DDL Script for Procedure HOSTMSTRADE.OD0068
-- Generated 11/04/2017 3:17:56 PM from HOSTMSTRADE@FLEX_111
...
BEGIN
    ...
    v_Clearday := to_number(fn_getsysclearday(I_DATE));
...
IF (CASHPLACE = 'ALL') THEN
    ...
    (
        ...
        SELECT af.custid, cleardate SETTDATE, CHD.TXDATE TRADATE,SB.symbol,chd.tradeplace,
            ...
            0 BF_BAMT,
            0 BF_SAMT
        FROM vw_stschd_tradeplace_all CHD, CFMAST CF, AFMAST AF, SBSECURITIES SB, SEMAST SE, AFTYPE AFT
        WHERE CF.CUSTID = AF.CUSTID AND AF.ACCTNO = SE.AFACCTNO AND SB.CODEID = SE.CODEID
            ...
            AND (case when v_TradePlace = '999' and chd.tradeplace IN ('001','002') then '999'
                    else chd.tradeplace end) like v_TradePlace
            AND chd.tradeplace IN ('001','002','005')
...
-- End of DDL Script for Procedure HOSTMSTRADE.OD0068
