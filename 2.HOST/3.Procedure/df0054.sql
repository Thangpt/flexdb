...
FOR
/*
SELECT case when a.rrtype = 'C' then 'BVSC' else cf.shortname end RRNAME, a.* FROM (
    SELECT LNM.RRTYPE,LNM.CUSTBANK,CF.CUSTODYCD, AF.ACCTNO AFACCTNO, LNM.OPNDATE, LNS.OVERDUEDATE DUEDATE, to_char(LNS.AUTOID) LNACCTNO
    , SB.SYMBOL, DF.DFQTTY - NVL(ODM.SELLQTTY,0) TRADE
    FROM DFTYPE DFT, AFMAST AF, CFMAST CF, vw_lnschd_all LNS, vw_lnmast_all LNM, SBSECURITIES SB, DFMAST DF
    ...
    AND LNM.OPNDATE >= V_F_DATE AND LNS.DUEDATE <= V_T_DATE
...
SELECT case when a.rrtype = 'C' then 'BVSC' else cf.shortname end RRNAME, a.* FROM (
    ...
    cf.fullname
    FROM DFTYPE DFT, AFMAST AF, CFMAST CF, vw_lnmast_all LNM, SBSECURITIES SB,
    (
    ...
    AND LNM.OPNDATE >= V_F_DATE AND df.DUEDATE <= V_T_DATE
    AND CF.CUSTODYCD LIKE v_CUSTODYCD
    AND AF.ACCTNO LIKE v_AFAcctno
    AND LNM.ACCTNO LIKE V_STRNUM
    AND case when V_STROPTION = 'A' then 1 else instr(l_BRID_FILTER,substr(AF.ACCTNO,1,4)) end  <> 0
