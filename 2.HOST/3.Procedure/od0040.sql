...
FOR
     ...
     SELECT  PV_CUSTODYCD p_custodycd, PV_CUSTODYPLACE custodyplace, T.FULLNAME,
                   v_noidetien noi_de_tien, v_chinhanh chi_nhanh, v_noilk noi_luu_ky, v_nhomql nhom_ql, DATE_T ck_thanh_toan,
             ...
             --- TIEN HOA HONG
             case when t.execamt > 0 and t.feeacr=0 AND decode(nvl(bs.bchsts,'N'),'Y','Y','N') = 'N' then
                  ROUND(IO.matchqtty * io.matchprice * t.deffeerate / 100, 2)
             else
               (CASE WHEN (t.execamt * t.feeacr) = 0 THEN 0 ELSE
                   (CASE WHEN T.TXDATE = V_CRRDATE THEN round(t.feeacr * io.matchqtty * io.matchprice / t.execamt,2)
                         ELSE ROUND(io.matchqtty * io.matchprice / t.execamt * t.feeacr, 2) END)
               END)
             end  FEE_AMT_DETAIL,
             --- THUE TNCN
             (CASE WHEN T.EXECTYPE IN('NS','SS','MS') AND T.VAT = 'Y' THEN
              ROUND(IO.MATCHQTTY * IO.MATCHPRICE * v_vat/100, 2) + NVL(T.ARIGHT, 0)
              ELSE 0 END) FEETAX_AMT_DETAIL, p_SIGNTYPE SIGNTYPE,
     (CASE WHEN T.EXECTYPE IN('NS','SS','MS') AND T.VAT = 'Y' AND (SELECT count(*) FROM sbbatchsts WHERE bchdate = t.txdate AND bchmdl = 'SABFB' AND bchsts = 'Y') = 0
             THEN v_vat * NVL(IO.MATCHQTTY,0) * NVL(IO.MATCHPRICE,0) /100 + NVL(sp.ARIGHT, 0)
        ELSE io.iodtaxsellamt + NVL(sp.ARIGHT, 0) END ) taxsellamt
     FROM
            (SELECT AF.ACCTNO, CF.CUSTODYCD, OD.TXDATE, OD.ORDERID, OD.CONTRAORDERID, CF.FULLNAME, CF.IDCODE, CF.IDDATE,
                CF.IDPLACE, CF.ADDRESS, AFT.VAT, OD.EXECTYPE,
                SB.SYMBOL, ODT.DEFFEERATE , OD.FEEACR, io.matchamt EXECAMT, sts.clearday,
             ...
             WHERE       OD.CODEID        =    SB.CODEID
                  ...
                  AND  CASE WHEN PV_SECTYPE = 'ALL' THEN 1
                                    WHEN sb.sectype in ('002', '001', '011', '111', '333') AND PV_SECTYPE = 'CP' THEN 1
                                    WHEN sb.sectype in ('003', '006', '222', '444') AND PV_SECTYPE = 'TP' THEN 1
                                    WHEN sb.sectype in ('008', '111', '444') AND PV_SECTYPE = 'CCQ' THEN 1
                                     ELSE 0 END = 1
                  AND    OD.MATCHTYPE     LIKE V_MATCHTYPE
                  AND    (case when V_TRADEPLACE = '999' and OD.TRADEPLACE IN ('001','002') then '999'
                                       WHEN V_TRADEPLACE = '888' AND OD.tradeplace in ('002','005') THEN '888'
                  ...
                  AND (cf.brid like V_p_STRBRID or instr(V_pV_STRBRID,cf.brid) <> 0 ) --1.5.6.1 MSBS-1907
                  and (cf.brid like V_STRBRID or instr(V_STRBRID,cf.brid) <> 0) --1.5.6.1 MSBS-1907
                  and cf.careby like V_STRCAREBY --1.5.6.1 MSBS-1907
            ) T, VW_IOD_ALL IO, (SELECT * FROM sbbatchsts bs WHERE bs.bchmdl = 'ODFEECAL') bs, (SELECT orgorderid, txnum, sum(aright) aright FROM sepitallocate GROUP BY orgorderid, txnum) sp
     WHERE    IO.DELTD                   <>    'Y' AND      T.ORDERID = IO.ORGORDERID AND io.orgorderid = sp.orgorderid(+) AND io.txnum = sp.txnum(+)
     AND      T.CUSTTYPE_NAME            LIKE  V_ACCTYPE AND      T.TXDATE = bs.bchdate(+)
     AND      T.CUSTODY_PLACE            LIKE  V_CUSTODYPLACE
     AND      CASE WHEN PV_CUSTOTYPE = '002' AND SUBSTR(T.CUSTODYCD, 4,1) = 'P' THEN 1
                   WHEN PV_CUSTOTYPE = '001' AND SUBSTR(T.CUSTODYCD, 4,1) NOT LIKE 'P' THEN 1
                   WHEN PV_CUSTOTYPE = 'ALL' THEN 1
                   ELSE 0 END = 1
