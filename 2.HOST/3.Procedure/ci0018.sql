...
BEGIN
    ...
    IF PV_BALANCE <> 'ALL'  THEN
         V_BALANCE  := PV_BALANCE;
    ELSE
         V_BALANCE  := '%';
    END IF;
...
FROM CIMAST CI_CURR, AFMAST AF, CFMAST CF,
    ...
    (   --- LAY GT THANH TOAN BU TRU TU NGAY T DEN NGAY GD - CAC GIAO DICH BAN CK
        ...
        WHERE    TXDATE >= GET_T_DATE(TO_DATE(V_IN_DATE, SYSTEMNUMS.C_DATE_FORMAT), fn_getsysclearday(V_IN_DATE))       --T2_HoangND edit
...
AND      SUBSTR(CF.CUSTID, 1, 4) LIKE  V_I_BRID
...
having case when ROUND(SUM(CI_CURR.BALANCE - NVL(CI_TR.TR_BALANCE, 0) - NVL(SECU.OD_BUY_SECU, 0)))  < 0 then '-1'
            when ROUND(SUM(CI_CURR.BALANCE - NVL(CI_TR.TR_BALANCE, 0) - NVL(SECU.OD_BUY_SECU, 0)))  = 0 then '0'
            else '1' end like V_BALANCE
