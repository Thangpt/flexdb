CREATE OR REPLACE PROCEDURE ln1003 (
   PV_REFCURSOR       IN OUT   PKG_REPORT.REF_CURSOR,
   p_OPT              IN      VARCHAR2,
   p_BRID             IN      VARCHAR2,
   p_I_DATE           IN      VARCHAR2,
   p_CUSTBANK         IN      VARCHAR2,
   p_F_LOAN_DATE      IN      varchar2,
   p_T_LOAN_DATE      IN      varchar2,
...
--
-- PURPOSE: BAO CAO THONG BAO CHENH LECH LAI MARGIN
-- MODIFICATION HISTORY
-- PERSON       DATE        COMMENTS
-- THENN        16-MAY-2012 CREATED
-- ---------    ------      -------------------------------------------
    ...
    V_F_LOAN_DATE         VARCHAR2(15);
    V_T_LOAN_DATE         VARCHAR2(15);
...
BEGIN
    ...
    V_IN_DATE := p_I_DATE;
    V_F_LOAN_DATE := p_F_LOAN_DATE;
    V_T_LOAN_DATE := p_T_LOAN_DATE;
    ...
    /*IF (P_AFACCTNO <> 'ALL' OR P_AFACCTNO <> '')
    THEN
    ...
    OPEN PV_REFCURSOR FOR
        ...
        (
            ...
            SELECT CF.CUSTODYCD, cf.acctno AFACCTNO, CF.FULLNAME, TO_CHAR(lnt.TXDATE,'DD/MM/YYYY') TXDATE, LN.overduedate, lnt.lnacctno, LN.AUTOID,
                ...
                CASE WHEN NVL(LNT.OVDPAIDAMT,0) = 0 OR LN.CUSTBANK = 'KBSV' THEN ln.custbank ELSE 'OVDB' END custbank,
                LN.CUSTBANK CUSTBANKVL, LN.BANKSHORTNAME,
                CASE WHEN NVL(LNT.OVDPAIDAMT,0) = 0 OR LN.CUSTBANK = 'KBSV' THEN LN.BANKNAME ELSE 'OVDB' END BANKNAME,
                CASE WHEN LN.overduedate < TO_DATE(V_IN_DATE,'DD/MM/YYYY') THEN 0
                    WHEN NVL(LNT.OVDPAIDAMT,0) >0 OR ln.BANKPAIDMETHOD = 'I' THEN NVL(lnt.INTPAIDAMT,0)
            ...
            FROM
                ...
                INNER JOIN
                (
                    SELECT ln.acctno, ln.trfacctno, ln.rlsdate,LNS.overduedate, ln.rate2 LOANRATE, ln.prinperiod LOANPERIOD,
                        ln.prinnml + LN.prinovd prinamt, DECODE(LN.RRTYPE,'C','KBSV', NVL(LN.CUSTBANK,'')) CUSTBANK,
            ...
            (
                SELECT ln.acctno, ln.trfacctno, ln.rlsdate,LNS.overduedate, ln.rate2 LOANRATE, ln.prinperiod LOANPERIOD,
                    ...
                    DECODE(LN.RRTYPE,'C','KBSV', NVL(LN.CUSTBANK,'KBSV')) CUSTBANK,
                    NVL(CF.FULLNAME,'KBSV') BANKNAME, NVL(CF.shortname,'KBSV') BANKSHORTNAME,
                    ln.BANKPAIDMETHOD, LNS.FEEINTPAID, LNS.intpaid, LNS.autoid
                FROM vw_lnmast_all LN, vw_lnschd_all lns, cfmast cf
                WHERE ln.acctno = lns.acctno AND LN.FTYPE = 'DF' AND LNS.reftype IN ('P','GP')
        ...
        /*    SELECT lns.autoid, ln.acctno, ln.trfacctno, lns.rlsdate, lns.overduedate, ln.rate2 LOANRATE, ln.prinperiod MRLOANPERIOD,
                    ...
                    DECODE(LN.RRTYPE,'C','KBSV', NVL(LN.CUSTBANK,'KBSV')) CUSTBANK, NVL(CF.FULLNAME,'KBSV') BANKNAME,
            ...
            FROM
                ...
                INNER JOIN
                (
                    SELECT lns.autoid, ln.acctno, ln.trfacctno, lns.rlsdate, LNS.overduedate, ln.rate2 MRLOANRATE, ln.prinperiod MRLOANPERIOD,
                        LN.orate2 GRLOANRATE, LN.oprinperiod GRLOANPERIOD,
                        lns.nml + lns.ovd MRprinamt, lns.nml + lns.ovd GRPRINAMT,
                        DECODE(LN.RRTYPE,'C','KBSV', NVL(LN.CUSTBANK,'KBSV')) CUSTBANK, NVL(CF.FULLNAME,'KBSV') BANKNAME,
                        ln.BANKPAIDMETHOD, LNS.FEEINTPAID, LNS.intpaid, NVL(CF.shortname,'KBSV') BANKSHORTNAME,
            ...
            SELECT CF.CUSTODYCD, CF.ACCTNO AFACCTNO, CF.FULLNAME, TO_CHAR(ln.overduedate,'DD/MM/YYYY') TXDATE, LN.overduedate, ln.ACCTNO lnacctno, LN.AUTOID,
                to_char(LN.RLSDATE,'DD/MM/YYYY') RLSDATE,
                DECODE(LN.LOANTYPE,'MR',LN.MRLOANRATE,LN.GRLOANRATE) LOANRATE,
                to_date('24/05/2012','DD/MM/YYYY') - ln.rlsdate LOANPERIOD,
            ...
            (
                SELECT lns.autoid, ln.acctno, ln.trfacctno, lns.rlsdate, lns.overduedate, ln.rate2 MRLOANRATE, ln.prinperiod MRLOANPERIOD,
                    ...
                    lns.feeintnmlacr+lns.feeintnmlovd+lns.feeintdue+lns.feeintnml+lns.feeintovd remainfee,
                    DECODE(LN.RRTYPE,'C','KBSV', NVL(LN.CUSTBANK,'KBSV')) CUSTBANK, NVL(CF.FULLNAME,'KBSV') BANKNAME,
                    NVL(CF.shortname,'KBSV') BANKSHORTNAME,
        ...
        WHERE A.LOANTYPE LIKE V_SVTYPE
            ...
            AND TO_DATE(A.RLSDATE,'DD/MM/YYYY') >= TO_DATE(V_F_LOAN_DATE,'DD/MM/YYYY')
            AND TO_DATE(A.RLSDATE,'DD/MM/YYYY') <= TO_DATE(V_T_LOAN_DATE,'DD/MM/YYYY')
            AND A.CUSTODYCD LIKE V_CUSTODYCD
            --AND A.AFACCTNO LIKE V_AFACCTNO
            AND (SUBSTR(A.AFACCTNO,1,4) LIKE V_STRBRID OR INSTR(V_STRBRID,SUBSTR(A.AFACCTNO,1,4)) >0)
            AND A.ORGPAIDAMT >=0
            AND (A.BANKINTPAIDAMT > 0 OR A.INTPAIDAMT > 0)
