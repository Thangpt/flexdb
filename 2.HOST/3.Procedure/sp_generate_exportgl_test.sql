CREATE OR REPLACE PROCEDURE SP_GENERATE_EXPORTGL_TEST (
  v_txdate in VARCHAR,
  ...
  AND TL.TXDATE=TO_DATE(v_txdate,'DD/MM/RRRR') AND TL.DELTD<>'Y' AND TL.TLTXCD LIKE v_tltxcd AND TL.TXNUM = '9900010460'
 /* UNION ALL
  SELECT TXNUM, TXDATE
  FROM TLLOGALL TL, (SELECT DISTINCT TLTXCD FROM EXTPOSTMAP) RFTX WHERE TL.TLTXCD=RFTX.TLTXCD
