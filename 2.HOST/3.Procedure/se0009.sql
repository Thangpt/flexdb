...
select v_OnDate txdate ,sebal.class class, sebal.custid, sebal.fullname, sebal.idcode, sebal.iddate,
    ...
    sum(case when instr(sebal.symbol,'_WFT') <> 0 then 0
            ...
            + */ nvl(khop_qtty.execqtty,0) - NVL(SR_QTTY.se_RETAIL_move_amt,0) end) khop_qtty,
...
*/
    ...
    sum(nvl(RECEIVING,0) - nvl(se_RECEIVING_move_amt,0)-
        nvl(order_buy_today.receiving_st,0) - nvl(order_buy_today.receiving_qtty,0)) CA_RECEIVING,
...
---END NAMNT--
    ...
    nvl(sumdebt,0)sumdebt, nvl(balance,0) balance, basicprice,
       vn_BRID chi_nhanh,vn_TRADEPLACE san_gd ,v_CustodyCD v_custody,sebal.afacctno v_afacc,v_Symbol v_lsymbol, I_DATE ngay_tc
from
( -- Tong so du CK hien tai group by tieu khoan, Symbol
    select cf.custid, a2.cdcontent class, af.acctno afacctno, symbol, se.acctno, symbol_g, cf.fullname,
    ...
    from cfmast cf, afmast af,allcode a2, semast se, --sbsecurities sb
...
---        and SUBSTR(cf.custodycd,4,1) like V_STRaftype
    ...
    group by cf.custid, af.acctno , a2.cdcontent, symbol, se.acctno, symbol_g, cf.fullname,
            cf.idcode, cf.iddate, cf.idplace, cf.address, cf.mobile, cf.custodycd, sectype
) sebal
LEFT JOIN
    (
        SELECT getcurrdate, ci.acctno afacctno, ci.balance, nvl(l_T0AMT,0) l_T0AMT, nvl(l_MRAMT,0) l_MRAMT, trunc(l_T0AMT+l_MRAMT+ci.depofeeamt) sumdebt
        FROM cimast ci,
        (
            SELECT trfacctno, nvl(sum(t0amt),0) l_T0AMT, nvl(sum(marginamt),0) l_MRAMT
            FROM vw_lngroup_all
            GROUP BY trfacctno
        ) debt
        WHERE ci.acctno = debt.trfacctno(+)
            AND getcurrdate = v_OnDate
    UNION ALL
        SELECT txdate, afacctno, max(balance) balance, max(t0amt) t0amt, max(mramt) mramt, max(t0amt)+max(mramt) sumdebt
        FROM tbl_mr3007_log
        WHERE txdate = v_OnDate
        GROUP BY txdate, afacctno
    ) mr ON sebal.afacctno = mr.afacctno
LEFT JOIN
    (
        SELECT getcurrdate txdate, codeid, symbol, basicprice FROM securities_info WHERE getcurrdate = v_OnDate
        UNION ALL
        SELECT histdate txdate, codeid, symbol, basicprice FROM securities_info_hist WHERE histdate = v_OnDate
    ) seinf ON sebal.symbol_g = seinf.symbol
left join
...
(   -- Phat sinh mua chung khoan ngay hom nay
    select se.acctno,
        ...
        sum(case when v_CurrDate > st.txdate then (qtty) else 0 END) receiving_st
...
group by sebal.custid, sebal.fullname, sebal.idcode, sebal.iddate,
    sebal.idplace, sebal.address, sebal.mobile, sebal.custodycd, sebal.afacctno, sebal.symbol_g, sebal.class, nvl(sumdebt,0), nvl(balance,0), basicprice
...
/*    sum(case when instr(sebal.symbol,'_WFT') <> 0 then 0
    ...
    --END CHAUNH---
    sum(nvl(RECEIVING,0) - nvl(se_RECEIVING_move_amt,0)-
        nvl(order_buy_today.receiving_st,0) - nvl(order_buy_today.receiving_qtty,0)) <> 0 or
    sum(nvl(order_buy_today.receiving_qtty,0) + nvl(order_buy_today.receiving_st,0) +  nvl(CA_RECEIVING,0)) <> 0 or
    sum(case when instr(sebal.symbol,'_WFT') <> 0 then 0
...
ORDER BY sebal.custodycd, sebal.afacctno, sebal.symbol_g
    ;
