-- Start of DDL Script for Procedure HOSTMSTRADE.OD0070
-- Generated 11/04/2017 3:15:56 PM from HOSTMSTRADE@FLEX_111
...
FROM     (
              ...
              FROM stschd chd, afmast af, cfmast cf, AFTYPE AFT,
                   (SELECT AF.ACCTNO, (CASE WHEN AFT.COREBANK = 'N' THEN 'MSBS' ELSE AF.BANKNAME END) BANKNAME
        ...
        /*            (select * from odmast where txdate = TO_DATE (i_date, 'DD/MM/YYYY')
              ...
              (
                    SELECT OD.*, CASE WHEN OD.EXECTYPE  IN('NS','SS','MS') THEN STS.CLEARDATE ELSE STS.TRFBUYDT END T_CLEARDAY
                    FROM
                        (
...
FROM (
              ...
              FROM stschdhist chd, afmast af, cfmast cf, AFTYPE AFT,
                   (SELECT AF.ACCTNO, (CASE WHEN AFT.COREBANK = 'N' THEN 'MSBS' ELSE AF.BANKNAME END) BANKNAME
                    FROM AFMAST AF, AFTYPE AFT
                    WHERE AF.ACTYPE = AFT.ACTYPE) AFB,
                   (SELECT * FROM sbsecurities
...
-- End of DDL Script for Procedure HOSTMSTRADE.OD0070
