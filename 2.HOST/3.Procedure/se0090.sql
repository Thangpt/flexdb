...
OPEN PV_REFCURSOR FOR
 ...
 Select sed.txnum , sed.txdate ,se.custodycd, Se.afacctno,
 cf.fullname,cf.IDDate, cf.idcode , cf.idplace ,cf.address,nvl(cf.phone,cf.mobile) CF_Phone,
  se.txdesc, se.symbol,sed.loai_ck Loai ,sed.Khoi_luong , tl.tlname,V_IN_DATE Ngay_VSD
 from cfmast cf ,
 ...
 (SELECT txdate, txnum ,acctno, depotrade khoi_luong, '1' loai_ck  FROM SEDEPOSIT WHERE STATUS = 'D' AND DELTD <> 'Y' and depotrade <> 0
    union all
 select txdate, txnum ,acctno, depoblock khoi_luong, '2' loai_ck from sedeposit where status = 'D' and deltd <> 'Y' and depoblock <> 0)SED
