...
FROM TLLOG TL, SETRAN SE, ALLCODE AL ,TLPROFILES TLPR1 ,TLPROFILES TLPR2,afmast af, cfmast cf
...
UNION ALL
SELECT cf.custodycd,TL.tltxcd , TL.txnum,TL.TXDATE,TL.BUSDATE, SE.namt, TL.MSGACCT MSGACCT, to_char(TL.TXDESC) txdesc ,TLPR1.TLNAME MAKER ,TLPR2.TLNAME CHECKER ,AL.CDCONTENT STATUS
FROM TLLOGALL TL, SETRANA SE, ALLCODE AL ,TLPROFILES TLPR1 ,TLPROFILES TLPR2,afmast af, cfmast cf
WHERE tl.txnum = se.txnum
AND tl.txdate = se.txdate
AND AL.CDTYPE ='SY' AND AL.CDNAME ='TXSTATUS' AND AL.CDVAL =TL.TXSTATUS
AND  TLPR1.TLID(+) =TL.TLID  AND TLPR2.TLID (+)=TL.OFFID
...
union all
select tr.custodycd, to_char(tr.tltxcd), to_char(tr.TXNUM) , tr.txdate, tr.busdate , tr.namt, to_char(acctno) MSGACCT , to_char(tr.trdesc) txdesc  , TLPR1.TLNAME MAKER ,TLPR2.TLNAME CHECKER ,'Hoan tat' STATUS
...
from vw_citran_gen tr ,TLPROFILES TLPR1 ,TLPROFILES TLPR2
...
UNION ALL -- GD SU DUNG CUSTID
SELECT cf.custodycd ,to_char(TL.TLTXCD),to_char(TL.TXNUM),TL.TXDATE,TL.BUSDATE ,TL.MSGAMT,TL.MSGACCT MSGACCT , to_char(TL.TXDESC) txdesc ,TLPR1.TLNAME MAKER ,TLPR2.TLNAME CHECKER ,AL.CDCONTENT STATUS
FROM VW_TLLOG_ALL TL,ALLCODE AL ,TLPROFILES TLPR1 ,TLPROFILES TLPR2,/*afmast af,*/ cfmast cf
WHERE AL.CDTYPE ='SY' AND AL.CDNAME ='TXSTATUS' AND AL.CDVAL =TL.TXSTATUS
AND  TLPR1.TLID(+) =TL.TLID  AND TLPR2.TLID (+)=TL.OFFID
and cf.custid(+)= substr(REPLACE(tL.MSGACCT,'.',''),1,10)
and tl.txdate <= to_date(T_DATE,'DD/MM/RRRR' )
and tl.txdate >= to_date(F_DATE,'DD/MM/RRRR' )
And Tl.Tltxcd Like V_STRTLTXCD
and TL.tltxcd in ('0047')
AND nvl(TL.tlID,'-') LIKE V_STRMAKER
AND nvl(TL.offid,'-') LIKE V_STRCHECKER
and tl.deltd <>'Y'
and cf.custodycd like V_STRCUSTOCYCD
...
UNION ALL -- GD 5502
SELECT cf.custodycd ,to_char(TL.TLTXCD),to_char(TL.TXNUM),TL.TXDATE,TL.BUSDATE ,TL.MSGAMT,af.acctno MSGACCT , to_char(TL.TXDESC) txdesc ,TLPR1.TLNAME MAKER ,TLPR2.TLNAME CHECKER ,AL.CDCONTENT STATUS
FROM VW_TLLOG_ALL TL,ALLCODE AL ,TLPROFILES TLPR1 ,TLPROFILES TLPR2,afmast af,cfmast cf,lnmast ln
WHERE AL.CDTYPE ='SY' AND AL.CDNAME ='TXSTATUS' AND AL.CDVAL =TL.TXSTATUS
AND  TLPR1.TLID(+) =TL.TLID  AND TLPR2.TLID (+)=TL.OFFID
and tl.msgacct=ln.acctno
and ln.trfacctno=af.acctno
and af.custid=cf.custid
and tl.txdate <= to_date(T_DATE,'DD/MM/RRRR' )
and tl.txdate >= to_date(F_DATE,'DD/MM/RRRR' )
And Tl.Tltxcd Like V_STRTLTXCD
and TL.tltxcd in ('5502')
AND nvl(TL.tlID,'-') LIKE V_STRMAKER
AND nvl(TL.offid,'-') LIKE V_STRCHECKER
and tl.deltd <>'Y'
and cf.custodycd like V_STRCUSTOCYCD
...
FROM TLLOGALL TL, ALLCODE AL ,TLPROFILES TLPR1 ,TLPROFILES TLPR2, cfmast cf, afmast af, iodhist io
WHERE  AL.CDTYPE ='SY' AND AL.CDNAME ='TXSTATUS' AND AL.CDVAL =TL.TXSTATUS
AND  TLPR1.TLID(+) =TL.TLID  AND TLPR2.TLID (+)=TL.OFFID
...
SELECT distinct TL.tltxcd , TL.txnum,TL.TXDATE,TL.BUSDATE, tl.msgamt,  af.acctno MSGACCT,
        TL.TXDESC ,TLPR1.TLNAME MAKER ,TLPR2.TLNAME CHECKER ,AL.CDCONTENT STATUS
FROM TLLOG TL, ALLCODE AL ,TLPROFILES TLPR1 ,TLPROFILES TLPR2, cfmast cf, afmast af, iod io
WHERE  AL.CDTYPE ='SY' AND AL.CDNAME ='TXSTATUS' AND AL.CDVAL =TL.TXSTATUS
AND  TLPR1.TLID(+) =TL.TLID  AND TLPR2.TLID (+)=TL.OFFID
AND tl.txnum = io.txnum
AND tl.txdate = io.txdate
