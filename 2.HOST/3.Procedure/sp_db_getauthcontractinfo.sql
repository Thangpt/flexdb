...
-- DECLARE PROGRAM VARIABLES AS SHOWN ABOVE
    ...
    BEGIN
        ...
        ELSIF v_TLTXCD = '2240' OR v_TLTXCD = '2200' THEN -- GUI/ RUT CK
            v_RIGHTPOS := 9;
        ELSIF INSTR('1100/1140/1101/1133/1120/1132/1134/1136/1139/1130/1108/1107/1110/1121/1184/1185',v_TLTXCD) >0 THEN -- GUI/RUT/CK TIEN
            v_RIGHTPOS := 3;
        ELSE
            v_RIGHTPOS := 0;
        END IF;
       ...
       /* SELECT AF.AFTYPE
        ...
        IF v_AFTYPE = 'I' THEN
            ...
            OPEN PV_REFCURSOR FOR
                SELECT AU.AUTOID, 'A' TYP, AU.CUSTID, CF.IDCODE, CF.IDDATE, CF.IDPLACE, CF.FULLNAME, A1.CDCONTENT REF,
                    AU.ACCTNO,TO_CHAR(AU.VALDATE,'DD/MM/YYYY') EFFDATE ,TO_CHAR(AU.EXPDATE,'DD/MM/YYYY') EXPDATE, CF.address
                FROM CFAUTH AU, CFMAST CF, ALLCODE A1
        ...
        ELSIF v_AFTYPE = 'B' THEN
            ...
            OPEN PV_REFCURSOR FOR
               ...
               SELECT 0 AUTOID, 'M' TYP, CF.CUSTID, CF.IDCODE, CF.IDDATE, CF.IDPLACE, CF.FULLNAME, CD.CDCONTENT REF
                  , AF.ACCTNO,to_char(cf.iddate,'DD/MM/RRRR') EFFDATE,nvl(to_char(cf.idexpired,'DD/MM/RRRR'),l_expdate_default) EXPDATE, cf.address
                FROM CFMAST CF, cfrelation LNK, ALLCODE CD,AFMAST AF
                WHERE TRIM(CF.CUSTID)=TRIM(LNK.RECUSTID)
                    AND TRIM(LNK.RETYPE)=CD.CDVAL AND CD.CDTYPE='CF' AND CD.CDNAME='RETYPE'
                    AND AF.CUSTID = TRIM( LNK.CUSTID)
                    --AND SUBSTR(LNK.LINKAUTH,4,2) IN ('YN','NY','YY')
                    AND TRIM(AF.ACCTNO)=V_AFACCTNO
