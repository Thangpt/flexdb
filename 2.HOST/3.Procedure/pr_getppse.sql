...
/*    begin
    ...
    if l_MarginType = 'T' then
        ...
        select l_pp0 PPSE, l_pp0Ref PPSEREF, '---' MRRATIOLOAN, '---' MRPRICELOAN,
                trunc(Floor( l_pp0 / (l_quoteprice* (1+l_DefFeeRate)))/l_TradeLot)*l_TradeLot MAXQTTY  from dual;
        return;
    end if;
    ...
    dbms_output.put_line('l_pp0Ref:'||l_pp0Ref);
    dbms_output.put_line('l_DefFeeRate:'||l_DefFeeRate);
    dbms_output.put_line('l_RskMarginRate:'||l_RskMarginRate);
    dbms_output.put_line('l_MarginPrice:'||l_MarginPrice);
    dbms_output.put_line('l_quoteprice:'||l_quoteprice);
    dbms_output.put_line('l_Advanceline:'||l_Advanceline);
    dbms_output.put_line('l_Trft0amt:'||l_Trft0amt);
    dbms_output.put_line('ppse:'||l_ppSERef);
    ...
    if l_roomchk ='Y' then
        ...
        dbms_output.put_line('l_avlsyqtty:'||l_avlsyqtty);
        dbms_output.put_line('l_avlqtty:'||l_avlqtty);
        if l_ChkSysCtrl = 'Y' then
        ...
        dbms_output.put_line('l_ppSERef:'||l_ppSERef);
        dbms_output.put_line('l_PP0_add:'||l_PP0_add);
    ...
    select l_ppSE PPSE, l_ppSERef PPSEREF,
        ...
        trunc(Floor(l_ppSE / (l_quoteprice* (1+l_DefFeeRate)))/l_TradeLot)*l_TradeLot MAXQTTY
    from dual;
    return;
EXCEPTION WHEN others THEN
