-- Start of DDL Script for Procedure HOSTMSTRADE.GETSECINFO4SELLEST
-- Generated 11/04/2017 3:24:32 PM from HOSTMSTRADE@FLEX_111

CREATE OR REPLACE 
PROCEDURE getsecinfo4sellest (
...
BEGIN
    ...
    BEGIN
        SELECT TO_NUMBER(varvalue) INTO v_dblCLEARDAY FROM sysvar WHERE varname = 'CLEARDAY' AND grname = 'OD';
    ...
    FOR
        SELECT SE.AFACCTNO, SEIF.SYMBOL, SE.TRADE /*+ SE.MORTAGE*/ - NVL(OD.SECUREAMT,0) QUANTITY,
             ...
             nvl(AFS.MRRATIORATE,0) MRRATE,
             ROUND((FLOOR((SE.TRADE /*+ SE.MORTAGE*/ - NVL(OD.SECUREAMT,0))/SEIF.TRADELOT)*SEIF.TRADELOT)*SEIF.FLOORPRICE*(1 - ODT.DEFFEERATE/100 - TO_NUMBER(SYS.VARVALUE)/100)*(1 - (getduedate (getcurrdate,'B','000',v_dblCLEARDAY)-getcurrdate)*ADT.ADVRATE/360/100)) SELLAMT,
             ...
             ROUND((FLOOR((SE.TRADE /*+ SE.MORTAGE*/ - NVL(OD.SECUREAMT,0))/SEIF.TRADELOT)*SEIF.TRADELOT) * SEIF.FLOORPRICE * (1 - ODT.DEFFEERATE/100 - TO_NUMBER(SYS.VARVALUE)/100)*(1 - (getduedate (getcurrdate,'B','000',v_dblCLEARDAY)-getcurrdate)*ADT.ADVRATE/360/100))
             ...
             SEIF.TRADELOT, SEIF.TRADEUNIT, SB.TRADEPLACE,  ODT.DEFFEERATE FEERATE, TO_NUMBER(SYS.VARVALUE) TAXRATE, ROUND((getduedate (getcurrdate,'B','000',v_dblCLEARDAY)-getcurrdate)*ADT.ADVRATE/360, 4) ADVRATE
...
-- End of DDL Script for Procedure HOSTMSTRADE.GETSECINFO4SELLEST
