...
OPEN PV_REFCURSOR FOR
        ...
        FROM
            ...
            (
                SELECT re.autoid ma_nhom, re.fullname grname, re.custid||re.actype acctno, cf.fullname ten_truong_nhom,
                ...
                FROM regrp re, cfmast cf WHERE re.custid = cf.custid
                UNION all
                SELECT re.autoid ma_nhom, re.fullname grname, re.custid||re.actype acctno, cf.fullname ten_truong_nhom,
                    re.custid custid, re.actype, re.effdate frdate, re.expdate todate, 'Truong phong' vi_tri--, SP_FORMAT_REGRP_MAPCODE(autoid) grpcode
                FROM regrphist re, cfmast cf WHERE re.custid = cf.custid
            ) grp,
            (
        ...
        where  br.acctno = tran.acctno
            ...
            AND br.CUSTID = rcf.custid --AND grp.acctno = '00010000750008'
...
select
    ...
    ,SP_FORMAT_REGRP_MAPCODE(ma_nhom) cap
    ,c2.fullname ten_moi_gioi
    ...
    , nvl(case when rerole <> 'RD' and ispaydsf = 'Y' then
               (case when   ruletype = 'T'--decode(ruletype,'T',0,( sum(feeacr) -    sum(execamt) * DECODE(Rf.CALTYPE,'0001',rf.RERFRATE,0)/100) * icrate / 100) = 0
                            ...
                            then   ( sum(nvl(feeacr,0)) -  sum(nvl(execamt,0) * DECODE(rf.CALTYPE,'0001',Rf.RERFRATE,0)/100))
                            * fn_idelta_rd(rdcustid, VF_DATE, VT_DATE) / 100
...
SELECT  kh.afacctno so_tk_kh, mg.custid, kh.reacctno so_tk_MG, retype.rerole,  mg.custid custid_mg,
    ...
    FROM reaflnk kh,recflnk mg,recfdef,retype,-- rerfee,
        (select g.refrecflnkid ma_nhom, g.reacctno, r.fullname ten_nhom, c.fullname ten_truong_nhom, g.clstxdate, g.todate, g.frdate
                from regrplnk g, regrp r, cfmast c
                where g.refrecflnkid = r.autoid and c.custid = r.custid ) g,
    ...
    WHERE OD.afacctno = kh.afacctno
        and od.txdate between /*VF_DATE AND VT_DATE--*TO_DATE(VF_DATE,'DD/MM/RRRR') and TO_DATE(VT_DATE,'DD/MM/RRRR')
        ...
        and to_char(g.ma_nhom) like V_GROUPID
...
/*and rdcustid = c3.custid*/
/*and rdcustid like V_RDCUSTID
and rdrerole like V_RDREROLE*
group by
