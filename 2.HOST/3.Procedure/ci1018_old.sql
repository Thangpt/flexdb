CREATE OR REPLACE PROCEDURE ci1018_old(
...
cf.custodycd,af.acctno, '' custodycdc,'' acctnoc,ci.namt /*tl.msgamt*/ amt,nvl(mk.tlname,'') mk,nvl(ck.tlname,'')  ck,
...
'1101','1120','1130','1111','1185','1188') --Chaunh bo 3384, 1188
...
--union
--begin chaunh
--phi chuyen tien ra ngan hang
/*select ' ' tltxcd, utf8nums.c_const_RPT_CI1018_Phi_DESC txdesc,tl.txnum,tl.busdate ,tl.txdate,cf.custodycd,af.acctno,'' custodycdc,'' acctnoc
...
, tltx.txdesc || ' - ' || tl.tltxcd  trdesc, ci.txtype
from vw_tllog_all TL ,vw_citran_gen ci,afmast af,cfmast cf, afmast afc,cfmast cfc ,tltx  , tlprofiles mk,tlprofiles ck
where tl.txnum =ci.txnum and tl.txdate =ci.txdate and ci.acctno= af.acctno and cf.custid =af.custid
and ci.acctno =afc.acctno and afc.custid =cfc.custid
and tltx.tltxcd = tl.tltxcd
and  tl.tlid = mk.tlid(+)
and  tl.OFFID =ck.tlid(+)
and tl.tltxcd in ('1101','1120','1130','1111','1185','1188') and ci.field ='BALANCE'
and ci.txcd ='0028'
and decode (V_STRTYPEDATE,'002',TL.busdate,TL.txdate)>=to_date(F_DATE,'DD/MM/YYYY')
AND decode (V_STRTYPEDATE,'002',TL.busdate,TL.txdate)<=to_date(T_DATE,'DD/MM/YYYY')
AND TL.TLTXCD LIKE V_STRTLTXCD
...
AND AF.acctno LIKE V_STRPV_AFACCTNO
union*/
-- phi lo le, phi rut luu ky
/*select tl.tltxcd,tltx.txdesc ,tl.txnum,tl.busdate ,tl.txdate,cf.custodycd,af.acctno, '' custodycdc,'' acctnoc
,ci.namt amt,nvl(mk.tlname,'') mk,nvl(ck.tlname,'')  ck,nvl(AF.brid,'') brid,nvl(tl.tlid,'') tlid,nvl(tl.OFFID,'') OFFID,
''bankid,af.corebank
,decode(af.bankname,'---',utf8nums.c_const_COMPANY_NAME,af.bankname) bankname
,ci.trdesc, ci.txtype
...
and  tl.tlid = mk.tlid(+)
and  tl.OFFID =ck.tlid(+)
and tl.tltxcd in ('2200','8894','3382','3383','0088','1113') and ci.field ='BALANCE'
and ci.txcd ='0011'
and decode (V_STRTYPEDATE,'002',TL.busdate,TL.txdate)>=to_date(F_DATE,'DD/MM/YYYY')
AND decode (V_STRTYPEDATE,'002',TL.busdate,TL.txdate)<=to_date(T_DATE,'DD/MM/YYYY')
AND TL.TLTXCD LIKE V_STRTLTXCD
...
SELECT case when  tl.tltxcd in ('3350','3354') then '3342'
        ...
        case when tl.tltxcd in ('1101','1120','1130','1111','1185','1188') and ci.txcd = '0028' then  utf8nums.c_const_RPT_CI1018_Phi_DESC
            else  tltx.txdesc end txdesc ,tl.txnum,tl.busdate ,tl.txdate,cf.custodycd,af.acctno, '' custodycdc,
            '' acctnoc,ci.namt/*tl.msgamt*/ amt,nvl(mk.tlname,'') mk,nvl(ck.tlname,'')  ck,nvl(tl.brid,'') brid,nvl(tl.tlid,'') tlid,nvl(tl.OFFID,'') OFFID,
''bankid,af.corebank,decode(af.bankname,'---',utf8nums.c_const_COMPANY_NAME,af.bankname) bankname ,
case when tl.tltxcd in ('1101','1120','1130','1111','1185','1188') and ci.txcd = '0028' then tltx.txdesc || ' - ' || tl.tltxcd
     when tl.tltxcd in ('2200','8894','3382','3383','0088','1113') and ci.txcd = '0011' then ci.trdesc else tl.txdesc end trdesc, ci.txtype
FROM vw_tllog_all TL,afmast af,cfmast cf ,tltx, tlprofiles mk,tlprofiles ck, vw_citran_gen ci
...
and tl.msgamt >0
union
SELECT   tl.tltxcd||'T'|| sts.trfbuyext  tltxcd ,tltx.txdesc ,tl.txnum,tl.busdate ,tl.txdate,cf.custodycd,af.acctno, '' custodycdc,'' acctnoc,tl.msgamt amt,nvl(mk.tlname,'') mk,nvl(ck.tlname,'')  ck,nvl(tl.brid,'') brid,nvl(tl.tlid,'') tlid,nvl(tl.OFFID,'') OFFID,
''bankid,af.corebank,decode(af.bankname,'---',utf8nums.c_const_COMPANY_NAME,af.bankname) bankname,nvl(ci.trdesc,ci.txdesc) trdesc, ci.txtype
FROM vw_tllog_all TL,afmast af,cfmast cf ,tltx, tlprofiles mk,tlprofiles ck,vw_citran_gen CI, vw_stschd_all sts
where tl.msgacct=af.acctno and af.custid = cf.custid
and tltx.tltxcd = tl.tltxcd
...
union
SELECT  tl.tltxcd   tltxcd ,tltx.txdesc ,tl.txnum,tl.busdate ,tl.txdate,cf.custodycd,af.acctno, '' custodycdc,'' acctnoc,ci.namt amt,nvl(mk.tlname,'') mk,nvl(ck.tlname,'')  ck,nvl(tl.brid,'') brid,nvl(tl.tlid,'') tlid,nvl(tl.OFFID,'') OFFID,
''bankid,af.corebank,decode(af.bankname,'---',utf8nums.c_const_COMPANY_NAME,af.bankname) bankname,CI.trdesc, ci.txtype
FROM vw_tllog_all TL,afmast af,cfmast cf ,tltx, tlprofiles mk,tlprofiles ck,
 vw_citran_gen   ci
where ci.acctno=af.acctno and af.custid = cf.custid
