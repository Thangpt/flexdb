...
SELECT SCHD.RLSDATE, CF.CUSTODYCD, CI.ACCTNO AFACCTNO, CF.FULLNAME, SCHD.ACCTNO, AFT.ACTYPE, SY.VARVALUE BUSDATE,
    ...
    CASE WHEN (SCHD.NML+SCHD.OVD) <> 0 THEN (TO_DATE(SY.VARVALUE,'DD-MM-YYYY') -
        TO_DATE(SCHD.RLSDATE,'DD-MM-YYYY') ) ELSE (TO_DATE(SCHD.OVDACRDATE,'DD-MM-YYYY') -
            TO_DATE(SCHD.RLSDATE,'DD-MM-YYYY') )  END DEALDAYS,
    nvl(reaflnk.re_name,' ') reman

FROM (SELECT * FROM LNSCHD UNION ALL SELECT * FROM LNSCHDHIST) SCHD, LNMAST MST, CFMAST CF,
     (SELECT max(cf.fullname) re_name, afacctno, max(cf.custid) custid FROM reaflnk re, cfmast cf
        WHERE re.deltd = 'N' AND re.status = 'A' AND cf.custid = substr(reacctno,1,10) GROUP BY re.afacctno
     ) reaflnk,
