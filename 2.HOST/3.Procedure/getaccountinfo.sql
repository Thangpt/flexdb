...
BEGIN
    ...
    if v_margintype='N' or v_margintype='L' then
        ...
        OPEN PV_REFCURSOR FOR
          ...
          greatest(NVL(ADV.AVLADVANCE,0) +nvl(A.mrcrlimit,0)+
          --1.5.8.9 MSBS-2055
          CASE WHEN nvl(a.deal,'Y') = 'N'
              THEN greatest(least(ROUND(least(nvl(a.balance-NVL(b.secureamt,0) + NVL(ADV.avladvance,0) + v_getsec.senavamt -
                   NVL(a.ovamt,0),0) * a.t0loanrate /100,
                   nvl(v_getsec.SELIQAMT2,0))),nvl(a.advanceline,0)),0)
             ELSE nvl(a.advanceline,0) END
          + BALANCE- ODAMT - dfdebtamt - dfintdebtamt- NVL (ADVAMT, 0)-NVL(SECUREAMT,0) - RAMT,0) PP,
          ...
          CASE WHEN nvl(a.deal,'Y') = 'N'
              THEN greatest(least(ROUND(least(nvl(a.balance-NVL(b.secureamt,0) + NVL(ADV.avladvance,0) + v_getsec.senavamt -
                   ...
                   nvl(v_getsec.SELIQAMT2,0))),nvl(a.advanceline,0)),0)
             ELSE nvl(a.advanceline,0) END
          + BALANCE- ODAMT - dfdebtamt - dfintdebtamt- NVL (OVERAMT, 0)-NVL(SECUREAMT,0) - RAMT AVLLIMIT
        FROM
            (
                SELECT AF.ACCTNO,CF.IDCODE LICENSE,CF.CUSTID, CF.CUSTODYCD, CF.FULLNAME, CF.ADDRESS,
                CI.COREBANK, CI.RAMT, CI.BALANCE, CI.ODAMT,ci.dfdebtamt, ci.dfintdebtamt, AF.BRATIO,
                AF.ACTYPE, NVL(AP.AAMT,0) AAMT , CI.BALANCE - CI.ODAMT + NVL(AP.AAMT,0) TOTAL,
                AF.ADVANCELINE,AF.MRCRLIMIT,AF.MRCLAMT,AF.MRCRLIMITMAX, CF.CONTRACTCHK, 
                af.deal, cf.t0loanrate, ci.ovamt
...
/*          GREATEST( LEAST((NVL(A.MRCRLIMIT,0) + NVL(SE.SEAMT,0)+
          ...
          nvl(a.balance - nvl(secureamt,0) + nvl(adv.avladvance,0) +
          --1.5.8.9 MSBS-2055
          CASE WHEN nvl(a.deal,'Y') = 'N'
              THEN greatest(least(ROUND(least(nvl(a.balance-NVL(b.secureamt,0) + NVL(ADV.avladvance,0) + v_getsec.senavamt -
                   NVL(a.ovamt,0),0) * a.t0loanrate /100,
                   nvl(v_getsec.SELIQAMT2,0))),nvl(a.advanceline,0)),0)
             ELSE nvl(a.advanceline,0) END
          + least(nvl(a.mrcrlimitmax,0)+nvl(a.mrcrlimit,0),nvl(a.mrcrlimit,0) + nvl(se.seamt,0)) - nvl(a.odamt,0) - a.dfdebtamt - a.dfintdebtamt,0) pp,
          NVL(ADV.AVLADVANCE,0) +
          --1.5.8.9 MSBS-2055
          CASE WHEN nvl(a.deal,'Y') = 'N'
              THEN greatest(least(ROUND(least(nvl(a.balance-NVL(b.secureamt,0) + NVL(ADV.avladvance,0) + v_getsec.senavamt -
                   ...
                   nvl(v_getsec.SELIQAMT2,0))),nvl(a.advanceline,0)),0)
             ELSE nvl(a.advanceline,0) END
          + NVL(A.MRCRLIMITMAX,0)+nvl(a.mrcrlimit,0)- DFODAMT + BALANCE- ODAMT - dfdebtamt - dfintdebtamt- NVL (OVERAMT, 0)-NVL(SECUREAMT,0) - RAMT AVLLIMIT
           FROM
        (SELECT AF.ACCTNO,CF.IDCODE LICENSE,CF.CUSTID, CF.CUSTODYCD, CF.FULLNAME,CF.ADDRESS,CI.COREBANK,
        CI.RAMT, CI.BALANCE, CI.ODAMT,ci.dfdebtamt, ci.dfintdebtamt, CI.DFODAMT, AF.BRATIO,AF.ACTYPE,
        NVL(AP.AAMT,0) AAMT , CI.BALANCE - CI.ODAMT + NVL(AP.AAMT,0) TOTAL,AF.ADVANCELINE,AF.MRCRLIMIT,
        AF.MRCLAMT,AF.MRCRLIMITMAX, CF.CONTRACTCHK, af.deal, cf.t0loanrate, ci.ovamt
         FROM CFMAST CF INNER JOIN AFMAST AF ON CF.CUSTID=AF.CUSTID
         INNER JOIN CIMAST CI ON AF.ACCTNO=CI.AFACCTNO
         LEFT JOIN (SELECT AFACCTNO ACCTNO, SUM(AMT-AAMT-FAMT+PAIDAMT+PAIDFEEAMT) AAMT FROM STSCHD WHERE DUETYPE = 'RM' AND STATUS='N' AND DELTD <> 'Y' AND AFACCTNO = V_AFACCTNO GROUP BY AFACCTNO) AP ON TRIM(AF.ACCTNO) = TRIM(AP.ACCTNO)
        ...
        SELECT v_margintype MRTYPE,v_isPPUsed ISPPUSED,ACCTNO,LICENSE,CUSTID,CUSTODYCD,FULLNAME,ADDRESS,CONTRACTCHK,v_margindesc TERM,COREBANK,
          ...
          v_aamt aamt, v_total total, greatest(
          --1.5.8.9 MSBS-2055
          CASE WHEN nvl(a.deal,'Y') = 'N'
              THEN greatest(least(ROUND(least(nvl(a.balance-NVL(b.secureamt,0) + v_aamt + v_getsec.senavamt -
                   NVL(a.ovamt,0),0) * a.t0loanrate /100,
                   nvl(v_getsec.SELIQAMT2,0))),nvl(a.advanceline,0)),0)
             ELSE nvl(a.advanceline,0) END
         + v_pp,0) pp,
         ...
         + v_AVLLIMIT AVLLIMIT
           FROM
        (SELECT AF.ACCTNO,CF.IDCODE LICENSE,CF.CUSTID, CF.CUSTODYCD, CF.FULLNAME,CF.ADDRESS,CI.COREBANK,
        CI.RAMT, CI.BALANCE, CI.ODAMT,ci.dfdebtamt, ci.dfintdebtamt, AF.BRATIO,AF.ACTYPE, NVL(AP.AAMT,0) AAMT ,
        CI.BALANCE - CI.ODAMT + NVL(AP.AAMT,0) TOTAL,AF.ADVANCELINE,AF.MRCRLIMIT,AF.MRCLAMT,AF.MRCRLIMITMAX,
        CF.CONTRACTCHK, af.deal, cf.t0loanrate, ci.ovamt
         FROM CFMAST CF INNER JOIN AFMAST AF ON CF.CUSTID=AF.CUSTID
         INNER JOIN CIMAST CI ON AF.ACCTNO=CI.AFACCTNO
         LEFT JOIN (SELECT AFACCTNO ACCTNO, SUM(AMT-AAMT-FAMT+PAIDAMT+PAIDFEEAMT) AAMT FROM STSCHD WHERE DUETYPE = 'RM' AND STATUS='N' AND DELTD <> 'Y' AND AFACCTNO = V_AFACCTNO GROUP BY AFACCTNO) AP ON TRIM(AF.ACCTNO) = TRIM(AP.ACCTNO)
         WHERE AF.ACCTNO=V_AFACCTNO) A
