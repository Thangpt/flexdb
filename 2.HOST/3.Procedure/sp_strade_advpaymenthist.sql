...
from (
        ...
        (SELECT sts.afacctno, sts.cleardate, sum(CASE WHEN sts.status <> 'C' AND aft.vat = 'Y' THEN (amt * (1- odt.deffeerate /100 - nvl(iccf.icrate,0)/100))
												  WHEN sts.status <> 'C' AND aft.vat <> 'Y' THEN (amt * (1- odt.deffeerate /100))
												ELSE (amt * (CASE WHEN aft.vat = 'Y' THEN 1 - nvl(iccf.icrate,0)/100 ELSE 1 END)) - (od.feeamt) END) EXECAMT
            FROM vw_stschd_all STS, vw_odmast_all od, odtype odt, aftype aft, afmast af,
			...
			AND odt.actype = od.actype AND af.acctno = od.afacctno AND af.actype = aft.actype AND iccf.actype(+) = aft.actype
...
select ci.txdate advdate, to_char(ci.txnum) txnum , cf.custodycd custodycd,  af.acctno acctno, sts.cleardate stdate,
    CASE WHEN sts.status <> 'C' AND aft.vat = 'Y' THEN sts.amt * (1-(odt.deffeerate/100)-nvl(iccf.icrate,0)/100)
		WHEN sts.status <> 'C' AND aft.vat <> 'Y'  THEN sts.amt * (1-(odt.deffeerate/100))
        ELSE sts.amt * (CASE WHEN aft.vat = 'Y' THEN 1 -nvl(iccf.icrate,0)/100 ELSE 1 END) - od.feeamt
    END SELLAMT,
