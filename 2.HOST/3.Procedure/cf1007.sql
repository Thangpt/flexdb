    ...
    OPEN PV_REFCURSOR FOR
        ...
        (
            ...
            where tse.busdate >= v_FromDate and tse.busdate <= v_ToDate
                ...
                and sectype <> '004'
                and tse.tltxcd NOT IN ('8867','8868','2247','2290','2255') --CHAUNH bo gd 2247,2290 -- HaiLT lay them gd '2244'
            ...
            select max(tse.autoid) orderid, tse.custid, tse.afacctno, max(tse.tllog_autoid) autoid,tse.txtype,tse.txcd,
                tse.busdate, case when tse.TLTXCD = '8867' then 'Bán ' || trim(to_char(sum(tse.namt),'999,999,999,999,999,999')) || ' ' || max(tse.symbol) ||  ' giá ' || round(sum(od.EXECAMT)/ decode(sum(od.EXECQTTY),0,1,sum(od.EXECQTTY)),0) || ' ngày ' || TO_CHAR(tse.busdate,'DD/MM/RRRR')
                             else  'Mua ' || trim(to_char(sum(tse.namt),'999,999,999,999,999,999')) || ' ' || max(tse.symbol) || ' giá ' || round(sum(od.EXECAMT)/ decode(sum(od.EXECQTTY),0,1,sum(od.EXECQTTY)),0) || ' ngày ' || TO_CHAR(max(od.TXDATE),'DD/MM/RRRR') end TXDESC,
                 to_char(max(tse.symbol)) symbol,
            ...
            where tse.busdate >= v_FromDate and tse.busdate <= v_todate
                ...
                and sectype <> '004' --AND od.ERROD <> 'Y'
