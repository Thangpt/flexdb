CREATE OR REPLACE PROCEDURE getaccountmarginrate_for_mr(
...
BEGIN
    ...
    if length(v_symbol)>0 then
        select nvl(rsk.mrpricerate,0) mrpricerate,nvl(rsk.mrratiorate,0) mrratiorate,sb.marginrefprice
            into v_mrpricerate,v_mrratiorate,v_marginprice
        from securities_info sb, (select * from afserisk where actype =v_actype) rsk where sb.codeid=rsk.codeid(+) and sb.symbol=v_symbol;
    else
        v_mrpricerate:=0;
        v_marginprice:=0;
        v_mrratiorate:=0;
...
/*                select (case when OUTSTANDING >=0 then 100000 else least(round(NAVACCOUNT/abs(OUTSTANDING),4)*100,100000) end) MARGINRATE,AVLLIMIT from
                (SELECT
                       nvl(af.MRCRLIMIT,0) + nvl(se.SEASS,0)  + nvl(se.trfass,0) + v_quantity* v_mrratiorate/100*least(v_marginprice,v_mrpricerate) NAVACCOUNT,
               ...
               OPEN PV_REFCURSOR FOR
                ...
                (SELECT v_acctno AFACCTNO,
                           ...
                           sum(nvl(af.mrcrlimitmax,0) + nvl(af.mrcrlimit,0)+balance- odamt - dfdebtamt - dfintdebtamt- nvl(secureamt,0) - ramt) avllimit
