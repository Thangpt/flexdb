CREATE OR REPLACE PROCEDURE ln0018 (
...
SELECT  fullname, idcode, max( BANK.bankacctno) bankacctno, sum(ad.amt) amt FROM
(SELECT cf.fullname, cf.idcode,af.acctno, SUM (amt+ads.feeamt) amt,nvl(cfb.shortname,'KBSV') bankcode
...
GROUP BY cf.fullname, cf.idcode,af.acctno,cfb.shortname)ad,
...
GROUP BY fullname, idcode,ad.bankcode
