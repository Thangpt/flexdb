...
SELECT IDATE, CUSTODYCD, AFACCTNO, FULLNAME, CODEID, SYMBOL, DFREFPRICE, SUM(RCVQTTY) RCVQTTY, SUM(BLOCKQTTY) BLOCKQTTY, SUM(OTHERQTTY) OTHERQTTY, SUM(TOTAL) TOTAL FROM (
...
FROM
  v_getdealinfo DFM ,AFMAST AF, CFMAST CF, DFTYPE DFT, LNTYPE LNT, ALLCODE A1,
        (
...
) GROUP BY        IDATE, CUSTODYCD, AFACCTNO, FULLNAME, CODEID, SYMBOL, DFREFPRICE
        ;
