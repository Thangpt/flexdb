...
from  vw_tllog_all tl, vw_setran_gen se, afmast af, cfmast cf,vw_stschd_all sts,vw_citran_gen ci
...
from  vw_tllog_all tl, vw_setran_gen se, afmast af, cfmast cf
...
from  vw_CItran_gen ci, afmast af, cfmast cf, vw_setran_gen se,vw_stschd_all sts
...
and se.afacctno = af.acctno and af.custid = cf.custid  and se.txcd ='0045'
and (af.brid like V_STRBRID or INSTR(V_STRBRID,af.brid) <> 0)
AND TL.deltd<>'Y'
and tl.txdate >= TO_DATE(F_DATE,'DD/MM/YYYY')
AND tl.txdate <= TO_DATE(T_DATE,'DD/MM/YYYY')
...
from  vw_tllog_all tl, vw_citran_gen ci, afmast af, cfmast cf
where tl.txnum = ci.txnum and tl.txdate =ci.txdate and tl.tltxcd ='8856'
and ci.acctno = af.acctno and af.custid = cf.custid  and ci.txcd ='0016'
and (af.brid like V_STRBRID or INSTR(V_STRBRID,af.brid) <> 0)
AND TL.deltd<>'Y'
and tl.txdate >= TO_DATE(F_DATE,'DD/MM/YYYY')
AND tl.txdate <= TO_DATE(T_DATE,'DD/MM/YYYY')
UNION ALL
select tl.tltxcd ||'T'|| sts.trfbuyext  tltxcd , tl.txnum, tl.txdate, af.acctno, cf.custodycd,' ' symbol ,tl.msgamt amt, 0  qtty,tl.txdesc,af.corebank
from  vw_tllog_all tl, vw_citran_gen ci, afmast af, cfmast cf,vw_stschd_all sts
where tl.txnum = ci.txnum and tl.txdate =ci.txdate and tl.tltxcd ='8855'
and ci.acctno = af.acctno and af.custid = cf.custid  and ci.txcd ='0011'
and (af.brid like V_STRBRID or INSTR(V_STRBRID,af.brid) <> 0)
AND TL.deltd<>'Y'
...
from  vw_tllog_all tl, vw_citran_gen ci, afmast af, cfmast cf,vw_stschd_all sts,sbsecurities sb
