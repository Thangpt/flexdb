...
SELECT  max(CASE WHEN SB.SECTYPE IN ('001','011') THEN ' I. CỔ PHIẾU '
        ...
        sb.tradeplace,
        sb.symbol,
        sum(decode(al.cdcontent,'Việt Nam', num.amt,0)) KL_VN  ,
        Sum(decode(al.cdcontent,'Việt Nam', 0,num.amt)) KL_NN  ,
        sum(num.amt) KL_ALL,
    ...
    (
        ...
        ( /*
            ...
            WHERE   TR.TXCD = APP.TXCD
                ...
                AND APP.FIELD IN   ('TRADE','BLOCKED','WITHDRAW','MORTAGE','SECURED','NETTING','DTOCLOSE','WTRADE')
            GROUP BY  TR.ACCTNO
    UNION ALL
            SELECT  SUM ((CASE WHEN APP.TXTYPE = 'D'THEN -TR.NAMT WHEN
                    APP.TXTYPE = 'C' THEN TR.NAMT ELSE 0 END )) AMT, TR.ACCTNO ACCTNO
