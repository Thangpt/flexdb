CREATE OR REPLACE PROCEDURE df0025 (
...
OPEN PV_REFCURSOR
FOR
  SELECT DF.ACCTNO, DF.AFACCTNO, TRALL.TXNUM, DF.TXDATE OPNDATE,
    TRALL.TXDATE EXDATE, LN.OVERDUEDATE OVERDUEDATE,
    TRALL.RLSAMT RLSAMT,
    ROUND(CASE WHEN (TRALL.INTPAID-TRALL.INTOVDACR) = 0 THEN 0 ELSE (TRALL.INTPAID-TRALL.INTOVDACR) END) PHIUYTHAC,
    ---((0.0014/100)*TO_NUMBER(TRALL.TXDATE-DF.TXDATE) * TRALL.RLSAMT) PHIUYTHAC,
    TRALL.INTOVDACR INTOVDACR,
    TRALL.RLSFEEAMT,
    BR.BRNAME BRNAME,
    TYP.ACTYPE
    ...
    (
        ...
        (
            ...
            SELECT TO_CHAR(TR.TXNUM) TXNUM, TR.TXDATE, TR.ACCTNO DFACCTNO,
                (case when TX.FIELD = 'RLSAMT' then TR.NAMT else 0 END) RLSAMT,
                (case when TX.FIELD = 'RLSFEEAMT' then TR.NAMT else 0 END) RLSFEEAMT,
                0 INTOVDACR,
                0 INTPAID
            FROM VW_DFTRAN_ALL TR, APPTX TX, VW_DFMAST_ALL DF
            WHERE TR.TXCD = TX.TXCD AND TR.ACCTNO = DF.ACCTNO
                AND (TX.FIELD = 'RLSAMT' or TX.field = 'RLSFEEAMT') AND TX.APPTYPE = 'DF' AND TX.TXTYPE = 'C'
                AND TR.NAMT > 0
