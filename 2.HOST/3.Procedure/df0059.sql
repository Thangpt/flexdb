...
SELECT * FROM (
   SELECT LNM.RRTYPE,LNM.CUSTBANK,CF.CUSTODYCD, AF.ACCTNO AFACCTNO, LNM.OPNDATE, LNS.DUEDATE, DF.GROUPID, DF.ACCTNO, SB.SYMBOL
    ...
    lnM.rrtype || case when lnM.rrtype = 'B' then cf.shortname when lnM.rrtype = 'C' then 'BVSC' else null end rsctype,
           case when lnM.rrtype = 'C' then 'BVSC'
...
FROM DFTYPE DFT, AFMAST AF, CFMAST CF, vw_lnschd_all LNS,  SBSECURITIES SB, vw_dfmast_all DF
    ...
    and case when l_BANKNAME = 'ALL' then 1
                when l_BANKNAME = 'BVSC' and LNM.rrtype = 'C' then 1
...
SELECT A.CUSTODYCD, A.ACCTNO, A.fullname, A.idcode, A.idplace, A.iddate, A.address, A.phone, A.fax,A.GROUPID LNACCTNO, A.OPNDATE, SYMBOL,PARVALUE,TRADEPLACE,DEBIT, CREDIT,
    'BVSC' RRNAME, SUM(RLSQTTY) TRADE , BBRID v_strbrid,
       case WHEN PV_BANKNAME ='ALL' THEN 'ALL'
            when a.rrtype = 'C' then 'BVSC' else cf.fullname end RRFULLNAME
    FROM (
...
GROUP BY A.CUSTODYCD, A.ACCTNO,A.fullname, A.idcode, A.idplace, A.iddate, A.address, A.phone, A.fax,A.GROUPID, A.OPNDATE, SYMBOL,PARVALUE,TRADEPLACE,DEBIT, CREDIT,
    'BVSC',case WHEN PV_BANKNAME ='ALL' THEN 'ALL'
            when a.rrtype = 'C' then 'BVSC' else cf.fullname end
