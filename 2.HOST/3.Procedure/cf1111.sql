...
select kh.*, dd.fullname dd_name, chuc_vu, dd.idcode dd_idcode,  dd.idplace dd_idplace,
dd.iddate dd_iddate, dd.address dd_address, dd.phone dd_phone, dd.mobile dd_mobile, dd.fax dd_fax, dd.email dd_email,
...
from
    (
    select c.custid, c.fullname name, gioi_tinh.cdcontent gioi_tinh, c.custodycd , a.acctno,
    ...
    and c.custtype = 'B' and a.acctno = PV_AFACCTNO
    ) kh,

    (
    select trim(r.custid) custid, c.fullname, a.cdcontent chuc_vu, c.idcode, c.idplace, c.iddate, c.address, c.phone, c.mobile, c.fax, c.email
    ...
    sysvar s1, sysvar s2
where kh.custid = dd.custid (+)
and s1.grname = 'REPRESENT' and s2.grname = 'REPRESENT'
