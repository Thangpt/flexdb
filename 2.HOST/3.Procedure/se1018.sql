...
SELECT  CASE WHEN SUBSTR(CF.custodycd,4,1)= 'P' THEN 'N01211' WHEN SUBSTR(CF.custodycd,4,1)= 'F' THEN 'N01213' ELSE 'N01212' END SHTK, 'N0121' TYP, 'N012' TYPETVTN,
 ...
 FROM semast se, afmast af, cfmast CF,sbsecurities sb,
      (
       select se.acctno,
       sum(case when a.qttytype = '002' AND a.STATUS ='F'  then a.qtty else 0 end) qtty_hccn_dtoclose ,
       sum(case when a.qttytype = '007' AND a.STATUS ='N'  then a.qtty else 0 end) qtty_TG_blocked
...
SELECT  CASE WHEN SUBSTR(CF.custodycd,4,1)= 'P' THEN 'N01221' WHEN SUBSTR(CF.custodycd,4,1)= 'F' THEN 'N01223' ELSE 'N01222' END SHTK, 'N0122' TYP, 'N012' TYPETVTN,
  ...
  FROM semast se, afmast af, cfmast CF,sbsecurities sb,
      (
      /*SELECT acctno , sum( case when qttytype='002' then qtty else 0 end)   qtty_hccn, sum(case when qttytype<>'002' then qtty else 0 end)   qtty_tg
       FROM semastdtl
       WHERE DELTD <>'Y' and status ='N' group by acctno
      */
       select se.acctno,
       sum( (case when a.qttytype = '002' then a.qtty else 0 end)
       ) qtty_hccn
    from semastdtl a, semast se, sbsecurities sb
    where a.acctno = se.acctno
        and se.codeid = sb.codeid
...
SELECT  CASE WHEN SUBSTR(CF.custodycd,4,1)= 'P' THEN 'N01231' WHEN SUBSTR(CF.custodycd,4,1)= 'F' THEN 'N01233' ELSE 'N01232' END SHTK, 'N0123' TYP, 'N012' TYPETVTN,
  ...
  FROM semast se, afmast af, cfmast CF,sbsecurities sb,
      (SELECT acctno , sum( case when qttytype='002' then qtty else 0 end)   qtty_hccn, sum(case when qttytype<>'002' then qtty else 0 end)   qtty_tg
       FROM semastdtl
       WHERE DELTD <>'Y' and status ='N' group by acctno) SEMASTDTL,
(select tr.acctno,
         sum
