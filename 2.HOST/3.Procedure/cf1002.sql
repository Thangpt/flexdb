...
(
    ...
    from   (select ci.autoid, cf.custodycd, cf.custid,
            ...
            from    (SELECT * FROM vw_citran_all WHERE nvl(bkdate,txdate) BETWEEN v_FromDate AND v_ToDate) CI,
                    (SELECT * FROM VW_TLLOG_ALL WHERE nvl(busdate,txdate) BETWEEN v_FromDate AND v_ToDate) TL, cfmast cf, afmast af, apptx app
            ...
            FROM (SELECT * FROM VW_TLLOG_ALL WHERE nvl(busdate,txdate) BETWEEN v_FromDate AND v_ToDate) TL, cfmast cf, afmast af
...
(
    ...
    from   (select (case when tl.tltxcd = '1153' and app.txtype = 'C'
            ...
            from    (SELECT * FROM vw_citran_all WHERE nvl(bkdate,txdate) BETWEEN v_FromDate AND v_CurrDate) CI,
                    (SELECT * FROM VW_TLLOG_ALL WHERE nvl(busdate,txdate) BETWEEN v_FromDate AND v_CurrDate) TL, cfmast cf, afmast af, apptx app
            where   ci.txdate       =    tl.txdate
            and     ci.txnum        =    tl.txnum
            and     cf.custid       =    af.custid
            and     ci.acctno       =    af.acctno
...
(
    ...
    from   (select ci.autoid, cf.custodycd, cf.custid,
            ...
            ' ' old_dfacctno,
            app.txtype, app.field, tl.autoid tllog_autoid, ci.trdesc, nvl(ci.bkdate, ci.txdate) bkdate
            from    (SELECT * FROM vw_citran_all WHERE nvl(bkdate,txdate) > v_ToDate AND nvl(bkdate,txdate) <= v_CurrDate) CI,
                    (SELECT * FROM VW_TLLOG_ALL WHERE nvl(busdate,txdate) > v_ToDate AND nvl(busdate,txdate) <= v_CurrDate) TL, cfmast cf, afmast af, apptx app
            where   ci.txdate       =    tl.txdate
            and     ci.txnum        =    tl.txnum
            and     cf.custid       =    af.custid
            and     ci.acctno       =    af.acctno
