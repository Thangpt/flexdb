...
IF PV_FUNDTYPE = 'KBSV' AND v_TXTYPE = 'DEPO' THEN
      --- Qui KBSV + NOP
      OPEN PV_REFCURSOR
      FOR
      SELECT  CI.TLTXCD,PV_FUNDTYPE FUND, '' BANKCODE, '' BANKACCTNO, '' BANKNAME, CI.BUSDATE,CI.TXTIME, CI.TXNUM, CF.FULLNAME, CI.CUSTODYCD, CI.ACCTNO, CI.NAMT, CI.TXDESC, CI.TXTYPE
...
ELSIF PV_FUNDTYPE = 'KBSV' AND v_TXTYPE = 'WITH' THEN
      --- Qui KBSV + RUT
      OPEN PV_REFCURSOR
      FOR
      SELECT  CI.TLTXCD, PV_FUNDTYPE FUND,'' BANKCODE, '' BANKACCTNO, '' BANKNAME,  CI.BUSDATE,CI.TXTIME, CI.TXNUM, CF.FULLNAME, CI.CUSTODYCD, CI.ACCTNO, CI.NAMT, CI.TXDESC, CI.TXTYPE
...
ELSIF PV_FUNDTYPE = 'KBSV' AND PV_TXTYPE = 'ALL' THEN
      ---Qui KBSV + NOP & RUT
      OPEN PV_REFCURSOR
      FOR
      SELECT  CI.TLTXCD, PV_FUNDTYPE FUND,'' BANKCODE, '' BANKACCTNO, '' BANKNAME,  CI.BUSDATE,CI.TXTIME, CI.TXNUM, CF.FULLNAME, CI.CUSTODYCD, CI.ACCTNO, CI.NAMT, CI.TXDESC, CI.TXTYPE
...
ELSIF PV_FUNDTYPE = 'ALL' AND v_TXTYPE = 'DEPO' THEN
      OPEN PV_REFCURSOR
      FOR
      SELECT * FROM (
      SELECT  CI.TLTXCD,PV_FUNDTYPE FUND, '' BANKCODE, '' BANKACCTNO, '' BANKNAME, CI.BUSDATE,CI.TXTIME, CI.TXNUM, CF.FULLNAME, CI.CUSTODYCD, CI.ACCTNO, CI.NAMT, CI.TXDESC, CI.TXTYPE, ci.autoid autoid
      ...
      select tl.TLTXCD, PV_FUNDTYPE FUND,
            fld.CVALUE BANKCODE, decode(v_BANKCODE,'%%','ALL',BANK.bankacctno) bankacctno,BANK.FULLNAME BANKNAME,  tl.BUSDATE,tl.TXTIME, tl.TXNUM, CF.FULLNAME,
            CF.CUSTODYCD, AF.ACCTNO, tl.msgamt NAMT, tl.TXDESC, 'C' TXTYPE, tl.autoid AUTOID
            ,'' BENEFACCT, '' BENEFCUSTNAME,'' BENEFBANK
        From vw_tllog_all tl, vw_tllogfld_all fld,
            afmast af, CFMAST CF, BANKNOSTRO BANK
        where tl.TLTXCD IN ('1131','1141')
        ...
        order by  A.busdate,A.autoid;


ELSIF PV_FUNDTYPE = 'ALL' AND v_TXTYPE = 'WITH' THEN
      OPEN PV_REFCURSOR
      FOR
      SELECT * FROM  (
      SELECT  CI.TLTXCD, PV_FUNDTYPE FUND,'' BANKCODE, '' BANKACCTNO, '' BANKNAME,  CI.BUSDATE,CI.TXTIME, CI.TXNUM, CF.FULLNAME, CI.CUSTODYCD, CI.ACCTNO, CI.NAMT, CI.TXDESC, CI.TXTYPE,
        ci.autoid AUTOID,'' BENEFACCT, '' BENEFCUSTNAME,'' BENEFBANK
      FROM    VW_CITRAN_GEN CI, CFMAST CF
      WHERE   CI.CUSTODYCD = CF.CUSTODYCD
      AND     FIELD = 'BALANCE'
      AND     TLTXCD IN ('1100')
      ...
      select tl.TLTXCD, PV_FUNDTYPE FUND,
            fld.CVALUE BANKCODE, decode(v_BANKCODE,'%%','ALL',BANK.bankacctno) bankacctno, BANK.FULLNAME BANKNAME,  tl.BUSDATE,tl.TXTIME, tl.TXNUM, CF.FULLNAME,
            CF.CUSTODYCD, AF.ACCTNO, tl.msgamt NAMT, tl.TXDESC, 'D' TXTYPE, TL.AUTOID
            ,'' BENEFACCT, '' BENEFCUSTNAME,'' BENEFBANK
        From vw_tllog_all tl, vw_tllogfld_all fld,
            afmast af, CFMAST CF, BANKNOSTRO BANK
      ...
      -- Them giao d?ch rut tien 1120,1101
      SELECT tl.TLTXCD,PV_FUNDTYPE FUND,
            '' BANKCODE, '' BANKACCTNO,'' BANKNAME,  tl.BUSDATE,tl.TXTIME, tl.TXNUM, CF.FULLNAME,
            CF.CUSTODYCD, AF.ACCTNO, tl.msgamt NAMT, tl.TXDESC, 'D' TXTYPE,tl.autoid
            , cir.benefacct BENEFACCT, cir.benefcustname BENEFCUSTNAME, case when tl.tltxcd='1101' then cir.benefbank else '' end BENEFBANK
            FROM vw_tllog_all tl ,ciremittance cir,
             afmast af,CFMAST CF
...
ELSIF PV_FUNDTYPE = 'NHG' AND v_TXTYPE = 'DEPO' THEN
  --- --- ---Qui Ngan hang + NOP
      ...
      */
      select tl.TLTXCD, PV_FUNDTYPE FUND,
            fld.CVALUE BANKCODE, decode(v_BANKCODE,'%%','ALL',BANK.bankacctno) bankacctno,BANK.FULLNAME BANKNAME,  tl.BUSDATE,tl.TXTIME, tl.TXNUM, CF.FULLNAME,
            CF.CUSTODYCD, AF.ACCTNO, tl.msgamt NAMT, tl.TXDESC, 'C' TXTYPE ,'' BENEFACCT, '' BENEFCUSTNAME,'' BENEFBANK
        From vw_tllog_all tl, vw_tllogfld_all fld,
            afmast af, CFMAST CF, BANKNOSTRO BANK
        where tl.TLTXCD IN ('1131','1141')
            and tl.msgacct = af.acctno
...
ELSIF PV_FUNDTYPE = 'NHG' AND PV_TXTYPE = 'ALL' THEN
      ...
      */
      ...
      select tl.TLTXCD, PV_FUNDTYPE FUND,
            ...
            (case when tl.TLTXCD IN ('1131','1141') then 'C' else 'D' end) TXTYPE, tl.AUTOID
            ,'' BENEFACCT, '' BENEFCUSTNAME,'' BENEFBANK
        From vw_tllog_all tl, vw_tllogfld_all fld,
            afmast af, CFMAST CF, BANKNOSTRO BANK
        where tl.TLTXCD IN ('1131','1132','1136','1189','1184','1141')
            and tl.msgacct = af.acctno
            and af.custid = cf.custid
            and fld.fldcd = '02'
            and tl.txnum = fld.txnum
      ...
      SELECT tl.TLTXCD,PV_FUNDTYPE FUND,
        ...
        order by  A.busdate,A.autoid
      ;
ELSIF PV_FUNDTYPE = 'ALL' AND PV_TXTYPE = 'ALL' THEN
     OPEN PV_REFCURSOR
      FOR
    SELECT * FROM (
      SELECT  CI.TLTXCD, PV_FUNDTYPE FUND, '' BANKCODE, '' BANKACCTNO, '' BANKNAME, CI.BUSDATE,CI.TXTIME, CI.TXNUM,
        CF.FULLNAME, CI.CUSTODYCD, CI.ACCTNO, CI.NAMT, CI.TXDESC, CI.TXTYPE,ci.autoid
        ,'' BENEFACCT, '' BENEFCUSTNAME,'' BENEFBANK
      FROM    VW_CITRAN_GEN CI, CFMAST CF
      WHERE   CI.CUSTODYCD = CF.CUSTODYCD
      AND     FIELD = 'BALANCE'
