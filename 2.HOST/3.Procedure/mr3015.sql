        ...
        -- HaiLT them dieu kien de lay len 3 ngay lien tiep ti le thuc te < ti le canh bao
            or af.acctno in (
                SELECT AFACCTNO FROM (
                    SELECT DISTINCT A.AFACCTNO,COUNT(*) NUM FROM mr3008_log a, afmast b
                    where a.afacctno = b.acctno AND LOG_ACTION = 'BF-EOD'
                    and a.marginrate < b.mrmrate
                    AND A.TXDATE>= (SELECT MIN(SBDATE) SBDATE FROM (
                                        SELECT      *
                                            FROM        SBCLDR
                                            WHERE       CLDRTYPE='000' AND SBDATE <= TO_DATE((SELECT VARVALUE FROM SYSVAR WHERE VARNAME = 'CURRDATE' AND GRNAME = 'SYSTEM'),'DD/MM/RRRR')
                                            AND HOLIDAY <> 'Y'
...
where --case when l_OPT = 'A' then 1 else instr(l_BRID_FILTER,substr(a.afacctno,1,4)) end  <> 0
    --and
    case when p_ISASSREMAIN = 'ALL' then 1
