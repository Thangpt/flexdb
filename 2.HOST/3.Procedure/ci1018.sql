...
and case when tl.tltxcd in ('1113', '2200','3382','3383') and ci.txcd <> '0011' then 0
    when tl.TLTXCD = '0088' and ci.txcd not in ('0011','0012') then 0
...
and tl.msgamt + ci.namt >0
union ALL
SELECT   tl.tltxcd||'T'|| sts.trfbuyext  tltxcd ,tltx.txdesc ,
...
union ALL
SELECT   ci.tltxcd ,tltx.txdesc ,ci.txnum,ci.busdate ,ci.txdate,cf.custodycd,af.acctno, '' custodycdc,'' acctnoc,ci.NAMT amt,nvl(mk.tlname,'') mk,nvl(ck.tlname,'')  ck,nvl(ci.brid,'') brid,nvl(ci.tlid,'') tlid,nvl(ci.OFFID,'') OFFID,
