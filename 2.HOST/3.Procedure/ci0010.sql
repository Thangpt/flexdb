...
BEGIN
    ...
    WHERE CF.custid = AF.custid AND CI.afacctno = AF.acctno AND AF.corebank = 'N' AND CF.custatcom = 'Y'
    ...
    OPEN PV_REFCURSOR FOR
        SELECT CI.transtype, CI.TRANSSUBTYPE, CI.CUSTBANK, MAX(CI.BANKNAME) BANKNAME, TO_CHAR(CI.TXDATE,'DD/MM/YYYY') TXDATE,
            ...
            SUM(CASE WHEN SUBSTR(CF.custodycd,4,1) = 'P' and  CI.TLTXCD <> '8894' AND CI.TXTYPE = 'D' THEN CI.NAMT
                     WHEN SUBSTR(CF.custodycd,4,1) <> 'P' and  CI.TLTXCD = '8894' AND CI.TXTYPE = 'D' THEN CI.NAMT ELSE 0 END) OW_DRAMT, --phi ban lo le vao phan tu doanh
        ...
        FROM CFMAST CF, AFMAST AF, TLTX TL,
            ...
            SELECT ci.acctno, ci.tltxcd, V_F_DATE txdate, ci.txnum, ci.txcd, ci.namt, ci.field, ci.txtype,
                ...
                CASE WHEN CI.TLTXCD IN ('1140','1131','1132','1107','1100','1136','1184') THEN 11 --chaunh them 1184
                     WHEN CI.TLTXCD IN ('1141','1101','1108','1111','1133') THEN 12
                     WHEN CI.TLTXCD IN ('1114') THEN 13
                     WHEN CI.TLTXCD IN ('1120','1130','1134','1188') THEN 14
            ...
            WHERE ci.tltxcd IN ('1131','1132','1141','1136','1185','1184') --chaunh them 1184
            ...
            WHERE ci.tltxcd IN ('1140','1107','1100','1101','1139','1108','1120','1130','1162','1111','1133','1134','1114',/*'1184',*/'1188') --chaunh bo 1184
...
--                     WHEN CI.TLTXCD IN ('8855') THEN 43
                     ...
                     WHEN CI.TLTXCD IN ('0066') AND CI.TXCD = '0028' THEN 47
                     when ci.tltxcd in ('8894') /*and ci.txcd = '0011'*/ then 48 END TRANSSUBTYPE --chaunh them 8894 phi ban lo le
            FROM (SELECT * FROM vw_citran_gen WHERE BUSDATE >= V_F_DATE AND BUSDATE <= V_T_DATE) ci
                  WHERE ci.tltxcd IN ('8866','8856','0066','8894') --'8855','8865',
            --  AND ci.txdate = '30-apr-2012'
                AND ci.field = 'BALANCE'
               -- and case when ci.tltxcd = '8894' and ci.txcd <> '0011' then 0 else 1 end = 1
            UNION ALL
            -- PHI MUA 8855
            SELECT ci.acctno, ci.tltxcd, V_F_DATE txdate, ci.txnum, ci.txcd, ci.namt, ci.field, decode(ci.tltxcd,'8865','D',ci.txtype) txtype,
                '----' CUSTBANK, '----' BANKNAME, 4 transtype,
                CASE WHEN NVL(ST.TRFBUYEXT,0) =  0 THEN 43 -- Tra phi mua T0
                     WHEN NVL(ST.TRFBUYEXT,0) =  1 THEN 43.1 -- Tra phi mua T1
                     WHEN NVL(ST.TRFBUYEXT,0) =  2 THEN 43.2 -- Tra phi mua T2
                     END TRANSSUBTYPE
            FROM (SELECT * FROM VW_CITRAN_GEN WHERE BUSDATE >= V_F_DATE AND BUSDATE <= V_T_DATE) CI,
                 (SELECT ORGORDERID,MAX(TRFBUYEXT) TRFBUYEXT FROM VW_STSCHD_ALL WHERE DUETYPE ='SM'
                  AND   CLEARDATE >= V_F_DATE AND CLEARDATE <= V_T_DATE  GROUP BY  ORGORDERID) ST
                  WHERE CI.TLTXCD ='8855' AND ci.field = 'BALANCE'
                  AND ci.REF = ST.ORGORDERID(+)
             UNION ALL
           -- GD GIAO TIEN MUA 8865
            SELECT ci.acctno, ci.tltxcd, V_F_DATE txdate, ci.txnum, ci.txcd, ci.namt, ci.field, 'D' txtype,
                '----' CUSTBANK, '----' BANKNAME, 4 transtype,
                 CASE
                        WHEN  NVL(ST.TRFBUYEXT,0) =  1 THEN 41.1--tien mua T1
                        WHEN  NVL(ST.TRFBUYEXT,0) =  2 THEN 42--tien mua T2

                      ELSE  41 END  TRANSSUBTYPE --tien mua T0
            FROM (SELECT * FROM vw_citran_gen WHERE BUSDATE >= V_F_DATE AND BUSDATE <= V_T_DATE) ci,
                    (SELECT ORGORDERID,MAX(TRFBUYEXT) TRFBUYEXT FROM VW_STSCHD_ALL WHERE DUETYPE ='SM'
                    AND   CLEARDATE >= V_F_DATE AND CLEARDATE <= V_T_DATE  GROUP BY  ORGORDERID) ST
            ...
            SELECT ci.acctno, '1153' tltxcd, V_F_DATE txdate , ci.txnum, ci.txcd, ci.namt, ci.field, ci.txtype,
                NVL(CF.custid,'KBSV') CUSTBANK, NVL(CF.shortname,'KBSV') BANKNAME, 5 transtype,
            ...
            FROM
                ...
                FROM (SELECT * FROM adsource WHERE TXDATE >= V_F_DATE AND TXDATE <= V_T_DATE) ci
                WHERE ci.deltd = 'N'
                UNION ALL
           ...
           /*
            -- GD VAY BAO LANH
            SELECT ci.acctno, ci.tltxcd, V_F_DATE txdate, ci.txnum, ci.txcd, ci.namt, ci.field, ci.txtype,
                NVL(CF.custid,'KBSV') CUSTBANK, NVL(CF.shortname,'KBSV') BANKNAME, 6 transtype,
            ...
            */
            ...
            SELECT ln.trfacctno ACCTNO,'5566' TLTXCD, V_F_DATE TXDATE, '' TXNUM, '0012' TXCD,
                   ...
                   case when ln.ftype ='DF' then lnslog.nml + lnslog.ovd + lnslog.paid
                    ...
                    NVL(CF.custid,'KBSV') CUSTBANK, NVL(CF.shortname,'KBSV') BANKNAME, 7 transtype,
                    71  TRANSSUBTYPE
              FROM
                 vw_lnmast_all LN,
            ...
            WHERE ci.field  IN ( 'BALANCE','MBLOCK')
                AND NOT EXISTS(SELECT TL.TLTXCD FROM TLTX TL WHERE TL.TLTXCD = CI.TLTXCD
                                    AND TL.tltxcd IN ('1131','1132','1141','1140','1107','1100','1136','1101',
                                                    ...
                                                    '1111','1133','1134','1144','1145','9100','6600','6690','1114','2635','2651','2653','2656',--them doan giao dich cl+df
                                                    '5565','5562','5502','2666','2664','2624','0088','3384','1182','9101', '8894'
   ...
   union all
    SELECT ci.acctno, ci.tltxcd, V_F_DATE txdate, ci.txnum, ci.txcd, ci.namt, ci.field, ci.txtype,
            ...
            FROM (SELECT * FROM vw_citran_gen WHERE BUSDATE >= V_F_DATE AND BUSDATE <= V_T_DATE) ci
            WHERE ci.field  IN ( 'BALANCE','MBLOCK')
            and ci.tltxcd='5540' and instr(trdesc,'Tra lai khac')<>0
        ...
        WHERE CF.custid = AF.custid AND AF.acctno = CI.ACCTNO AND TL.TLTXCD = CI.TLTXCD AND AF.corebank = 'N' AND CF.custatcom = 'Y'
