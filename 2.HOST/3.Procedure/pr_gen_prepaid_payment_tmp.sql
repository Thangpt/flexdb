...
begin
    ...
    (
            SELECT ci.acctno trfacctno,
                round(BALANCE + nvl(avladvance,0)-nvl(dtl.avlamt,0)-depofeeamt,0) AVLBAL
            ...
            and round(BALANCE + nvl(avladvance,0)-nvl(dtl.avlamt,0)-depofeeamt,0) > 0
    ...
    loop -- rec
        if cspks_lnproc.fn_Gen_Prepaid_Payment_tmp(rec.trfacctno, rec.AVLBAL, 'R', p_err_code) <> systemnums.c_success then
            return;
        end if;
    ...
    loop -- rec
        if cspks_lnproc.fn_Gen_Prepaid_Payment_tmp(rec.trfacctno, rec.AVLBAL, 'N', p_err_code) <> systemnums.c_success then
            return;
        end if;
    ...
    (
            SELECT ci.acctno trfacctno,
                round(BALANCE -NVL (trf.amt, 0) + nvl(avladvance,0)-nvl(dtl.avlamt,0)-depofeeamt,0) AVLBAL
            FROM CIMAST ci, afmast af, aftype aft, mrtype mrt,
                (select sum(depoamt) avladvance,afacctno
                    from v_getAccountAvlAdvance group by afacctno) adv,
                v_getbuyorderinfo b,
            ...
            and round(BALANCE -NVL (trf.amt, 0) + nvl(avladvance,0)-nvl(dtl.avlamt,0)-depofeeamt,0) > 0
            and exists (select 1 from lnmast where ftype = 'AF' and trfacctno = ci.acctno
                            and prinnml + prinovd + intnmlacr + intdue + intovdacr + intnmlovd + feeintnmlacr + feeintdue + feeintovdacr + feeintnmlovd
                                                    + oprinnml + oprinovd + ointnmlacr + ointdue + ointovdacr + ointnmlovd > 0)
            order by ci.acctno
    ...
    loop -- rec
        if cspks_lnproc.fn_Gen_Prepaid_Payment_tmp(rec.trfacctno, rec.AVLBAL, 'L', p_err_code) <> systemnums.c_success then
            return;
        end if;
