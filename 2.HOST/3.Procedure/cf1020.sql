...
-- return;
    ...
    FROM (
        SELECT od.afacctno,
                SUM (CASE WHEN chd.trfbuydt >= v_CurrDate AND chd.trfbuydt >= v_ToDate THEN (od.execamt +  od.feeacr)
                          WHEN trfbuydt < v_CurrDate AND  trfbuydt > v_ToDate THEN (od.execamt +  od.feeacr)
                          ELSE 0 END ) amt
        FROM
        (SELECT * FROM stschd WHERE duetype = 'SM' AND deltd <> 'Y' UNION ALL SELECT * FROM stschdhist WHERE duetype = 'SM' AND deltd <> 'Y') chd,
        (SELECT * FROM odmast union ALL SELECT * FROM odmasthist) od
        WHERE chd.orgorderid = od.orderid AND od.afacctno = v_AFAcctno
        AND chd.txdate <= v_ToDate
        and od.txdate <> v_CurrDate
        GROUP BY od.afacctno
        HAVING SUM (CASE WHEN chd.trfbuydt >= v_CurrDate AND chd.trfbuydt >= v_ToDate THEN (od.execamt +  od.feeacr)
               WHEN trfbuydt < v_CurrDate AND  trfbuydt > v_ToDate THEN (od.execamt +  od.feeacr)
               ELSE 0 END ) <> 0
       );
        ...
        (
            ...
            where tci.busdate >= v_FromDate AND tci.busdate <= v_ToDate
              ...
              select  tci.custid, tci.acctno afacctno,max(tci.tllog_autoid) autoid,tci.txtype,tci.txcd,
                 tci.busdate,
                 case when TCI.TLTXCD = '8865' then ' Tr? ti?n mua CK ngày ' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    when TCI.TLTXCD = '8889' then ' Tr? ti?n mua CK ngày ' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    when TCI.TLTXCD = '8855' then ' Tr? phí mua CK ngày ' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    when TCI.TLTXCD = '8866' then ' Nh?n ti?n bán CK ngày ' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    else  'Tr? phí bán CK ngày ' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    end TXDESC,
                '' symbol, 0 se_credit_amt, 0 se_debit_amt,
            ...
            union all
              ...
              select  tci.custid, tci.acctno afacctno,max(tci.tllog_autoid) autoid,tci.txtype,tci.txcd,
                 tci.busdate,
                 case when tci.tltxcd = '5566' and instr(tci.trdesc,'/CL+/') > 0 then '  Nh?n chuy?n kho?n'
                      when tci.tltxcd in ('5567','5540') and instr(tci.trdesc,'/CL+/') > 0 then '  Chuy?n kho?n ti?n'
            ...
            select af.custid, af.acctno afacctno, -1 autoid, 'C' txtype, '0000' txcd,
                txdate busdate,
                ' Ngân hàng c?p h?n m?c gi?i ngân' txdesc,
                '' symbol, 0 se_credit_amt, 0 se_debit_amt,
                ceil((nvl(od.amt,0))/1000000)*1000000 ci_credit_amt, 0 ci_debit_amt,
                '0000' tltxcd, ' Ngân hàng c?p h?n m?c gi?i ngân' trdesc
            from
            ...
            select af.custid, af.acctno afacctno, 999999999 autoid, 'C' txtype, '0000' txcd,
                txdate busdate,
                ' Ngân hàng thu h?i h?n m?c gi?i ngân ' txdesc,
                '' symbol, 0 se_credit_amt, 0 se_debit_amt,
               0 ci_credit_amt,  ceil((nvl(od.amt,0))/1000000)*1000000 ci_debit_amt,
                '0000' tltxcd, ' Ngân hàng thu h?i h?n m?c gi?i ngân' trdesc
            from
                (select txdate, afacctno, sum(amt) amt from
                    (select od.txdate, od.afacctno, sum((od.cancelqtty+od.remainqtty) * od.quoteprice * od.bratio /100) amt
                        from vw_odmast_all od
