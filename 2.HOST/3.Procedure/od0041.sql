...
-- GET REPORT'S PARAMETERS
    ...
    ELSIF CASHPLACE = '000' THEN
        v_noidetien := ' Cty ch?ng khoán ';
    ELSIF CASHPLACE = '111' THEN
        v_noidetien := ' K?t n?i ngân hàng ';
    ELSE
...
FOR
     ...
     FROM
            (SELECT AF.ACCTNO, CF.CUSTODYCD, OD.TXDATE, OD.ORDERID, OD.CONTRAORDERID, CF.FULLNAME, CF.IDCODE, CF.IDDATE,
                CF.IDPLACE, CF.ADDRESS, AFT.VAT, OD.EXECTYPE, ----A1.CDCONTENT PUTTYPE,
                SB.SYMBOL, ODT.DEFFEERATE , OD.FEEACR, io.matchamt EXECAMT,
                    case when OD.EXECTYPE  IN('NS','SS','MS') then sts.cleardate else sts.trfbuydt end clearday,
