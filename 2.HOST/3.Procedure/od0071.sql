...
FOR
     ...
     SELECT (CASE WHEN BR.BRID = '0001' THEN ' H?i s? '
             WHEN BR.BRID = '1000' THEN ' Chi nhánh HN '
             WHEN BR.BRID = '2000' THEN ' Chi nhánh HCM '
        ELSE TO_CHAR(BR.BRNAME) END) GR_I,
       (CASE WHEN (TRIM(CF.CUSTTYPE) || TRIM(SUBSTR(CF.CUSTODYCD, 4, 1))) = 'IC' THEN ' Cá nhân trong nu?c '
             WHEN (TRIM(CF.CUSTTYPE) || TRIM(SUBSTR(CF.CUSTODYCD, 4, 1))) = 'BC' THEN ' T? ch?c trong nu?c '
             WHEN (TRIM(CF.CUSTTYPE) || TRIM(SUBSTR(CF.CUSTODYCD, 4, 1))) = 'IF' THEN ' Cá nhân nu?c ngoài '
             WHEN (TRIM(CF.CUSTTYPE) || TRIM(SUBSTR(CF.CUSTODYCD, 4, 1))) = 'BF' THEN ' T? ch?c nu?c ngoài '
             WHEN (TRIM(SUBSTR(CF.CUSTODYCD, 4, 1))) = 'P'                       THEN ' T? doanh '
        END) GR_CUSTTYPE,
...
FROM   VW_ODMAST_ALL OD, VW_IOD_ALL IO, AFMAST AF, CFMAST CF, SBSECURITIES SB, BRGRP BR, ODTYPE ODT
WHERE  OD.DELTD                      <>    'Y'
