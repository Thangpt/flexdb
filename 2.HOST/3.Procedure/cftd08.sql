CREATE OR REPLACE PROCEDURE cftd08 (
...
from
(select  se.afacctno  afacctno,se.trade,se.mortage,se.acctno ,se.secured,se.blocked,se.receiving,sb.symbol ,cd2.cdcontent tradeplace, cd.cdcontent econimic
        from semast se,sbsecurities sb,issuers iss,allcode cd, allcode cd2
        where se.status ='A' and se.afacctno =V_STRAFACCTNO
...
(select nvl(sum(dcramt ),0) dcramt,nvl(sum(ddroutamt ),0) ddroutamt, acctno
      from
    ( select   sum ((case when app.txtype = 'C' and app.field='DCRAMT' then (case when tl.tltxcd = '8879' then tl.msgamt else tr.namt end) else 0 end )) dcramt,
               sum ((case when app.txtype = 'C' and app.field='DDROUTAMT' then (case when tl.tltxcd = '8879' then tl.msgamt else tr.namt end) else 0 end )) ddroutamt,
               tr.acctno acctno
        from apptx app, setran tr, tllog tl
        where tr.txcd = app.txcd
            and tl.txnum =tr.txnum
  ...
  union all
      select   sum ((case when app.txtype = 'C' and app.field='DCRAMT' then (case when tl.tltxcd = '8879' then tl.msgamt else tr.namt end) else 0 end )) dcramt,
         ...
         where tr.txcd = app.txcd
               ...
               group by  tr.acctno
    union all
    select sum(AMT) dcramt, 0 ddroutamt, acctno
    from (select * from stschd union select * from stschdhist) where  duetype='RS' and deltd <>'Y'
