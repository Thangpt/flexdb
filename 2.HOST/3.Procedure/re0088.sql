...
select
   ...
   so_tk_kh, so_tk_mg, rerole, custid_mg, retype, r.actype, rdtype, rdcustid,
    r.ten_kh, r.cust_kh, r.ten_mg, CASE WHEN V_REROLE = '%' THEN '%' ELSE a.cdcontent END pa_role
    , ten_nhom, ten_truong_nhom
    , ten_mg_lq
...
SELECT   substr(rd.reacctno,11,4) rdtype, substr(rd.reacctno,1,10) rdcustid , rd.icrate
    ...
    FROM
        (
/*           select kh.afacctno so_tk_kh, mg.custid, kh.reacctno so_tk_MG, retype.rerole,  mg.custid custid_mg,cfmg.fullname ten_mg,
                retype.typename retype, od.execamt, od.feeacr, OD.sectype, retype.actype, ispaydsf, od.txdate, od.afacctno,
                c1.fullname ten_kh,c1.custodycd cust_kh
        from
            (SELECT afacctno,SUM( execamt) EXECAMT, SUM(feeacr) feeacr,OD.TXDATE , sb.sectype FROM VW_odmast_ALL OD , sbsecurities sb
             ...
             AND   od.txdate between to_date(F_DATE,'DD/MM/RRRR') and to_date(T_DATE,'DD/MM/RRRR')AND OD.CODEID = SB.CODEID
             GROUP BY afacctno,sectype,TXDATE
            ) OD
            ,reaflnk kh,recflnk mg,
            recfdef,retype,cfmast cfmg,cfmast c1, afmast af1
        ...
        AND retype.rerole LIKE V_REROLE
        and mg.custid like V_CUSTID
        AND mg.custid =cfmg.custid
        AND od.afacctno = af1.acctno
        AND af1.custid= c1.custid
        AND (mg.brid LIKE V_STRBRID OR instr(V_STRBRID,mg.brid)<> 0)
 */
...
(SELECT cfmast.fullname ten_truong_nhom, tn.fullname ten_nhom, nhom.reacctno FROM regrplnk nhom, regrp tn, cfmast
    ...
    AND tn.custid = cfmast.custid) b, allcode a
...
group by
    so_tk_kh, so_tk_mg, rerole, custid_mg, retype, r.actype, rdtype, rdcustid,
       r.ten_kh, r.ten_mg , CASE WHEN V_REROLE = '%' THEN '%' ELSE a.cdcontent END, ten_nhom, ten_truong_nhom,ispaydsf,
    ruletype,Rf.CALTYPE,rf.RERFRATE, icrate,  r.cust_kh, ten_mg_lq/*, c3.fullname*/
