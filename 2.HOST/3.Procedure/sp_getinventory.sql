...
BEGIN
          ...
          ELSIF (V_CLAUSE IN ('DBNAVGRP','RETAX','RERFEEID','REACTYPE','CITYPE', 'ODTYPE', 'SETYPE', 'AFTYPE', 'RPTYPE', 'FOTYPE', 'CLTYPE', 'LNTYPE', 'DDTYPE', 'MRTYPE', 'MTTYPE','PRTYPE')) THEN
             OPEN PV_REFCURSOR
             FOR
                  ...
                  FROM (SELECT * FROM (SELECT actype INVACCT FROM CITYPE WHERE V_CLAUSE = 'CITYPE'
                        ...
                        UNION ALL
                        SELECT actype INVACCT FROM DBNAVGRP WHERE V_CLAUSE = 'DBNAVGRP'
                        UNION ALL
          ...
          ELSIF (V_CLAUSE = 'AFACCTNO') THEN
             ...
             FOR
                  SELECT SUBSTR(INVACCT,1,4), MAX(ODR)+1 AUTOINV FROM
                  (SELECT ROWNUM ODR, INVACCT
                  FROM (SELECT ACCTNO INVACCT FROM AFMAST WHERE SUBSTR(ACCTNO,1,4)= V_BRID ORDER BY ACCTNO) DAT
                  WHERE TO_NUMBER(SUBSTR(INVACCT,5,6))=ROWNUM) INVTAB
                  GROUP BY SUBSTR(INVACCT,1,4);
            ...
            OPEN PV_REFCURSOR
             FOR
                  SELECT  NVL(MAX(ODR)+1,1) AUTOINV FROM
                    ...
                    WHERE TO_NUMBER(INVACCT)=ROWNUM) INVTAB;
        -- 1.5.7.6|iss:1965
        ELSIF (V_CLAUSE = 'NAVID') THEN
             OPEN PV_REFCURSOR
             FOR
                  SELECT  NVL(MAX(ODR)+1,1) AUTOINV FROM
                    (SELECT ROWNUM ODR, INVACCT
                    FROM (SELECT NAVID INVACCT FROM NAVMANAGE ORDER BY NAVID) DAT
                    WHERE TO_NUMBER(INVACCT)=ROWNUM) INVTAB;
         ---MSBS-2346 1.8.1.9         
         ELSIF (V_CLAUSE = 'AREA') THEN
             OPEN PV_REFCURSOR
             FOR
                  SELECT  NVL(MAX(ODR)+1,1) AUTOINV FROM
                  (SELECT ROWNUM ODR, INVACCT
                  FROM (SELECT AREAID INVACCT FROM AREA ORDER BY AREAID) DAT
                  WHERE TO_NUMBER(INVACCT)=ROWNUM) INVTAB;
		---MSBS-2234 1.1.7.3
         ELSIF (V_CLAUSE = 'LNID') THEN
             OPEN PV_REFCURSOR
             FOR           
                  SELECT  NVL(MAX(ODR)+1,1) AUTOINV FROM
                  (SELECT ROWNUM ODR, INVACCT
                  FROM (SELECT LNID INVACCT FROM LNTYPEXT ORDER BY LNID) DAT
                  WHERE TO_NUMBER(INVACCT)=ROWNUM) INVTAB;
        END IF;
