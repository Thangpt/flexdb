...
OPEN PV_REFCURSOR
    FOR
        ...
        from
            (
                select
                A.CUSTID, A.CUSTODYCD, A.FULLNAME, A.ADDRESS, A.ACCTNO, a.brid, --1.5.4.7 MSBS-1900
                            A.LNACC,a.lnacctno,
                            A.RLSDATE, A.EXPDATE,
                ...
                from
                    (   --lay du no dau ky
                        ...
                        (SELECT CF.CUSTID, CF.CUSTODYCD, CF.FULLNAME, CF.ADDRESS, AF.ACCTNO, cf.brid, --1.5.4.7 MSBS-1900
                        ...
                        FROM CFMAST CF, AFMAST AF,
                            (SELECT * FROM LNMAST UNION ALL SELECT * FROM LNMASTHIST) LN,
                            (SELECT LNS.*,'N' L_HIST FROM LNSCHD LNS where reftype in ('P','GP') UNION ALL  SELECT LNS.*,'Y' L_HIST FROM LNSCHDHIST LNS where reftype in ('P','GP')) LNS,
       ...
       AND (a.brid LIKE V_STRBRID or instr(V_STRBRID,a.brid) <> 0) --1.5.4.7 MSBS-1900
  ;
