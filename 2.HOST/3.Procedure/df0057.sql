...
IS
   ...
   V_STROPTION := OPT;
      l_BANKNAME:=P_RRTYPE; -- ALL, MSBS, CF.SHORTNAME
...
SELECT case when a.rrtype = 'C' then 'MSBS' else cf.shortname end RRNAME, a.*, BBRID v_strbrid,
       case WHEN P_RRTYPE = 'ALL' THEN 'ALL'
            when a.rrtype = 'C' then 'MSBS' else cf.fullname end RRFULLNAME
FROM (
 ...
 SELECT df.txdate opndate,LNM.RRTYPE,LNM.CUSTBANK,CF.CUSTODYCD, AF.ACCTNO AFACCTNO,df.acctno, LNM.ACCTNO LNACCTNO, SB.SYMBOL
    ...
    FROM DFTYPE DFT, AFMAST AF, CFMAST CF, vw_lnmast_all LNM, SBSECURITIES SB,   vw_dfmast_all DF, CFMAST CFB,
