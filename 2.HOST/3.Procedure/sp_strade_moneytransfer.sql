...
v_errcode:=0;
  ...
  ELSE
    ...
    SELECT count(1) INTO v_count FROM AFMAST AF, CFMAST CF WHERE CF.CUSTID=AF.CUSTID and AF.STATUS <> 'C' AND AF.TRADEONLINE='Y' AND CF.CUSTODYCD=v_afacctno;
    if v_count > 1 then
      v_errcode:=308;  --Khai bao nhieu tai khoan dang ki online tren cung 1 ma luu ky chung khoan
      return;
    end if;
    BEGIN
    SELECT AF.ACCTNO INTO v_subaccount FROM AFMAST AF, CFMAST CF WHERE CF.CUSTID=AF.CUSTID and AF.STATUS <> 'C' AND AF.TRADEONLINE='Y' AND CF.CUSTODYCD=v_afacctno;
    EXCEPTION
