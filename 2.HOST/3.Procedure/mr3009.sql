...
if l_CURRDATE = I_DATE then --Lay du lieu trong ngay
    ...
    select l_OPT OPT,l_BRID BRID, l_NEXTDATE RPTDATE,'AF' FTYPE,cf.custodycd, af.acctno afacctno, '' dfgroupid, cf.fullname,
        ...
        round(greatest(round((case when nvl(sec.rlsmarginrate,0) * af.mrirate =0 then - outstandingt2 else
                         greatest( 0,- outstandingt2 - sec.navaccountt2*100/af.mrirate) end),0),greatest(ovamt+depofeeamt - balance - nvl(avladvance,0),0)),0) rtnamtCL,
        0 rtnamtDF,
        nvl(lnt0.ovd,0) ovd,
    ...
    and ((sec.rlsmarginrate<af.mrlrate and af.mrlrate <> 0)
     ...
     /*    or (EXISTS (select 1 from mr3008_log where afacctno= af.acctno and txdate = to_date(l_PREVDATE,'DD/MM/RRRR') and log_action ='AF-END' and FTYPE ='AF')
                and sec.rlsmarginrate > af.mrlrate and sec.rlsmarginrate<af.mrmrate and l_ISCALLED='Y' and af.mrlrate <>0 and af.mrmrate <> 0
             )*/
        ...
        AND sec.rlsmarginrate < AF.mrmrate
          ...
          --chaunh: neu dau ngay xu ly thi van giu o man hinh xu ly
    ...
    if l_ISCALLED='Y' then
        ...
        and case when p_ISASSREMAIN = 'ALL' then 1
                 when NAVACCOUNT > 0 and p_ISASSREMAIN = 'Y' then 1
                 when NAVACCOUNT <= 0 and p_ISASSREMAIN = 'N' then 1
            else 0 end <> 0
        order by custodycd, dfgroupid;
    else
        OPEN PV_REFCURSOR
        ...
        and ((marginrate<=mrlrate and mrlrate <> 0)
                or ovd>1 or MARGINOVD>1)
        order by custodycd, dfgroupid;
    end if;
end if;
