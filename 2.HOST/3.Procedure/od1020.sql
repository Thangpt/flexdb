...
from (
    select
    sum(case when via ='O' then 1 else 0 end) So_Lenh_Online,
    sum(case when via ='O' then orderqtty * quoteprice else 0 end) Gia_Tri_Lenh_Online,
    sum(case when via ='O' then execamt else 0 end) Gia_Tri_khop_Lenh_Online,
    count(1) So_Lenh,
    ...
    sum(case when exectype in ('CB','CS','AB','AS') and via='O' then 1 else 0 end) So_Lenh_HUYSUA_Online,
    sum(case when exectype in ('CB','CS','AB','AS') and via='O' then orderqtty * quoteprice else 0 end) Gia_Tri_Lenh_HUYSUA_Online,
    sum(case when exectype in ('NB','NS','MS') and via='O' then 1 else 0 end) So_Lenh_DAT_Online,
    sum(case when exectype in ('NB','NS','MS') AND via = 'O' then orderqtty * quoteprice ELSE 0 end) Gia_Tri_Lenh_DAT_Online
