...
,ISS.fullname SYMBOLNAME , substr(NUM.ACCTNO,1,10) ACCTNO,NUM.AMT, af.email, af.fax1, cf.custodycd, af.acctno
FROM AFMAST AF,CFMAST CF,  SBSECURITIES  SB,ALLCODE AL, ISSUERS ISS,
(
SELECT SE.AFACCTNO || NVL(SB.REFCODEID,SB.CODEID ) ACCTNO  , SUM(SE.TRADE + SE.BLOCKED + se.secured + SE.WITHDRAW + SE.MORTAGE +NVL(SE.netting,0) + NVL(SE.dtoclose,0)   - NVL(NUM.AMT,0) + SE.WTRADE) AMT
