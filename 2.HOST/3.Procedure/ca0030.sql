...
BEGIN
   ...
   THEN
      BEGIN
            ...
            ELSE
               V_COREBANK_NAME :=  utf8nums.c_const_ca0030_name_cb_BVS;
            END IF;
      END;
   ELSE
...
OPEN PV_REFCURSOR
FOR
     SELECT ISS.FULLNAME ISS_NAME, SB.SYMBOL, SB.PARVALUE, MST.REPORTDATE, AFT.VAT, V_I_BRIDGD,
        V_ISCOREBANK, V_CASHPLACE,
            ...
            NVL(CA.AMT, 0) AMT, NVL(MST.PITRATE, 0) PITRATE,
            (NVL(CA.AMT, 0) -  round(NVL(CA.AMT, 0) * NVL(MST.PITRATE, 0) / 100) ) AMT_AFT,
            a.cdcontent noi_thu_thue,
            V_BRNAME BR_NAME, V_COREBANK_NAME COREBANK_NAME, V_CASHPLACE_NAME CASHPLACE_NAME, AF.ISFCT
    FROM VW_CASCHD_ALL CA, AFMAST AF, CFMAST CF, ALLCODE AL, SBSECURITIES SB, ISSUERS ISS, AFTYPE AFT,
         (SELECT * FROM CAMAST UNION ALL SELECT * FROM CAMASTHIST) MST, allcode a
    WHERE CA.AFACCTNO    = AF.ACCTNO
    AND   AF.CUSTID      = CF.CUSTID
    AND   CF.COUNTRY     = AL.CDVAL
    AND   AL.CDNAME      = 'COUNTRY'
    and   a.cdname =    'PITRATEMETHOD'
    and a.cdval = mst.PITRATEMETHOD
...
FOR
     ...
     SELECT ISS.FULLNAME ISS_NAME, SB.SYMBOL, SB.PARVALUE, MST.REPORTDATE, AFT.VAT, V_I_BRIDGD, V_ISCOREBANK, V_CASHPLACE,
            (case when DEVIDENTVALUE = 0 then  MST.DEVIDENTRATE || '%' else TO_CHAR(mst.DEVIDENTVALUE) end ) PCENT, MST.ACTIONDATE,
            (CASE WHEN SUBSTR(CF.CUSTODYCD, 4, 1) = 'C' THEN utf8nums.c_const_ca0030_custodytype_1
                  WHEN SUBSTR(CF.CUSTODYCD, 4, 1) = 'F' THEN utf8nums.c_const_ca0030_custodytype_2
                  WHEN SUBSTR(CF.CUSTODYCD, 4, 1) = 'P' THEN utf8nums.c_const_ca0030_custodytype_3 END) GR_I,
            ...
            CF.CUSTODYCD, AF.ACCTNO AFACCTNO, (CA.BALANCE + CA.PBALANCE) BALANCE, (CA.QTTY + CA.PQTTY) QTTY,
            NVL(CA.AMT, 0) AMT, NVL(MST.PITRATE, 0) PITRATE,
            (NVL(CA.AMT, 0) - (NVL(CA.AMT, 0) * NVL(MST.PITRATE, 0) / 100)) AMT_AFT,
            a.cdcontent noi_thu_thue,
            V_BRNAME BR_NAME, V_COREBANK_NAME COREBANK_NAME, V_CASHPLACE_NAME CASHPLACE_NAME, AF.ISFCT
    FROM VW_CASCHD_ALL CA, AFMAST AF, CFMAST CF, ALLCODE AL, SBSECURITIES SB, ISSUERS ISS, AFTYPE AFT,
         (SELECT * FROM CAMAST UNION ALL SELECT * FROM CAMASTHIST) MST, allcode a
