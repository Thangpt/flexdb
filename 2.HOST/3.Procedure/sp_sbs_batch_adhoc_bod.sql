CREATE OR REPLACE PROCEDURE sp_sbs_batch_adhoc_bod  IS
  ...
  CURSOR pv_refcursor_custodian IS
    SELECT ACCTNO FROM CFMAST CF, AFMAST AF, AFTYPE TYP
        WHERE substr(cf.custodycd ,1,3) <> (select varvalue from sysvar where grname ='SYSTEM' and varname ='COMPANYCD') AND CF.CUSTID=AF.CUSTID AND AF.ACTYPE=TYP.ACTYPE;
...
BEGIN
  ...
  UPDATE DFMAST d SET FLAGTRIGGER='C' WHERE FLAGTRIGGER<>'T' AND EXISTS
 ( SELECT 1 FROM LNMAST LN, DFMAST DF, SECURITIES_INFO SB
  WHERE DF.CODEID=SB.CODEID AND DF.LNACCTNO=LN.ACCTNO  AND (SB.BASICPRICE<=DF.mrate/100*DF.refprice OR
    (DF.CALLTYPE<>'P' AND (case when df.CALLTYPE='P' then 0 else
        ...
        end) <= DF.mrate)) AND DF.ACCTNO = d.acctno);
  ...
  UPDATE DFMAST d SET FLAGTRIGGER='T' WHERE FLAGTRIGGER<>'T' AND EXISTS
  (SELECT 1 FROM LNMAST LN, DFMAST DF, SECURITIES_INFO SB
  WHERE DF.CODEID=SB.CODEID AND DF.LNACCTNO=LN.ACCTNO  AND (sb.basicprice<=df.triggerprice OR
    (DF.CALLTYPE<>'P' AND (case when df.CALLTYPE='P' then 0 else
        ...
        end) < DF.Lrate))AND DF.ACCTNO = d.acctno);

--cap nhat trang thai trigger cho deal tong
UPDATE DFGROUP d SET FLAGTRIGGER='T' WHERE EXISTS
(
SELECT 1 FROM LNMAST LN, dfgroup DFG,
