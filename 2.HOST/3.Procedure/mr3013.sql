...
/*    select mst.*,
    ...
    select cf.custodycd, sec.afacctno, cf.fullname, af.mriratio mrirate, af.mrmratio mrmrate, af.mrlratio mrlrate, sec.marginratio marginrate,
        ...
        case when (sec.semaxcallrealass + GREATEST(balance + nvl(avladvance,0) - nvl(ln.marginamt,0),0)) = 0 then greatest(nvl(SEC.marginamt,0) - nvl(sec.avladvance,0) - balance,0)
            else round(greatest((af.mrmratio/100 - sec.marginratio/100) * (sec.semaxcallrealass + GREATEST(balance + nvl(avladvance,0) - nvl(ln.marginamt,0),0)),0),0) end addvnd,
        nvl(refullname,'') refullname
    ...
    where cf.custid = af.custid and af.acctno = sec.afacctno
        ...
        and sec.marginratio <= af.mrlratio
        ...
        and greatest(nvl(SEC.marginamt,0) - nvl(sec.avladvance,0) - balance,0) >0
