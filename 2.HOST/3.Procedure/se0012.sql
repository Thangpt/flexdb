...
OPEN PV_REFCURSOR
    FOR
SELECT iss.fullname, sb2.symbol, a.cdcontent tradeplace, sum(a.amt) amt ,
    ...
    FROM
        ...
        ) a, sbsecurities sb1, sbsecurities sb2, issuers iss, securities_info inf, allcode a
    ...
    and a.cdname = 'TRADEPLACE' and a.cdtype = 'OD'
    and a.cdval = sb2.tradeplace
    ...
    AND CASE WHEN V_STRSECTYPE = '%' THEN 1
         ...
         ELSE 0 END = 1
    GROUP BY iss.fullname, sb2.symbol,a.cdcontent, CASE WHEN inf.txdate < V_FROMDATE THEN 'Old' ELSE 'New' END
    HAVING sum(a.amt) <> 0
    ORDER BY sb2.symbol
