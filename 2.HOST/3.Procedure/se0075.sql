   ...
   SELECT  NVL(SE.BAL - DTL.AMT,0)  BE_BALANCE
          FROM
               (SELECT SUM(SE.TRADE + SE.BLOCKED + SE.WITHDRAW + SE.MORTAGE
                ...
                NOT IN ('004','003','006') --AND  SB.TRADEPLACE   IN ('001', '002', '005')
 ...
 FOR
   SELECT  NVL(SE.BAL - DTL.AMT,0)  BE_BALANCE
        ...
        where   nvl(sb.refcodeid,' ') = sb1.codeid(+)) SB
                ...
                AND CF.CUSTID = SE.CUSTID AND CF.CUSTATCOM ='Y'
                AND SB.SECTYPE IN ('006','003') --AND  SB.TRADEPLACE   IN ('001', '002', '005')
                AND substr( se.CUSTID, 1,4) LIKE V_STRBRID ) SE,
                (SELECT   NVL(SUM ((CASE WHEN se.TXTYPE = 'D'THEN -se.NAMT
                                   WHEN se.TXTYPE = 'C' THEN se.NAMT
                                   ELSE 0   END )),0) AMT
...
FOR
    ...
    FN_GET_SEBAL ('Y',TO_DATE (F_DATE  ,'DD/MM/YYYY'),TO_DATE (T_DATE  ,'DD/MM/YYYY'),V_STRBRID) AMT_CP_BAL,
    FN_GET_SEBAL ('N',TO_DATE (F_DATE  ,'DD/MM/YYYY'),TO_DATE (T_DATE  ,'DD/MM/YYYY'),V_STRBRID) AMT_TP_BAL
...
FOR
/*
SUA LAI LAY CA NGAY NGHI
    SELECT F_D,T_D,BE_CP_BAL,BE_TP_BAL,BE_CP_BAL + AMT_CP_BAL  F_CP,
    BE_TP_BAL+ AMT_TP_BAL F_TP,
    ...
    FN_GET_SEBAL ('Y',TO_DATE (F_DATE  ,'DD/MM/YYYY'),SB2.SBDATE,V_STRBRID) AMT_CP_BAL,
    FN_GET_SEBAL ('N',TO_DATE (F_DATE  ,'DD/MM/YYYY'),SB2.SBDATE,V_STRBRID) AMT_TP_BAL
    FROM SBCLDR SB1, SBCLDR SB2   WHERE
    SB1.HOLIDAY='N' AND SB1.CLDRTYPE='000'
    AND SB2.HOLIDAY='N' AND SB2.CLDRTYPE='000'
    AND SB2.SBDATE = GETDUEDATE(SB1.SBDATE,'B','001',1)
    AND SB1.SBDATE >= TO_DATE (F_DATE  ,'DD/MM/YYYY') AND SB1.SBDATE <= TO_DATE (T_DATE  ,'DD/MM/YYYY')
    AND SB2.SBDATE >= TO_DATE (F_DATE  ,'DD/MM/YYYY') AND SB2.SBDATE <= TO_DATE (T_DATE  ,'DD/MM/YYYY')
--ORDER BY SB1.SBDATE
    );
