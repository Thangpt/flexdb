...
/*    select trfbuyamt into l_trfbuyamt -- Tong so tien tra cham.
    from cimast where acctno = l_afacctno;

    SELECT least(greatest(nvl(sum(AMT * (1-TRFBUYRATE/100) - trfexeamt),0),0),l_TOODAMT) into l_trfamt_aft_rls
    FROM STSCHD
    WHERE DUETYPE='SM' AND STATUS='C' AND TRFBUYSTS='N' and afacctno = l_afacctno AND TXDATE=to_date(l_txdate,'DD/MM/RRRR');

    select greatest(advanceline - l_TOODAMT - (l_trfbuyamt - l_trfamt_aft_rls),0)
        into l_release_advanceline -- So bao lanh release tu advanceline (Bao lanh da cap - So tien sap giai ngan - So tien phai giu lai)
