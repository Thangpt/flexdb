...
left join
    ...
    select tr.acctno,
        ...
        sum
        (case when field = 'BLOCKED' and tr.tltxcd not in ('2203','2202') and  nvl(tr.ref,'000') not in ('007','002')
                then
                (case when tr.txtype = 'D' then -tr.namt else tr.namt end)
            else 0
            end
...
left join   -- So du chung khoan chuyen nhuong co dieu kien
    ...
    select se.acctno,
       ...
       sum(case when a.qttytype = '011' then a.qtty else 0 end) curr_block_2232
    from semastdtl a, semast se, sbsecurities sb
    where a.acctno = se.acctno
        and se.codeid = sb.codeid
        and a.qttytype in ('002','007','011')
        and sb.sectype <>'004'
        and a.status = 'N'
        and a.deltd <> 'Y'
    group by se.acctno
...
left join   -- Phat sinh giao dich phong toa/giai toa CK chuyen nhuong co dieu kien
    ...
    where tr.field = 'BLOCKED'
        AND tr.symbol LIKE '%'
        and tr.tltxcd in ('2202','2203') and tr.ref in ('002','007') and tr.namt <> 0
