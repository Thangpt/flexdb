...
select sts.cleardate,sts.txdate ,  SUM (  DECODE (  AFTYPE.vat ,'Y', od.taxsellamt ,0)     )     VAT , SUM(DECODE ( STS.duetype,'RM',FEEACR,0 )) FEEACR_S, SUM(DECODE ( STS.duetype,'SM',FEEACR,0 )) FEEACR_B
from (select orgorderid, duetype,txdate, cleardate, sum( round( DECODE (duetype,'RM',AMT*0.0005,0 ))) amtvat
      from  vw_stschd_all
      where duetype in ('RM','SM') group by orgorderid, duetype,cleardate,txdate) sts ,vw_odmast_all od,afmast af, cfmast cf,AFTYPE
