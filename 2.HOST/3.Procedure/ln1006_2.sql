CREATE OR REPLACE PROCEDURE ln1006_2 (
...
if to_date(p_I_DATE,'DD/MM/RRRR') >= l_INITRLSDATE then
    OPEN PV_REFCURSOR
     FOR
        ...
        SELECT fullname, custodycd, contractno, lnprin amtu, lnamt amt,  nvl(lmamtmax,0) limitamt,
            rlsdate txdate, overduedate cleardt, txdate bankdate, to_date(p_I_DATE,'DD/MM/RRRR') oddate, to_date(p_I_DATE,'DD/MM/RRRR') I_DATE
        FROM
        (
            select p_BRID hs_chinhanh,
                   ...
                   , (select to_char(max(sbdate),'DD/MM/RRRR') from sbcldr where cldrtype = '000' and holiday = 'N' and sbdate <= l.rlsdate + (CASE WHEN l.rlsdate >= '04-Aug-2017' AND b.shortname LIKE 'VB%' THEN 90 ELSE 180 END) ) overduedate --fix TPBANK mac dinh la ky han 6 thang
                   , l.rlsdate, b.txdate,
                    p_BRID BRID , l_BANKNAME  Nguon_GN
                    , ln.rate2, cfl.lmamtmax

            from rlsrptlog_eod l, cfmast cf, vw_lnmast_all ln, vw_lnschd_all ls, cfmast cfb, (SELECT * FROM cflimit WHERE lmsubtype = 'DFMR') cfl,
                (
    ...
    OPEN PV_REFCURSOR
     FOR
        SELECT fullname, custodycd, contractno, lnprin amtu, lnamt amt, nvl(lmamtmax,0) limitamt,
            rlsdate txdate, overduedate cleardt, bankdate, to_date(p_I_DATE,'DD/MM/RRRR') oddate, to_date(p_I_DATE,'DD/MM/RRRR') I_DATE
        FROM
        (
             select p_BRID hs_chinhanh,
                ...
                (select cf.custodycd,cf.custid, cf.idcode, to_char(cf.iddate,'DD/MM/RRRR') iddate, cf.idplace, af.acctno afacctno, cf.fullname,  b.contractno, b.bankacctno,
                    ...
                    case when ln.ftype || ls.reftype = 'AFGP' then 'B?o lãnh'--utf8nums.c_const_RPT_LN1000_AFGP_DESC
                            ...
                            when  ln.ftype || ls.reftype = 'AFP' and ln.rrtype = 'B' then 'Ngu?n ng?h?' --utf8nums.c_const_RPT_LN1000_AFPB_DESC
                            else ' '
                            end reftype_desc,
                    ...
                    case when ln.rrtype = 'C' then 'KBSV'
                        when ln.rrtype = 'B' then cf.fullname
                    ...
                    ,ln.prinperiod
                    , ln.rate2, b.txdate bankdate, cfl.lmamtmax
                from cfmast cf, afmast af, vw_lnmast_all ln, vw_lnschd_all ls, cfmast cfb, (SELECT * FROM cflimit WHERE lmsubtype = 'DFMR') cfl,
