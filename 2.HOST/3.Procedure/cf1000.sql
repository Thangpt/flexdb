...
FROM (
        SELECT od.afacctno,
               SUM (CASE WHEN chd.trfbuydt > v_CurrDate
                              AND chd.trfbuydt > v_ToDate
                              THEN od.execamt +  od.feeacr --(chd.amt +  od.feeacr - chd.trfexeamt - od.feeamt)--
                         WHEN trfbuydt < v_CurrDate AND  trfbuydt > v_ToDate THEN od.execamt +  od.feeacr--(chd.amt +  od.feeacr) --
                         ELSE 0 END ) amt
        FROM
        ...
        and od.txdate <> v_CurrDate --dk tra cham
        GROUP BY od.afacctno
        HAVING SUM (CASE WHEN chd.trfbuydt > v_CurrDate AND chd.trfbuydt > v_ToDate THEN od.execamt +  od.feeacr--(chd.amt +  od.feeacr - chd.trfexeamt - od.feeamt)--od.execamt +  od.feeacr
               WHEN trfbuydt < v_CurrDate AND  trfbuydt > v_ToDate THEN od.execamt +  od.feeacr--(chd.amt +  od.feeacr ) -- od.execamt +  od.feeacr
               ELSE 0 END ) <> 0
       );*/
SELECT nvl(sum(amt),0) INTO v_tmtracham
FROM (
...
(
    ...
    --CHAUNH hien giao dich va dien giai 2200
    --having sum(case when tse.txtype = 'D' then -tse.namt else tse.namt end) <> 0
    having sum(case when tse.txtype = 'C' and tse.field in ('TRADE','MORTAGE','BLOCKED') then tse.namt else 0 end) <> 0
       or sum(case when tse.txtype = 'D' AND tse.field in ('TRADE','MORTAGE','BLOCKED') then tse.namt
                WHEN tltxcd = '2248' AND tse.field = 'DTOCLOSE' THEN tse.namt
                when tltxcd = '2266' AND tse.field = 'WITHDRAW' THEN tse.namt
               else 0 end) <> 0
