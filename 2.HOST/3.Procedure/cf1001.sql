...
SELECT v_CurrDate CurrDate, v_TxDate txdate, (CASE WHEN v_AFAcctno = '%' THEN 'ALL' ELSE afacctno END) custid,
        fullname, idcode, iddate, idplace, address, mobile, custodycd,  afacctno, symbol,
...
(select cf.custid, cf.fullname, cf.idcode, cf.iddate, cf.idplace, cf.address, cf.mobile, cf.custodycd,
    ...
    nvl(se_block_move.cr_block_amt,0) + nvl(se_block_move.dr_block_amt,0) END end_blocked_bal, -- han che CN
    */
...
(
    ...
    where tr.busdate > v_TxDate and tr.busdate <= v_CurrDate
        ...
        and tr.sectype <>'004'
        and tr.field in ('TRADE','MORTAGE','BLOCKED','NETTING','STANDING','RECEIVING','WITHDRAW','DTOCLOSE')
    group by tr.acctno
...
GROUP BY v_TxDate,custid, fullname, idcode, iddate, idplace, address, mobile, custodycd, afacctno, symbol;
