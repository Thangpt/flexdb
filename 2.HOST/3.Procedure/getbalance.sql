    ...
    SELECT MR.MRTYPE,af.actype,mst.groupleader into v_margintype,v_actype,v_groupleader from afmast mst,aftype af, mrtype mr where mst.actype=af.actype and af.mrtype=mr.actype and mst.acctno=pv_afacctno;
        if v_margintype='N' then
            ...
            OPEN PV_REFCURSOR FOR
                   ...
                   (CASE WHEN ci.balance - ci.odamt - NVL (advamt, 0) - NVL (v.secureamt, 0) < 0 THEN 0 ELSE ci.balance - ci.odamt - NVL (advamt, 0) - NVL (v.secureamt, 0) END) cashforwithdrawl,
                   (  ci.balance+least(nvl(af.mrcrlimit,0), NVL (v.secureamt, 0)) - ci.odamt - NVL (v.secureamt, 0) + af.advanceline - NVL (v.overamt, 0) ) moneyavaiableforinvestment
            ...
            SELECT cf.custodycd, cf.fullname, af.aftype, af.acctno afacctno, af.isotc,
                   ...
                   + af.advanceline + ci.balance- ci.odamt - nvl(v.secureamt,0) - ci.ramt,0) moneyavaiableforinvestment,
                   TRUNC((CASE WHEN af.mrirate>0
                            THEN GREATEST(LEAST(100* (/*nvl(af.MRCRLIMIT,0) +*/  nvl(se.SEASS,0))/af.mrirate + (ci.balance+least(nvl(af.MRCRLIMIT,0),nvl(v.secureamt,0))+ nvl(se.receivingamt,0)- ci.odamt - NVL (v.advamt, 0)-nvl(v.secureamt,0) - ci.ramt),
                                greatest(ci.balance- ci.ovamt-ci.dueamt - ci.ramt-af.advanceline,0)
                                ,(af.advanceline + nvl(af.mrcrlimitmax,0) + ci.balance- ci.odamt - nvl (v.overamt, 0)-nvl(v.secureamt,0) - ci.ramt)-nvl(af.advanceline,0)),0)
