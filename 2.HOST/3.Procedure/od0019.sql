...
SELECT * FROM (
    ...
    FROM (select od.*,VOD.MATCHTYPE,vod.custid from odmastdtl OD, vw_odmast_all VOD WHERE OD.orderid=VOD.orderid)OD,
          confirmodrsts c, ALLCODE A0, ALLCODE A1, ALLCODE A2, ALLCODE A3,aLLCODE A4,ALLCODE A5, tlprofiles tl, cfmast cf, afmast af, sbsecurities sb --1.7.2.1
    WHERE od.orderid = c.orderid(+) AND od.afacctno = af.acctno and od.codeid= sb.codeid
...
UNION all
      ...
      FROM
          ...
          and se.sectype like V_SECTYPE--1.7.2.1
          and se.bondtype like V_BONDTYPE
...
/*
      ...
      FROM
          (SELECT OD.ORDERID,OD.CODEID, A0.CDCONTENT TRADEPLACE, A1.CDCONTENT EXECTYPE,
            OD.PRICETYPE, A3.CDCONTENT VIA, OD.ORDERQTTY,OD.QUOTEPRICE, OD.REFORDERID,
            se.symbol,
            CASE WHEN od.via in ('O','M','K') OR (od.via = 'E' AND od.tlid = systemnums.C_ONLINE_USERID) THEN 'Y' ELSE nvl(CFMSTS.CONFIRMED,'N') END CONFIRMED,
            od.afacctno, cf.custodycd, cf.fullname, ' ' ROOTORDERID,
            od.txdate, od.txtime, re.refullname, re1.DFS,
            tl.tlname tlid, CFMSTS.Userid,CFMSTS.custid cfafacctno,
            (CASE WHEN od.via in ('O','M','K') OR (od.via = 'E' AND od.tlid = systemnums.C_ONLINE_USERID) THEN 'Y' ELSE nvl(CFMSTS.CONFIRMED,'N') END) CONFIRMEDVAL,
            a4.cdcontent via_confirm_name, cfmsts.via via_confirmed
          FROM CONFIRMODRSTS CFMSTS,-- vao truc tiep day cho nhanh
          (SELECT * FROM vw_odmast_all od
            WHERE od.txdate >=to_date(F_DATE,'DD/MM/YYYY')
                and od.txdate <=to_date(T_DATE,'DD/MM/YYYY')
                and od.txdate >=to_date('01/01/2012','DD/MM/YYYY')) OD, SBSECURITIES SE,
          ...
          AND A3.cdtype = 'OD' AND A3.cdname = 'VIA' AND A3.cdval = OD.VIA
          AND a4.cdtype(+) = 'OD' and a4.cdname(+) = 'VIA' AND cfmsts.via = a4.cdval(+)
          and ( (od.exectype in ('NB','NS','MS') AND od.via in ('F','T','O','H','M','K','E')) or (od.exectype  not in ('NB','NS','MS')))
          and od.exectype not in ('AB','AS')
          and od.afacctno=af.acctno and af.custid=cf.custid
          AND od.afacctno = re.afacctno (+) AND od.txdate >= re.frdate(+) AND od.txdate <= re.todate(+)
          AND od.afacctno = re1.afacctno (+) AND od.txdate >= re1.frdate(+) AND od.txdate <= re1.todate(+)
          ...
          AND NVL(CFMSTS.CUSTID,'a') LIKE v_strCFAfacctno
          AND (CASE WHEN (nvl(od.reforderid,'a') = 'a' OR od.exectype IN ('CB','CS'))  THEN od.tlid
                    ELSE (SELECT max(tlid) tlid FROM (SELECT * FROM vw_odmast_all od
                                                        WHERE od.txdate >=to_date(F_DATE,'DD/MM/YYYY')
                                                            and od.txdate <=to_date(T_DATE,'DD/MM/YYYY')) od2
                           WHERE od2.orderid <> od.orderid AND od.reforderid= od2.reforderid ) END) = tl.tlid
          AND od.via LIKE V_STRVIA
          AND AF.CAREBY like v_strGRCAREBY
          and AF.CAREBY IN (SELECT TLGRP.GRPID FROM TLGRPUSERS TLGRP WHERE TLID = V_STRTLID)
