...
FROM (
    --Trading
    SELECT 0 ORD2, se.CODEID,se.AFACCTNO,'Giao d?ch' STATUS,
    ...
    union
    --Mortgage
    select 1 ORD2, df.CODEID,df.AFACCTNO,'C?m c?' STATUS,df.DFTRADING QTTY, df.acctno REFID,'D' TYPE
    from v_getdealinfo df
    where df.CUSTODYCD=p_custodycd AND df.CODEID=p_codeid
    and df.dftrading>0
    union
    --Trading dat lenh
    SELECT 2 ORD2, od.CODEID,od.AFACCTNO,'Giao d?ch bán' STATUS,od.remainqtty -nvl(ext.qtty,0)  QTTY, od.orderid REFID, 'O' TYPE
    ...
    union
    --Mortgage dat lenh
    SELECT 3 ORD2, od.CODEID,od.AFACCTNO,'C?m c? bán' STATUS,od.remainqtty-nvl(ext.qtty,0)  QTTY, od.orderid REFID, 'M' TYPE
    from odmast od, afmast af, cfmast cf,
    (select refid, sum(qtty) qtty from odmapext where deltd<>'Y' and type ='O' group by refid) ext,
    (SELECT '001' TRADEPLACE, SYSVALUE FROM ordersys where SYSNAME='CONTROLCODE' ) S,
    (SELECT '002' TRADEPLACE, SYSVALUE FROM ordersys where SYSNAME='HNXCONTROLCODE' ) S1, SBSECURITIES SB
