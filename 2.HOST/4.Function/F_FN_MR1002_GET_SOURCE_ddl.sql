CREATE OR REPLACE FUNCTION fn_mr1002_get_source( pv_acctno IN VARCHAR2)
    RETURN VARCHAR IS
...
--rao lai de khong gui mail thong bao no den han
/*
if L_intdue >0 then
    ...
    v_Result := 'select ''' || l_custody_code || ''' custodycode, ''' ||pv_acctno || ''' account, '''
                ...
                || to_char(ROUND(l_intdue),'99G999G999G999G999MI')||' VND' || ''' intdue, ''' ||to_char(ROUND(l_nml),'99G999G999G999G999MI')||' VND'   || ''' nml, ''' ||l_sex || ''' sex, ''' ||
                 l_fullname || ''' fullname , ''T2219'' TEMPLATECODE from dual';

ELSE

*/
 SELECT custodycd,fullname,to_char(round(marginrate,2),'9,990.99')||' %' marginrate,to_char(ROUND(mrmrate))||' %' mrmrate, to_char (ROUND( CLAMT),'99G999G999G999G999MI')||' VND'CLAMT --thuyct edit 20191225
 ...
 FROM VW_MR1002 WHERE acctno = pv_acctno  ;
 ...
 if l_marginrate <= l_mrmrate then
    ...
    v_Result := 'select ''' || l_custody_code || ''' custodycode, ''' ||pv_acctno || ''' account, '''
                ...
                ||l_CLAMT || ''' CLAMT, ''' || l_ODAMT || ''' ODAMT, ''' || l_ADDVND || ''' ADDVND, ''' ||l_sex || ''' sex, ''' ||
                 l_fullname || ''' fullname, ''T2218'' TEMPLATECODE from dual';
end if;
