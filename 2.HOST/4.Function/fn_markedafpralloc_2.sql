CREATE OR REPLACE FUNCTION fn_markedafpralloc_2 (p_afacctno varchar2,
                                p_codeid varchar2,
...
begin
    ...
    pr_error ('fn_markedafpralloc','Begin function for afacctno: '||p_afacctno);
    ...
    begin
        ...
        (
            select
                   least(
                        greatest(    -least(
                                                decode(l_hoststatus,'1',balance+nvl(adv.avladvance,0),greatest(balance+nvl(adv.avladvance,0),0))
                                        - nvl(t0.marginamt,0)
                   ...
                   least(
                        greatest(    -least(
                                          decode(l_hoststatus,'1',
                                                                                        balance+nvl(adv.avladvance,0)
                                                                                                - mst.dfdebtamt
                                                                                                - mst.dfintdebtamt
                                                                                                - mst.depofeeamt
                                                                                                - nvl(t0.t0prin,0),
                                                                                        greatest(balance+nvl(adv.avladvance,0)
                                                                                                - mst.dfdebtamt
                                                                                                - mst.dfintdebtamt
                                                                                                - mst.depofeeamt
                                                                                                - nvl(t0.t0prin,0),0))
    ...
    pr_error('fn_markedafpralloc','afacctno: '|| p_afacctno ||' ,l_marked_outstanding:'||l_marked_outstanding);
    pr_error('fn_markedafpralloc','afacctno: '|| p_afacctno ||' ,l_unmarked_outstanding:'||l_unmarked_outstanding);
    pr_error('fn_markedafpralloc','afacctno: '|| p_afacctno ||' ,l_marked_sys_outstanding:'||l_marked_sys_outstanding);
    pr_error('fn_markedafpralloc','afacctno: '|| p_afacctno ||' ,l_unmarked_sys_outstanding:'||l_unmarked_sys_outstanding);
    ...
    */
    ...
    if l_marked_outstanding > 0 and l_isMarginAccount = 'Y' and p_mark_flag in ('M','A') and l_markroom74inday='Y' then --l_hoststatus = '0' then -- chi danh dau tai san dam bao khi co du no phat sinh them.
        ...
        loop
            ...
            pr_error('markedRecords.','codeid:'||marked_rec.codeid);
    ...
    if l_marked_sys_outstanding > 0 and p_mark_flag in ('M','A') then -- chi danh dau tai san dam bao khi co du no phat sinh them.
        ...
        loop
            ...
            pr_error('sy_markedRecords.','codeid:'||sy_marked_rec.codeid);
    ...
    if l_unmarked_outstanding > 0 and l_isMarginAccount = 'Y' and p_mark_flag in ('U','A') and l_markroom74inday = 'Y' then -- l_hoststatus = '0' then
        ...
        loop
            ...
            pr_error ('fn_markedafpralloc','        End Xac dinh so luong chung khoan co the phan bo vao nguon ');
            pr_error('unmarkedRecords.','codeid:'||unmarked_rec.codeid);
    ...
    if l_unmarked_sys_outstanding > 0 and p_mark_flag in ('U','A') then
        ...
        loop
            ...
            pr_error('sy_unmarkedRecords.','codeid:'||sy_unmarked_rec.codeid);
    ...
    pr_error ('fn_markedafpralloc','End function for afacctno '||p_afacctno);
