...
BEGIN
  -- chia co tuc bang tien
    if(pv_CATYPE='010') THEN
             ...
             v_result:= 'Chia c? t?c b?ng ti?n, '||pv_OPTSYMBOL||', ' || 'ngày ch?t ' || pv_REPORTDATE || ', t? l? ' ||pv_DEVIDENTRATE || '%';
             ELSE
             v_result:= 'Chia c? t?c b?ng ti?n, '||pv_OPTSYMBOL||', ' || 'ngày ch?t ' || pv_REPORTDATE || ', giá tr? ' ||pv_DEVIDENTVALUE;
             END IF;
    END if;
