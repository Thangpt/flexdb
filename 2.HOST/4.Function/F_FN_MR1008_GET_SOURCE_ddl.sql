CREATE OR REPLACE FUNCTION fn_mr1008_get_source( pv_acctno IN VARCHAR2)
    RETURN VARCHAR IS
...
v_Result:='';
For vc in (
 SELECT custodycd,fullname,to_char(round(MARGINRATE_LN,2),'9,990.99')||' %' marginrate,to_char(ROUND(mrmrate))||' %' mrmrate, -- thuyct edit 20191225
 ...
 ,to_char(ROUND(SECOUTSTANDING),'99G999G999G999G999MI')||' VND' SECOUTSTANDING
  ...
  FROM VW_MR1008 WHERE acctno = pv_acctno )
Loop
  ...
  l_mrmrate:=vc.mrmrate;
   ...
   l_ADDVND:=VC.ADDVND_LOAN;
    ...
    v_Result := 'select ''' || l_custody_code || ''' custodycode, ''' ||pv_acctno || ''' account, '''
                ...
                ||l_sex || ''' sex, ''' || l_fullname || ''' fullname, ''T4084'' TEMPLATECODE from dual';
end loop;
RETURN v_Result;
