...
begin
    ...
    if v_count=0 and v_hostatus='1' then
        ...
        v_message:='H? th?ng dang ho?t d?ng, ngày h? th?ng ' || v_currdate || '!';
        return v_message;
    end if;
    ...
    if v_count=0 and v_hostatus='0' then
        ...
        v_message:='H? th?ng dang ch?y batch t?i bu?c ' || to_char(v_count) || '/' || to_char(v_countAllBatch) || ' và dã chuy?n sang ngày m?i ' || v_currdate || '!';
        return v_message;
    end if;
    ...
    if v_count>0 then
        if v_count<v_countBFBatch and v_BFCount = 0 then
            ...
            v_message:='H? th?ng dang trong quá trình ch?y x? lý tru?c cu?i ngày t?i bu?c ' || to_char(v_count) || '/' || to_char(v_countBFBatch);
            return v_message;
        end if;
        if v_BFCount>0 and v_hostatus='1' then
           --He thong dang hoat dong sau buoc xu ly truoc cuoi ngay
            v_message:='H? th?ng dang ho?t d?ng và dã ch?y bu?c x? lý tru?c cu?i ngày, ngày h? th?ng ' || v_currdate  || '!';
            return v_message;
        end if;
        if v_BFCount>0 and v_hostatus='0' then
            --He thong danh chay batch cuoi ngay tai buoc
            v_message:='H? th?ng dang trong quá trình ch?y x? lý cu?i ngày t?i bu?c ' || to_char(v_count) || '/' || to_char(v_countAllBatch);
            return v_message;
        end if;
    end if;
end;
