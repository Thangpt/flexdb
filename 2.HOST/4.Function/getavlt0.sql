...
if v_margintype in ('N','L') then
    select least(T0,avllimit) AVLT0 into v_Result  from
            (SELECT cimast.afacctno,af.T0AMT, af.mrirate,nvl(af.advanceline,0) T0,
                       decode(l_isstopadv,'Y',0,NVL(adv.avladvance,0)) + af.advanceline + balance- odamt  - dfdebtamt - dfintdebtamt- nvl (advamt, 0)-nvl(secureamt,0) - ramt avllimit
        ...
        select T0-LEAST(greatest(least(T0, T0-PP),0),BUYAMT) AVLT0 into v_Result  from
            (SELECT cimast.afacctno,af.T0AMT, af.mrirate,nvl(af.advanceline,0) T0,
                      ...
                      /* nvl(af.MRCRLIMIT,0) +*/ nvl(se.SEASS,0)   NAVACCOUNT,
                       ...
                       decode(l_isstopadv,'Y',0,NVL(adv.avladvance,0)) + balance+least(nvl(af.MRCRLIMIT,0),nvl(secureamt,0)) - odamt - dfdebtamt - dfintdebtamt- NVL (advamt, 0)-nvl(secureamt,0) - ramt OUTSTANDING,
/*                       greatest(least((nvl(se.MRCRLIMIT,0) + nvl(se.SEAMT,0)+
                       ...
                       greatest(cimast.balance - nvl(secureamt,0) + decode(l_isstopadv,'Y',0,NVL(adv.avladvance,0)) + af.advanceline + least(nvl(af.mrcrlimitmax,0)+nvl(af.mrcrlimit,0),nvl(af.mrcrlimit,0) + nvl(se.seamt,0)) - nvl(cimast.odamt,0) - cimast.dfdebtamt - cimast.dfintdebtamt,0) pp
