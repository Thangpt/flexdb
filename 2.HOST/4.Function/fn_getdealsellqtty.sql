...
BEGIN
    ...
    -- lay thue
	begin
    SELECT max(decode(aft.vat , 'Y',iccf.icrate,0))
        INTO  l_vatrate
    FROM aftype aft, (SELECT * FROM iccftypedef WHERE modcode = 'CF' AND eventcode = 'CFSELLVAT') iccf , afmast af
    WHERE iccf.actype(+) = aft.actype AND af.actype = aft.actype
    AND af.acctno = l_afacctno;
	EXCEPTION
    WHEN others THEN
        l_vatrate:=0;
