...
SELECT  AMTPAID,INTPAID,FEEPAID,INTPENA,FEEPENA, CASE WHEN INTMIN+FEEMIN < CURINT+CURFEE THEN ROUND(CURAMT + INTPENA_CUR + FEEPENA_CUR + CURINT + CURFEE)
    ...
    into l_AMTPAID, l_INTPAID, l_FEEPAID, l_INTPENA, l_FEEPENA, l_SUMAMT, l_INTMIN, l_FEEMIN
FROM  (
...
SELECT AMTPAID,INTPAID,FEEPAID, ROUND (AMTPAID * RATE1 * GREATEST (least(MINTERM,PRINFRQ) - TO_NUMBER(TO_DATE((SELECT VARVALUE FROM SYSVAR WHERE VARNAME='CURRDATE'),'DD/MM/RRRR') - TO_DATE(RLSDATE,'DD/MM/RRRR')),0 ) /100/360
        + AMTPAID * RATE2 * GREATEST (GREATEST(MINTERM-PRINFRQ,0)-GREATEST(TO_DATE((SELECT VARVALUE FROM SYSVAR WHERE VARNAME='CURRDATE'),'DD/MM/RRRR') - TO_DATE(DUEDATE,'DD/MM/RRRR'),0 ),0) /100/360,4) INTPENA,
       ...
       ROUND(AMTPAID * CFRATE1 * GREATEST (least(MINTERM,PRINFRQ) - TO_NUMBER(TO_DATE((SELECT VARVALUE FROM SYSVAR WHERE VARNAME='CURRDATE'),'DD/MM/RRRR') - TO_DATE(RLSDATE,'DD/MM/RRRR')),0 ) /100/360
        + AMTPAID * CFRATE2 *  GREATEST (GREATEST(MINTERM-PRINFRQ,0)-GREATEST(TO_DATE((SELECT VARVALUE FROM SYSVAR WHERE VARNAME='CURRDATE'),'DD/MM/RRRR') - TO_DATE(DUEDATE,'DD/MM/RRRR'),0 ),0) /100/360,4) FEEPENA,
...
FROM (
    SELECT
        CASE WHEN INTPAIDMETHOD IN ('I','P') THEN
          CASE WHEN    GREATEST (INTMIN+FEEMIN, CURINT+CURFEE)=0 THEN pv_AMT ELSE ROUND((pv_AMT * RATEX) /( 1+ RATEX ),4) END  ELSE
            CASE WHEN  round(pv_AMT,0) =round(CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)) + INTPENA_CUR + FEEPENA_CUR,0) THEN CURAMT ELSE
             CASE WHEN pv_AMT < CURAMT THEN pv_AMT ELSE case when round(pv_AMT,0) = round(CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)),0) then CURAMT ELSE 0 END  END END  END AMTPAID,

          CASE WHEN INTPAIDMETHOD IN ('I','P') THEN
           CASE WHEN    GREATEST ( INTMIN, CURINT) = 0 THEN 0 ELSE ROUND(pv_AMT/( ( 1+ RATEX ) * (1+RATEY) ),4) END ELSE
            CASE WHEN  round(pv_AMT,0) =round(CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)) + INTPENA_CUR + FEEPENA_CUR,0) THEN ROUND(GREATEST ( INTMIN,CURINT ) + INTPENA_CUR,4) ELSE
                    case when round(pv_AMT,0) = round(CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)),0) then INTMIN ELSE 0 END  END  END INTPAID,

           CASE WHEN INTPAIDMETHOD IN ('I','P') THEN
             CASE WHEN GREATEST ( FEEMIN, CURFEE) = 0 THEN 0 ELSE ROUND((pv_AMT*RATEY)/( ( 1+ RATEX ) * (1+RATEY) ),4) END ELSE
                    CASE WHEN  round(pv_AMT,0) =round(CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)) + INTPENA_CUR + FEEPENA_CUR,0) THEN ROUND(GREATEST ( FEEMIN,CURFEE ) + FEEPENA_CUR,4) ELSE
                    case when round(pv_AMT,0) = round(CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)),0) then  FEEMIN ELSE 0 END
                    END  END FEEPAID,
    ...
    FROM (
        ...
        + CURAMT * RATE2 * GREATEST (GREATEST(MINTERM-PRINFRQ,0)-GREATEST(TO_DATE((SELECT VARVALUE FROM SYSVAR WHERE VARNAME='CURRDATE'),'DD/MM/RRRR') - TO_DATE(DUEDATE,'DD/MM/RRRR'),0 ),0) /100/360,4) INTPENA_CUR,
     ...
     ROUND(CURAMT * CFRATE1 * GREATEST (least(MINTERM,PRINFRQ) - TO_NUMBER(TO_DATE((SELECT VARVALUE FROM SYSVAR WHERE VARNAME='CURRDATE'),'DD/MM/RRRR') - TO_DATE(RLSDATE,'DD/MM/RRRR')),0 ) /100/360
        + CURAMT * CFRATE2 *  GREATEST (GREATEST(MINTERM-PRINFRQ,0)-GREATEST(TO_DATE((SELECT VARVALUE FROM SYSVAR WHERE VARNAME='CURRDATE'),'DD/MM/RRRR') - TO_DATE(DUEDATE,'DD/MM/RRRR'),0 ),0) /100/360,4) FEEPENA_CUR
        ...
        FROM (
            ...
            FROM (
                ...
                FROM (
                        SELECT LNS.NML + LNS.OVD CURAMT,
                                CASE WHEN LNT.INTOVDCD = 'N' THEN  LNS.INTNMLACR+LNS.intdue+LNS.intovd + LNS.intovdprin
                                    ELSE LNS.INTNMLACR+LNS.intdue+LNS.intovd + LNS.intovdprin
                                END CURINT,
                                CASE WHEN LNT.INTOVDCD = 'N' THEN LNS.FEEINTNMLACR + LNS.FEEINTOVDACR + LNS.FEEINTDUE + LNS.FEEINTNMLOVD
                                ELSE LNS.FEEINTNMLACR + LNS.FEEINTOVDACR + LNS.FEEINTDUE + LNS.FEEINTNMLOVD
                                END CURFEE, LN.INTPAIDMETHOD,
...
FROM (
    SELECT
        CASE WHEN INTPAIDMETHOD IN ('I','P') THEN
          CASE WHEN    GREATEST (INTMIN+FEEMIN, CURINT+CURFEE)=0 THEN pv_AMT ELSE ROUND(pv_AMT * (CURAMT/ GREATEST( CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)),1))) END  ELSE
            CASE WHEN  round(pv_AMT,0) =round(CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)) + INTPENA_CUR + FEEPENA_CUR,0) THEN CURAMT ELSE
             CASE WHEN pv_AMT < CURAMT THEN pv_AMT ELSE case when round(pv_AMT,0) = round(CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)),0) then CURAMT ELSE CURAMT END  END END  END AMTPAID,

          CASE WHEN INTPAIDMETHOD IN ('I','P') THEN
           CASE WHEN    GREATEST ( INTMIN, CURINT) = 0 THEN 0 ELSE ROUND(pv_AMT * GREATEST(INTMIN, CURINT) / GREATEST( CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)),1)) END ELSE
            CASE WHEN  round(pv_AMT,0) =round(CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)) + INTPENA_CUR + FEEPENA_CUR,0) THEN ROUND(GREATEST ( INTMIN,CURINT ) + INTPENA_CUR,0) ELSE
                    case when round(pv_AMT,0) = round(CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)),0) then INTMIN ELSE 0 END  END  END INTPAID,

           CASE WHEN INTPAIDMETHOD IN ('I','P') THEN
             CASE WHEN GREATEST ( FEEMIN, CURFEE) = 0 THEN 0 ELSE ROUND(pv_AMT - ROUND(pv_AMT * (CURAMT/ GREATEST( CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)),1))) - ROUND(pv_AMT * GREATEST(INTMIN, CURINT) / GREATEST( CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)),1)) ) END ELSE
                    CASE WHEN  round(pv_AMT,0) =round(CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)) + INTPENA_CUR + FEEPENA_CUR,0) THEN ROUND(GREATEST ( FEEMIN,CURFEE ) + FEEPENA_CUR,0) ELSE
                    case when round(pv_AMT,0) = round(CURAMT + GREATEST((INTMIN + FEEMIN), (CURINT + CURFEE)),0) then  FEEMIN ELSE 0 END
                    END  END FEEPAID,
        INTPENA_CUR, FEEPENA_CUR,
        RATEX, RATEY, INTMIN,FEEMIN, CURAMT, CURINT, CURFEE, INTPAIDMETHOD, RATE1, RATE2,CFRATE1, CFRATE2, MINTERM, PRINFRQ, RLSDATE, DUEDATE
