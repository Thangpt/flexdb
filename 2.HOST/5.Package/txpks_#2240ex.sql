...
BEGIN
   ...
   plog.debug(pkgctx,'BEGIN OF fn_txPreAppCheck');
    if (p_txmsg.txfields('06').value*p_txmsg.txfields('07').value <> 0) then
       p_err_code:='-900008';
    ...
    ***************************************************************************************************/
    ...
    SELECT sb.symbol INTO v_symbol FROM sbsecurities sb WHERE sb.codeid = p_txmsg.txfields('01').value;
    IF instr(UPPER(v_symbol),'_WFT') > 1 THEN
      IF p_txmsg.txfields('77').VALUE IS NULL THEN
         p_err_code:= -900091;
          RETURN -900091;
      END IF;
    END IF;
...
v_description := p_txmsg.txfields('77').VALUE;
    ...
    INSERT INTO SEDEPOSIT (AUTOID,ACCTNO,TXNUM,TXDATE,DEPOSITPRICE,DEPOSITQTTY,STATUS,DELTD,DESCRIPTION,DEPOTRADE,DEPOBLOCK,TYPEDEPOBLOCK,RDATE,WTRADE,DEPODATE,
                           CAQTTY011,TAXRATE011,CAQTTY021,Taxrate021)
    VALUES(SEQ_SEDEPOSIT.NEXTVAL,v_strACCTNO,v_txnum,V_txdate,v_nPRICE,v_nAMT,'D','N',v_description,v_nDEPOTRADE,v_nDEPOBLOCK,v_strTYPEDEPOBLOCK,V_RDATE,V_WTRADE,p_txmsg.busdate,
                           p_txmsg.txfields('16').VALUE,p_txmsg.txfields('17').VALUE,p_txmsg.txfields('18').VALUE,p_txmsg.txfields('19').VALUE);
