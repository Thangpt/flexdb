...
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Cash withdraw on PP0
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('03').value,'CIMAST','03','1184')<>'TRUE' then
...
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
     ...
     IF NOT (to_number(l_PP) >= to_number(ROUND(p_txmsg.txfields('10').value,0))) THEN
        p_err_code := '-400116';
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
