-- Start of DDL Script for Package Body HOSTMSTRADE.TXPKS_AUTO
-- Generated 11/04/2017 11:23:02 AM from HOSTMSTRADE@FLEX_111
...
IS
  ...
  PROCEDURE pr_fo2odsyn (p_orderid varchar2, p_err_code  OUT varchar2, p_timetype varchar2 default 'T' );
   PROCEDURE pr_fo2odsyn_break (p_orderid varchar2, p_err_code  OUT varchar2, p_timetype varchar2 default 'T' );
...
----GianhVG
  ...
  PROCEDURE pr_CreateTermDeposit(p_afacctno varchar2,p_actype varchar2,p_amount number,p_err_code  OUT varchar2,p_txdate  OUT varchar2,p_txnum  OUT varchar2);
  ...
  -- CLOSE c_SqlMaster;
   ...
   IS
      ...
      l_strEXECTYPE         odmast.exectype%TYPE;
       ...
       l_tradingsessionid  varchar2(50);
      ...
      LOOP
         ...
         LOOP
            plog.debug (pkgctx, 'inside for indx: ' || indx);
                   --2015.12.08 Begin Phan nhanh cho chia lenh de di rieng theo nhanh khac
            --Chi thuc hien cho lenh khong co option chia va lenh mua ban thong thuong
            if l_build_msg(indx).SPLOPT='N' and l_build_msg(indx).fld22 in ('NB','NS') then
                 IF l_build_msg(indx).tradeplace = '001' then
                    --Neu san HSX thi xe toi da theo l_hosebreakingsize
                    if l_build_msg(indx).fld12 > l_hosebreakingsize then
                        pr_fo2odsyn_break(l_build_msg(indx).acctno,l_err_code,l_build_msg(indx).fld20);
                        --plog.setendsection (pkgctx, 'pr_fo2od');
                        CONTINUE;
                    end if;
                 ELSIF l_build_msg(indx).tradeplace in( '002','005') then
                      --Neu san HNX thi xe toi da theo l_hasebreakingsize
                    if l_build_msg(indx).fld12 > l_hasebreakingsize then
                        pr_fo2odsyn_break(l_build_msg(indx).acctno,l_err_code,l_build_msg(indx).fld20);
                        --plog.setendsection (pkgctx, 'pr_fo2od');
                        CONTINUE;
                    end if;
            ...
            --2015.12.08 End Phan nhanh cho lenh chia
            ...
            BEGIN
               ...
               IF l_build_msg(indx).tradeplace ='002' THEN
                  ...
                  For vc in(select securitytradingstatus
                              from hasecurity_req
                              where symbol =l_build_msg(indx).fld07 )
                      Loop
                   ...
                   IF l_build_msg(indx).fld27 IN ('MTL','MOK','MAK') and  l_build_msg(indx).fld22 in ('NB','NS') AND l_hnxTRADINGID IN ('CLOSE','CLOSE_BL') THEN
                        RAISE errnums.e_invalid_session;
                   END IF;
                   ...
                   -- QuyetKD check ko duoc dat lenh lo le voi ma chung khoan moi len san , va CK dac biet
                    IF l_build_msg(indx).fld12<100 AND  l_tradingsessionid in ('1','27') then
                      RAISE errnums.e_invalid_session;
                   end if;
               ...
               IF l_build_msg (indx).fld27 = 'ATO' and l_build_msg(indx).tradeplace ='001'
               ...
               IF l_build_msg (indx).fld27 not in ( 'LO','IO','SO>','SO<')
               THEN                                               -- Pricetype
                  ...
                  ELSE
                    IF l_build_msg (indx).fld27  not in ( 'MTL') THEN
                     l_build_msg (indx).fld11   :=
               ...
               plog.debug (pkgctx, 'HAILT: ' || l_build_msg (indx).fld11);
            ...
            --                             OR sectype = '000')
                                    ...
                                    ---001+002+008-->111
                                    ---006+003-->222
                                    AND (instr(case when l_build_msg (indx).sectype in ('001','002','011') then l_build_msg (indx).sectype || ',' || '111,333'
               ...
               IF l_txmsg.txfields ('22').VALUE = 'AB' or l_txmsg.txfields ('22').VALUE = 'AS'     then
                   ...
                   l_txmsg.txfields ('16').VALUE     := l_build_msg(indx).refquantity;--Ducnv sua lay khoi luong lenh goc
               ...
               LOOP
                  ...
                  --v_strOrderID = FO_PREFIXED and "00" and Mid(Replace(v_strTXDATE, "/", vbNullString), 1, 4) and Mid(Replace(v_strTXDATE, "/", vbNullString), 7, 2) and Strings.Right(gc_FORMAT_ODAUTOID and CStr(v_DataAccess.GetIDValue("ODMAST")), Len(gc_FORMAT_ODAUTOID))
                  /*SELECT    systemnums.c_fo_prefixed
                         || '00'
     ...
     /* plog.debug (pkgctx,
      ...
      (
         SELECT  a.codeid fld01,
                 ...
                 --a.clearday fld10,
                 --T2_HoangND add
                 CASE WHEN p_timetype = 'G' THEN to_number(l_CLEARDAY) ELSE a.clearday END fld10,
                 --End T2_HoangND add
                 a.clearcd fld26,
                 'O' fld72,                                       --puttype fld72,
                 a.pricetype fld27,
      ...
      LOOP
            BEGIN
                              --2015.12.08 Begin Phan nhanh cho chia lenh de di rieng theo nhanh khac
                --Chi thuc hien cho lenh khong co option chia va lenh mua ban thong thuong
                if l_build_msg.SPLOPT='N' and l_build_msg.fld22 in ('NB','NS') then
                     IF l_build_msg.tradeplace = '001' then
                        --Neu san HSX thi xe toi da theo l_hosebreakingsize
                        if l_build_msg.fld12 > l_hosebreakingsize then
                            pr_fo2odsyn_break(p_orderid,p_err_code,p_timetype);
                            plog.setendsection (pkgctx, 'pr_fo2odsyn');
                            return;
                        end if;
                     ELSIF l_build_msg.tradeplace in( '002','005') then
                          --Neu san HNX thi xe toi da theo l_hasebreakingsize
                        if l_build_msg.fld12 > l_hasebreakingsize then
                            pr_fo2odsyn_break(p_orderid,p_err_code,p_timetype);
                            plog.setendsection (pkgctx, 'pr_fo2odsyn');
                            return;
                        end if;
               ...
               IF l_build_msg.tradeplace ='002' THEN
                   ...
                   For vc in(select securitytradingstatus
                              from hasecurity_req
                              where symbol =l_build_msg.fld07 )
                      Loop
                            l_tradingsessionid:= vc.securitytradingstatus;
                      End loop;
                   ...
                   END IF;
                   -- PHIEN DONG CUA KHONG DC NHAP LENH THI TRUONG
                   IF l_build_msg.fld27 IN ('MTL','MOK','MAK')and l_build_msg.fld22 in ('NB','NS') AND l_hnxTRADINGID IN ('CLOSE','CLOSE_BL') THEN
                        RAISE errnums.e_invalid_session;
                   END IF;
                   -- CHAN HUY SUA 10 PHUT CUOI
                   ...
                   IF l_build_msg.fld12<100 AND  l_hnxTRADINGID <> 'CONT' then
                    ...
                    -- QuyetKD check ko duoc dat lenh lo le voi ma chung khoan moi len san , va CK dac biet
                    IF l_build_msg.fld12<100 AND  l_tradingsessionid in ('1','27') then
                      RAISE errnums.e_invalid_session;
                   end if;
               ...
               IF l_build_msg.fld27 = 'ATO' AND l_build_msg.tradeplace ='001'
               THEN                                        -- fld27: pricetype
                  IF l_mktstatus IN ('O', 'A')
                  THEN
                     RAISE errnums.e_invalid_session;
               ...
               IF l_build_msg.fld27 not in ( 'LO','IO','SO>','SO<')
               THEN                                               -- Pricetype
                  IF l_strEXECTYPE='NB'--l_build_msg (indx).fld22 = 'NB'
                  THEN                                             -- exectype
                     l_build_msg.fld11   :=
                  ...
                  ELSE
                    IF l_build_msg.fld27  not in ( 'MTL') THEN
                     l_build_msg.fld11   :=
            ...
            --                             OR sectype = '000')
               ...
               IF l_build_msg.fld22 = 'AB' or l_build_msg.fld22 = 'AS'     then
                   l_txmsg.txfields ('16').defname   := 'ORGORDERQTTY';
                   l_txmsg.txfields ('16').TYPE      := 'N';
                   l_txmsg.txfields ('16').VALUE     := l_build_msg.refquantity; --Ducnv sua lay kl lenh goc
                   l_txmsg.txfields ('17').defname   := 'ORGQUOTEPRICE';
                   l_txmsg.txfields ('17').TYPE      := 'N';
                   l_txmsg.txfields ('17').VALUE     := 0;
               end if;
              ...
              /* plog.debug (pkgctx,
               ...
               ELSIF l_build_msg.tradeplace = '002' then
                    --Neu san HNX thi xe toi da theo l_hasebreakingsize
                    if l_breakingsize > l_hasebreakingsize then
                        l_breakingsize:=l_hasebreakingsize;
                    else
...
*/
            ...
            --                             OR sectype = '000')
                                    ...
                                    --order BY A.deffeerate DESC, B.ACTYPE DESC
                                    --order BY A.deffeerate , B.ACTYPE DESC -- Lay ti le phi nho nhat
                                    ORDER BY CASE WHEN (SELECT VARVALUE FROM SYSVAR WHERE GRNAME = 'SYSTEM' AND VARNAME = 'ORDER_ORD') = 'ODRNUM' THEN ODRNUM ELSE DEFFEERATE END ASC -- HaiLT lay theo ordnum cua afidtype
                                    ) where rownum<=1;
                           EXCEPTION
                              WHEN NO_DATA_FOUND
                              THEN
...
IS
    ...
    --=2: Loai hinh khong ton tai
    --=3: So tien gui phai lon hon 0
    --=4: Lai suat khong hop le
    --=5: Ky han khong hop le
      l_txmsg               tx.msg_rectype;
      l_err_param varchar2(300);
  ...
  BEGIN
    ...
    l_txmsg.tltxcd:='1670';
    Begin
        select cf.fullname, cf.address, cf.idcode into v_fullname, v_address, v_idcode from cfmast cf, afmast af where cf.custid = af.custid and af.acctno =p_afacctno;
    exception when others then
        p_err_code:='-570012';
        return;
    end;
    ...
    loop
        ...
        l_txmsg.txfields ('03').TYPE      := 'C';
        l_txmsg.txfields ('03').VALUE     := p_afacctno;
        --06    C   CIACCTNO    81CIACCTNO
        l_txmsg.txfields ('06').defname   := 'CIACCTNO';
        l_txmsg.txfields ('06').TYPE      := 'C';
        l_txmsg.txfields ('06').VALUE     := p_afacctno;
        --10    N   AMT
        l_txmsg.txfields ('10').defname   := 'AMT';
        l_txmsg.txfields ('10').TYPE      := 'N';
        l_txmsg.txfields ('10').VALUE     := p_amount;
        --11    N   MSTRATE     81INTRATE
        l_txmsg.txfields ('11').defname   := 'MSTRATE';
        l_txmsg.txfields ('11').TYPE      := 'N';
        l_txmsg.txfields ('11').VALUE     := rec.INTRATE;
        --12    N   MSTTERM     81TDTERM
        l_txmsg.txfields ('12').defname   := 'MSTTERM';
        l_txmsg.txfields ('12').TYPE      := 'N';
        l_txmsg.txfields ('12').VALUE     := rec.TDTERM;
        --30    C   T_DESC
        l_txmsg.txfields ('30').defname   := 'T_DESC';
        l_txmsg.txfields ('30').TYPE      := 'C';
        l_txmsg.txfields ('30').VALUE     := 'Tao mon tiet kiem Online';
        --80    C   TERMCD      81TERMCD
        l_txmsg.txfields ('80').defname   := 'TERMCD';
        l_txmsg.txfields ('80').TYPE      := 'C';
        l_txmsg.txfields ('80').VALUE     := rec.TERMCD;
        --81    C   ACTYPE
        l_txmsg.txfields ('81').defname   := 'ACTYPE';
        l_txmsg.txfields ('81').TYPE      := 'C';
        l_txmsg.txfields ('81').VALUE     := p_actype;
        --82    N   TDTERM      81TDTERM
        l_txmsg.txfields ('82').defname   := 'TDTERM';
        l_txmsg.txfields ('82').TYPE      := 'N';
        l_txmsg.txfields ('82').VALUE     := rec.TDTERM;
        --83    C   SCHDTYPE    81SCHDTYPE
        l_txmsg.txfields ('83').defname   := 'SCHDTYPE';
        l_txmsg.txfields ('83').TYPE      := 'C';
        l_txmsg.txfields ('83').VALUE     := rec.SCHDTYPE;
        --84    N   INTRATE     81INTRATE
        l_txmsg.txfields ('84').defname   := 'INTRATE';
        l_txmsg.txfields ('84').TYPE      := 'N';
        l_txmsg.txfields ('84').VALUE     := rec.INTRATE;
        --85    N   SPREADTERM  81SPREADTERM
        l_txmsg.txfields ('85').defname   := 'SPREADTERM';
        l_txmsg.txfields ('85').TYPE      := 'N';
        l_txmsg.txfields ('85').VALUE     := rec.SPREADTERM;
        --86    N   SPREADRATE  81SPREADRATE
        l_txmsg.txfields ('86').defname   := 'SPREADRATE';
        l_txmsg.txfields ('86').TYPE      := 'N';
        l_txmsg.txfields ('86').VALUE     := rec.SPREADTERM;
        --87    C   TDSRC       81TDSRC
        l_txmsg.txfields ('87').defname   := 'TDSRC';
        l_txmsg.txfields ('87').TYPE      := 'C';
        l_txmsg.txfields ('87').VALUE     := rec.TDSRC;
        --88    N   ISCI        81ISCI
        l_txmsg.txfields ('88').defname   := 'ISCI';
        l_txmsg.txfields ('88').TYPE      := 'C';
        l_txmsg.txfields ('88').VALUE     := rec.ISCI;
        --90    C   CUSTNAME
        l_txmsg.txfields ('90').defname   := 'CUSTNAME';
        l_txmsg.txfields ('90').TYPE      := 'C';
        ...
        if not (to_number(l_txmsg.txfields ('10').VALUE)>0) then
            p_err_code:='-570013'; --So tien gui phai lon hon 0
            return;
        end if;
        if not (to_number(l_txmsg.txfields ('12').VALUE)>=to_number(l_txmsg.txfields ('82').VALUE)-to_number(l_txmsg.txfields ('85').VALUE)) then
            p_err_code:='-570014'; --Lai suat gui khong hop le
            return;
        end if;
        if not (to_number(l_txmsg.txfields ('11').VALUE)>=to_number(l_txmsg.txfields ('84').VALUE)-to_number(l_txmsg.txfields ('86').VALUE)) then
            p_err_code:='-570015';--Ky han gui khong hop le
            return;
        end if;
        if not (to_number(l_txmsg.txfields ('12').VALUE)<=to_number(l_txmsg.txfields ('82').VALUE)+to_number(l_txmsg.txfields ('85').VALUE)) then
            p_err_code:='-570014'; --Lai suat gui khong hop le
...
EXCEPTION
  WHEN OTHERS
   THEN
      plog.debug (pkgctx,'got error on pr_CreateTermDeposit');
      ROLLBACK;
      p_err_code := errnums.C_SYSTEM_ERROR;
      plog.error (pkgctx, SQLERRM);
      plog.error(pkgctx,'row:'||dbms_utility.format_error_backtrace);
                  ...
                  SELECT MST.ACCTNO,MSt.ACTYPE,MSt.AFACCTNO,MSt.STATUS,MSt.EXECTYPE,MSt.PRICETYPE,MSt.TIMETYPE,
                   ...
                   WHERE MST.DELTD<>'Y' AND MST.BOOK = 'A' AND  MST.REMAINQTTY>0
                     ...
                     AND CASE WHEN MST.EXECTYPE IN ('NB','BC') THEN
                                CHECKGTCBUYORDER(mst.afacctno,MST.REMAINQTTY,
            ...
            (select mst.acctno
                 ...
                 SELECT mst.acctno
                  ...
                  WHERE MST.DELTD<>'Y' AND MST.BOOK = 'A' AND  MST.REMAINQTTY>0 AND SE.ACCTNO=V.seacctno(+)
                    ...
                    AND MST.ACCTNO=rec.acctno);
             if l_count>0 and fnc_check_buy_sell(rec.AFACCTNO,to_date(v_strTxdate,'dd/mm/rrrr'),rec.CODEID,rec.EXECTYPE, rec.PRICETYPE, rec.MATCHTYPE, rec.tradeplace)
             THEN
...
PROCEDURE pr_fo2odsyn_break (p_orderid varchar2, p_err_code  OUT varchar2, p_timetype varchar2 default 'T' )
   ...
   BEGIN
      plog.setbeginsection (pkgctx, 'pr_fo2odsyn_break');
      plog.debug (pkgctx, 'BEGIN OF pr_fo2odsyn_break');
      plog.debug (pkgctx, 'p_orderid: '||p_orderid);
      plog.debug (pkgctx, 'p_timetype: '||p_timetype);
      /***************************************************************************************************
       ** PUT YOUR CODE HERE, FOLLOW THE BELOW TEMPLATE:
      ...
      LOOP
            ...
            --                             OR sectype = '000')
              ...
              /* plog.debug (pkgctx,
               ...
               -- ghep chia lenh
               --Kiem tra cho lenh tong
               --Thuc hien truoc khi chi lenh
               --0. Init check message
               l_txmsg_check:=l_txmsg;
               l_txmsg_check.txfields ('12').VALUE   :=l_remainqtty;

               l_txmsg_check.txfields ('40').VALUE     :=
                    greatest(l_feerate/100 * TO_NUMBER (l_txmsg_check.txfields('12').VALUE)         --quantity
                     * TO_NUMBER (l_build_msg.fld11)       --quoteprice
                     * TO_NUMBER (l_build_msg.fld98),l_feeamountmin);
              ...
              SELECT    systemnums.c_fo_prefixed
                     ...
                     || TO_CHAR(TO_DATE (VARVALUE, 'DD\MM\RR'),'DDMMRR')
                     || LPAD (seq_odmast.NEXTVAL, 6, '0')
              INTO l_txmsg_check.txfields ('04').VALUE
              FROM SYSVAR WHERE VARNAME ='CURRDATE' AND GRNAME='SYSTEM';



              SELECT REGEXP_REPLACE (l_txmsg_check.txfields ('04').VALUE,
                                         '(^[[:digit:]]{4})([[:digit:]]{2})([[:digit:]]{10}$)',
                                         '\1.\2.\3.'
                         )
                         || l_fullname
                         || '.'
                         || l_txmsg_check.txfields ('24').VALUE          --MATCHTYPE
                         || l_txmsg_check.txfields ('22').VALUE       ---ORGEXECTYPE
                         || '.'
                         || l_txmsg_check.txfields ('07').VALUE             --SYMBOL
                         || '.'
                         || l_txmsg_check.txfields ('12').VALUE
                         || '.'
                         || l_txmsg_check.txfields ('11').VALUE         --QUOTEPRICE
                  INTO l_txmsg_check.txfields ('30').VALUE
                  FROM DUAL;
               IF l_txmsg_check.txfields ('22').VALUE = 'NB' then
                    l_txmsg_check.tltxcd   := '8876';
               else
                    l_txmsg_check.tltxcd   := '8877';
               end if;

            /*   --1. Lock Account
               IF l_txmsg_check.txfields ('22').VALUE = 'NB' then
                    pr_lockaccountdirect(l_build_msg.fld03, 'CI', p_err_code);
               else
                    pr_lockaccountdirect(l_build_msg.fld03, 'SE', p_err_code);
               end if;*/
               --2. Kiem tra suc mua/ suc ban; Kiem tra Poll/Rooom cho lenh mua
               IF l_txmsg_check.txfields ('22').VALUE = 'NB' then
                   IF txpks_#8876EX.fn_txPreAppCheck(l_txmsg_check,p_err_code) <> systemnums.C_SUCCESS THEN
                       raise errnums.e_biz_rule_invalid;
                   END IF;

                   -- Run After check
                   IF txpks_#8876EX.fn_txAftAppCheck(l_txmsg_check,p_err_code) <> systemnums.C_SUCCESS THEN
                       raise errnums.e_biz_rule_invalid;
                   END IF;
                   --plog.debug (pkgctx, 'Begin of checking pool and room');
                   IF txpks_prchk.fn_txAutoCheck(l_txmsg_check, p_err_code) <> systemnums.C_SUCCESS THEN
                       plog.setendsection (pkgctx, 'fn_txAppCheck');
                       raise errnums.e_biz_rule_invalid;
                   END IF;
               else
                   IF txpks_#8877EX.fn_txPreAppCheck(l_txmsg_check,p_err_code) <> systemnums.C_SUCCESS THEN
                       raise errnums.e_biz_rule_invalid;
                   END IF;

                   -- Run After check
                   IF txpks_#8877EX.fn_txAftAppCheck(l_txmsg_check,p_err_code) <> systemnums.C_SUCCESS THEN
                       raise errnums.e_biz_rule_invalid;
                   END IF;
                   --plog.debug (pkgctx, 'Begin of checking pool and room');
                   IF txpks_prchk.fn_txAutoCheck(l_txmsg_check, p_err_code) <> systemnums.C_SUCCESS THEN
                 ...
                 (
                  ...
                  THEN
                     BEGIN
                        ...
                        -- 2: Process
                       -- Run Pre Update
                        IF txpks_#8876EX.fn_txPreAppUpdate(l_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
                            plog.error (pkgctx,
                                          '8876: '
                                       || p_err_code
                                       || ':'
                        ...
                        END IF;
                         -- Run After Update
                        IF txpks_#8876EX.fn_txAftAppUpdate(l_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
                            plog.error (pkgctx,
                                          '8876: '
                                       || p_err_code
                                       || ':'
                            ...
                            raise  errnums.e_biz_rule_invalid;
                        END IF;
                         txpks_#8876.pr_txlog(l_txmsg, p_err_code)  ;
                  ...
                  THEN
                    ...
                    -- 2: Process
                       -- Run Pre Update
                        IF txpks_#8877EX.fn_txPreAppUpdate(l_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
                            plog.error (pkgctx,
                                          '8877: '
                                       || p_err_code
                                       || ':'
                        ...
                        END IF;
                         -- Run After Update
                        IF txpks_#8877EX.fn_txAftAppUpdate(l_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
                            plog.error (pkgctx,
                                          '8877: '
                                       || p_err_code
                                       || ':'
                            ...
                            raise  errnums.e_biz_rule_invalid;
                        END IF;
                         txpks_#8877.pr_txlog(l_txmsg, p_err_code)  ;
                     END;                                              -- 8887
...
/*
                  ...
                  END IF;*/
               END LOOP;
                    --Thuc hien sau khi chi lenh
               --3.hach toan Pool/Room cho lenh mua
               IF l_txmsg_check.txfields ('22').VALUE = 'NB' then
                  --Hach toan Pool Room theo so luong lenh dat thanh cong
                  l_txmsg_check.txfields ('12').VALUE:= l_build_msg.fld12-l_remainqtty;
                  IF txpks_prchk.fn_txAutoUpdate(l_txmsg_check, p_err_code) <> systemnums.C_SUCCESS THEN
              ...
              /* --4. unlock Account
               IF l_txmsg_check.txfields ('22').VALUE = 'NB' then
                    pr_unlockaccountdirect(l_build_msg.fld03, 'CI');
               else
                    pr_unlockaccountdirect(l_build_msg.fld03, 'SE');
               end if;*/
               /*
               IF l_count >= l_commit_freq
               THEN
      ...
      ** END;
      ***************************************************************************************************/
      plog.debug (pkgctx, '<<END OF pr_fo2odsyn_break');
      plog.setendsection (pkgctx, 'pr_fo2odsyn_break');
   EXCEPTION
      WHEN OTHERS
      THEN
         ROLLBACK;
...
-- End of DDL Script for Package Body HOSTMSTRADE.TXPKS_AUTO
