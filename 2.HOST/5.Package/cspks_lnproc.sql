...
-- Procedure pr_LNApplyTypeToMaster(p_err_code  OUT varchar2);
  FUNCTION fn_Gen_Prepaid_Payment(p_afacctno varchar2,
                                  ...
                                  p_chksysctl varchar2 default 'ALL',
                                  p_balance number default 0,
                                  p_adv number default 0,
                                  p_bchmdl varchar2 default 'ALL')
  RETURN VARCHAR2;
  -- Ducnv day so no co the tra
  FUNCTION fn_get_Prepaid_Payment(p_afacctno varchar2,
                                  p_avlpaidamt out number,
                                  p_type varchar2,
                                  p_err_code  OUT varchar2,
                                  p_chksysctl varchar2 default 'ALL')
  RETURN varchar2;
    ...
    ***************************************************************************************************/
    ...
    begin
        ...
        SELECT MST.TRFACCTNO, MST.LNTYPE, MST.RLSAMT + MST.ORLSAMT RLSAMT,
               ...
               MST.CFRATE2, MST.CFRATE3,mst.minterm
          INTO v_strTRFACCTNO, v_strLNTYPE, v_dblLNRLSAMT, v_strLNCLDR,
               ...
               v_dblCFRATE1, v_dblCFRATE2, v_dblCFRATE3,v_minterm
    ...
    If (p_amount > 0) or (v_ISVSD = 'Y') Then --Giai ngan vay
        If v_strLNCLDR = 'N' Then --Lich tra no theo lich he thong: N: lich thuong (bao gom thu 7, CN); B: lich lam viec business
            IF v_strNINTCD='001' THEN -- 000: Cong Don; 001: Bac Thang
              ...
              v_strDUEDATE:= getduedate(TO_DATE(v_strCURRDATE,systemnums.c_date_format),'B','000',v_intPRINTFRQ3)  ;
            ...
            --  v_strDUEDATE:=to_char(TO_DATE(v_strCURRDATE,systemnums.c_date_format) + v_lngPRINFRQ,systemnums.c_date_format);
                v_strDUEDATE:=getduedate(TO_DATE(v_strCURRDATE,systemnums.c_date_format),'B','000',v_lngPRINFRQ)  ;
...
/*            i := v_lngPRINFRQ;
        ...
        INSERT INTO LNSCHD
          (AUTOID, ACCTNO, DUENO, RLSDATE, DUEDATE, OVERDUEDATE, ACRDATE,
           ...
           FEE, DUE,RATE1,RATE2,RATE3,CFRATE1,CFRATE2,CFRATE3,mintermdate)
        VALUES
          (SEQ_LNSCHD.NEXTVAL, p_lnacctno, v_intDueno,
           ...
           round(p_amount, 0), 0, 0, 'N', NULL, 0, 0, 'N',
            v_dblRATE1, v_dblRATE2, v_dblRATE3,
               ...
               TO_DATE(v_mintermdate, systemnums.c_date_format));
        UPDATE LNMAST
             SET
...
-- Phan bo phi d.v? Margin den han
    ...
    v_dblT0RETRIEVED := pv_dblT0PRINOVD + pv_dblT0PRINNML;
      plog.error (pkgctx, 'LINHLNB:v_dblT0RETRIEVED:'||v_dblT0RETRIEVED);
      plog.error (pkgctx, 'LINHLNB:pv_dblT0PRINOVD:'||pv_dblT0PRINOVD);
      plog.error (pkgctx, 'LINHLNB:pv_dblT0PRINNML:'||pv_dblT0PRINNML);
      plog.error (pkgctx, 'LINHLNB:pv_strACCTNO:'||pv_strACCTNO);
    ...
    IF v_dblT0RETRIEVED > 0 THEN
        ...
        END LOOP;
    END IF;
    plog.debug (pkgctx, 'HaiLT : Lan tra cuoi cung: ' || pv_strACCTNO );
    --Kiem tra xem co phai lan tra cuoi cung khongd

    for rec_last in (
         SELECT SUM(trunc(lns.nml)+trunc(lns.ovd)+trunc(lns.FEE)+trunc(lns.FEEOVD)+trunc(lns.FEEDUE)+trunc(lns.INTNMLACR)
                +trunc(lns.INTDUE)+trunc(lns.INTOVD)+trunc(lns.INTOVDPRIN)+trunc(lns.FEEINTNMLOVD)
                +trunc(lns.FEEINTOVDACR)+trunc(lns.FEEINTDUE)+trunc(lns.FEEINTNMLACR)) REMAINLNAMT
         --INTO v_REMAINLNAMT
         FROM lnschd lns
         WHERE ACCTNO = trim(pv_strACCTNO) AND REFTYPE = 'P'
     )
     loop
          plog.debug (pkgctx, 'HaiLT : Neu la lan tra cuoi cung thi update vao LNSCHDLOG: ' || pv_strACCTNO );
         -- Neu la lan tra cuoi cung thi update vao LNSCHDLOG
         IF rec_last.REMAINLNAMT <1 THEN
...
BEGIN
    ...
    v_dblINTDUE:= round(pv_dblINTDUE,0);
    v_dblINTNMLACR:= round(pv_dblINTNMLACR,0);
    v_dblintnmlacrbank:= round(pv_dblintnmlacrbank,0);
...
-- Phan bo phi d.v? Margin den han
 ...
 IF v_dblintnmlacrbank > 0 THEN
 FOR REC14 IN
            (SELECT AUTOID FROM LNSCHD WHERE ACCTNO = pv_strACCTNO and AUTOID = pv_dblAUTOID )
        LOOP
                UPDATE LNSCHD SET intnmlacrbank = intnmlacrbank -round (v_dblintnmlacrbank)  WHERE AUTOID = pv_dblAUTOID ;
                INSERT INTO LNSCHDLOG(AUTOID, TXNUM, TXDATE, NML, OVD, PAID, INTNMLACR, FEE,intnmlacrbank  )
                VALUES(REC14.AUTOID, pv_strTXNUM, TO_DATE(pv_strTXDATE,'dd/mm/rrrr'), 0,0, 0, 0, 0,-v_dblintnmlacrbank  );
        end loop;
    END IF;
...
l_chksysctl varchar2(1);
l_count number;
l_subcount number;
l_balance number;
l_adv     number;
l_remain_balance number;
l_exe_balance number;
l_count_pro       number;
l_avlamt_group    number;
l_store_balance   NUMBER;
BEGIN
...
l_balance:= p_balance;
l_adv:= p_adv;
l_remain_balance:= p_balance;
l_exe_balance:=0;
l_store_balance:=0;
--end 1.5.7.0|iss1922
if p_chksysctl = 'ALL' then
    l_chksysctl:='%';
else
    l_chksysctl:=p_chksysctl;
end if;
...
(
    ...
    and ln.ftype = 'AF' and lnt.actype = ln.actype and lnt.CHKSYSCTRL like l_chksysctl
    and instr(ls.reftype,'P') > 0
    and ln.status NOT IN ('R','C','P')
    and ln.trfacctno = p_afacctno
    and case when ln.rrtype = 'B' then to_number(l_busdate - getduedate(ls.rlsdate, ln.lncldr, '000', ln.minterm)) else 1 end >= 0
    -- Duwcnv check dieu kien phai qua ngay tra no som nhat
    and nvl(ls.mintermdate,l_default_date) <= l_busdate
...
LOOP --- REC
  --1.5.7.3 | Thay T10 thanh MAS10
  select count(*) into l_count_pro from RegisterProduc where produccode ='MAS10' and afacctno =p_afacctno;--1.5.7.0|iss1922
  --=============================================================================================
  --1.5.7.0 | Giu nguyen luong tra no QH, DH, no UB va cac TH tra no trong han ko dang ky sp T10
  --=============================================================================================
  if (p_type = 'N') or (p_type='R' and l_count_pro=0) or (p_type='L' and p_bchmdl<> 'LNADVPAYAFTSM' ) or (p_type='L' and p_bchmdl=  'LNADVPAYAFTSM' and  l_count_pro=0)then
        -- Theo OVERDUEDATE:
        l_exec_avlamt_group:=0;
        ...
        begin
           ...
           select  sum(round(case when p_type = 'N' then 0
                                       when p_type = 'R' and ((ln.prepaid = 'Y' and ln.advpay = 'Y') or (ln.lntype = 'S'))
                                                  then case when ls.overduedate > l_busdate then nvl(nml,0) else 0 end
                                       when p_type = 'L' and (ln.prepaid = 'Y' or ln.lntype = 'S')
                                                  then case when ls.overduedate > l_busdate then nvl(nml,0) else 0 end
              ...
              from lnmast ln, lnschd ls, LNTYPE lnt
              where ln.acctno = ls.acctno and lnt.actype = ln.actype and lnt.CHKSYSCTRL like l_chksysctl
              and ln.ftype = 'AF'
              and instr(ls.reftype,'P') > 0
              and ln.status NOT IN ('R','C','P')
              and ln.trfacctno = p_afacctno
        ...
        exit when l_remain_avlamt <= 0;
         ...
         --==============================================================
         --1.5.7.0|iss1922: Sua tra no trong han co dang ky sp T10
         --chua qua ngay doi muc lai ko cho phep tra = UTTB
         --Tinh tong no trong han chua den ngay chuyen lai
         --Tinh tong no trong han qua ngay chuyen lai
         --Theo ngay den han
         --==============================================================
         for rec_TH in
           (
            select sum(round( case when p_type='R' then
                                        case WHEN ((ln.prepaid = 'Y' and ln.advpay = 'Y') or (ln.lntype = 'S'))
                                               then case when ls.Duedate > l_busdate then nvl(nml,0) else 0 end
                                        ELSE 0 END
                                   when p_type='L' then
                                        case WHEN ((ln.prepaid = 'Y') or (ln.lntype = 'S'))
                                               then case when ls.Duedate > l_busdate then nvl(nml,0) else 0 end
                                        ELSE 0 END
                                   else 0 end
                           ,0) +
                    round(case when ls.reftype = 'GP' then
                        nvl(ls.intnmlacr,0)
                            ...
                            case when p_type='R' then
                                      case WHEN  ((ln.prepaid = 'Y' and ln.advpay = 'Y') or (ln.lntype = 'S')) and ls.Duedate > l_busdate  then nvl(ls.intnmlacr,0)
                                      else 0 end
                                 when p_type='L' then
                                      case WHEN  ((ln.prepaid = 'Y' ) or (ln.lntype = 'S')) and ls.Duedate > l_busdate  then nvl(ls.intnmlacr,0)
                                      else 0 end
                                 else 0 end
                        end,0) +
                    round(nvl(ls.fee,0),0) +
                    round(case when ls.reftype = 'GP' then
                        nvl(ls.feeintnmlacr,0)
                            ...
                            case when p_type='R' then
                                      case WHEN  ((ln.prepaid = 'Y' and ln.advpay = 'Y') or (ln.lntype = 'S')) and ls.Duedate > l_busdate  then nvl(ls.feeintnmlacr,0)
                                      else 0 end
                                 when p_type='L' then
                                      case WHEN  ((ln.prepaid = 'Y') or (ln.lntype = 'S')) and ls.Duedate > l_busdate  then nvl(ls.feeintnmlacr,0)
                                      else 0 end
                                 else 0 end
                        end,0) +
                    round(case when ls.reftype = 'GP' then
          ...
          and nvl(ls.mintermdate,l_default_date) <= l_busdate
           ...
           loop
             --=========================================================================
             --TRa cho cac mon trong han da qua ngay chuyen lai truoc --giong hien tai
             --=========================================================================
             if rec_th.amt_out_duedate>0 then
                   -- Theo OVERDUEDATE:
                    l_exec_avlamt_group:=0;
                    l_remain_avlamt_group:= least(l_remain_avlamt, greatest(rec_TH.amt_out_duedate,0));
                    l_avlamt_group:=l_remain_avlamt;--Phuc vu tinh so tien tien mat da tra cho truong hop nay
                    l_exec_avlamt:=0;
                    ...
                    l_remain_avlamt:= greatest(l_remain_avlamt - l_exec_avlamt,0);
                    exit when l_remain_avlamt <= 0;
              end if;
              --======================================================================
              --TRa cho cac mon trong han chua qua ngay chuyen lai: Ko dung tien UT
              --======================================================================
              --Tinh tien mat con lai = max (tien mat - tien da tra th tren,0)
              if  rec_th.amt_out_duedate>0  then
              l_remain_balance:= greatest( l_remain_balance-(l_avlamt_group-l_remain_avlamt),0 );
               end if;
              l_store_balance:=l_remain_balance ; -- Lưu tien mat de tinh cho vong sau

             if rec_th.amt_in_duedate>0 and l_remain_balance>0 then
                   -- Theo OVERDUEDATE:
                    l_exec_avlamt_group:=0;
                    l_remain_avlamt_group:= least(l_remain_balance, greatest(rec_TH.amt_in_duedate,0));
                    l_exec_avlamt:=0;
                    ...
                    l_remain_balance:= greatest(l_remain_balance - l_exec_avlamt,0);
                    exit when l_remain_balance <= 0;
              end if;
        end loop;
        l_remain_avlamt:= l_remain_avlamt -greatest(l_store_balance-l_remain_balance,0) ; --Tong con lai bao gom ca UT
        exit when l_remain_avlamt <= 0;
    end if;
END LOOP; ---REC
...
v_totalamt number;
BEGIN
plog.setendsection(pkgctx, 'fn_Get_Prepaid_Payment');
p_err_code:=systemnums.C_SUCCESS;
l_default_date:=to_date('01/01/1999','DD/MM/RRRR');
l_busdate:= to_date(cspks_system.fn_get_sysvar('SYSTEM','CURRDATE'),'DD/MM/RRRR');
if p_chksysctl = 'ALL' then
...
LOOP --- REC
    v_totalamt:=v_totalamt +     greatest(REC.totalamt,0);
END LOOP; ---REC
p_avlpaidamt := v_totalamt;
p_err_code:=systemnums.C_SUCCESS;
plog.setendsection(pkgctx, 'fn_Get_Prepaid_Payment');
return systemnums.C_SUCCESS;
EXCEPTION
