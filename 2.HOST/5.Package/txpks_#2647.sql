...
WHEN OTHERS
   THEN
      ...
      plog.debug(pkgctx,'pr_txlog: ' || dbms_utility.format_error_backtrace);
...
/*
    ...
    BEGIN
         ...
         FROM CFMAST, LNAPPL, LNMAST MST
         WHERE CFMAST.CUSTID = LNAPPL.CUSTID
         AND LNAPPL.APPLID=MST.APPLID
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
    ...
    /* If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('02').value,'DFMAST','02','2647')<>'TRUE' then
