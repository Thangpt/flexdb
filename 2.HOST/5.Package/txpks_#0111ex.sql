CREATE OR REPLACE PACKAGE txpks_#0111ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#0111EX
...
END;
/
CREATE OR REPLACE PACKAGE BODY txpks_#0111ex
IS
   pkgctx   plog.log_ctx;
            ...
            select af.acctno afacctno,
                   least(
                        greatest(    -least(
                                            ...
                                            0)
                                        - nvl(t0.marginamt,0)
                                        - (CASE WHEN af.trfbuyrate * af.trfbuyext > 0 then nvl(trf.trfsecuredamt_inday,0) + nvl(trf.secureamt_inday,0)
                                                else nvl(b.secureamt,0) + nvl(b.overamt,0) END)
                                                * decode(nvl(lnt.chksysctrl,'N'),'Y',1,0)
                                        +least(af.mrcrlimit,((CASE WHEN af.trfbuyrate * af.trfbuyext > 0 then nvl(trf.trfsecuredamt_inday,0) + nvl(trf.secureamt_inday,0)
                                                else nvl(b.secureamt,0) + nvl(b.overamt,0) END)
                                                * decode(nvl(lnt.chksysctrl,'N'),'Y',1,0)))
                ...
                from cimast mst,
                    afmast af, lntype lnt, aftype aft,
                    (select pr.afacctno,
...
BEGIN
    ...
    ***************************************************************************************************/
    plog.error('IN:'||p_txmsg.txfields(c_afacctno).value || '.');
    l_txnum:=p_txmsg.txnum; --1.6.1.2
    ...
    if trim(p_txmsg.txfields(c_afacctno).value) = 'ALL'
        ...
        loop
            ...
            if fn_markedafpralloc_2(marked_rec.afacctno,
                            null,
                            'A',
                            'T',
                            null,
        ...
        --ThanhNV: Gen su kien dong bo xuong FO:
        ...
        if l_count > 0 then
            ...
            if fn_markedafpralloc_2(p_txmsg.txfields(c_afacctno).value,
                            null,
                            'A',
                            'T',
                            null,
    ...
    SELECT varvalue INTO l_hoststatus FROM sysvar WHERE grname = 'SYSTEM' AND varname = 'HOSTATUS';
    if nvl(l_hoststatus, '0') = '0' then
      p_err_code := '-100021'; -- Pre-defined in DEFERROR table
