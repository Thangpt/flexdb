CREATE OR REPLACE PACKAGE txpks_#1107ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#1107EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#1107ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    select count(1)
        into l_count
    from stschd
    where duetype = 'SM' and deltd <> 'Y' and trfbuyrate * trfbuyext * (amt-trfexeamt) > 0 and afacctno = p_txmsg.txfields(c_acctno).value;

    IF NOT l_count > 0 THEN
        p_err_code := '-400048'; -- Pre-defined in DEFERROR table
        plog.setendsection (pkgctx, 'fn_txPreAppCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
    END IF;
