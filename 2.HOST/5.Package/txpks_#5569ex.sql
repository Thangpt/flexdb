...
IS
    ...
    v_onltype_chksys varchar2(1);
    v_retype_chksys varchar2(1);
    l_PRINPERIOD number;
    l_TOTALDAY number;
    l_REFOPNAUTOID number;
    l_INTNMLACRBANK number;
    l_custbank_olntype  varchar2(100);
BEGIN
    ...
    ***************************************************************************************************/
    ...
    select nvl(greatest(getbaldeftrfamtex(p_txmsg.txfields(c_acctno).value),getbaldefovd(p_txmsg.txfields(c_acctno).value)),0)
        into l_avlbal
    from dual;
    ...
    if l_count > 0 then
         -- Neu ACTYPE cu va ACTYPE tai ky' la tk UY Ban thi check
         select count(1) into l_count from lntype where actype IN(SELECT ACTYPE FROM LNMAST WHERE ACCTNO = p_txmsg.txfields('03').value)
                    and status <> 'N' and chksysctrl = 'Y';
         ...
         IF  l_count > 0 then
           ...
           if not l_MAXDEBTCF >= l_margin74amt + (to_number(p_txmsg.txfields(c_amt).value) - ( to_number(p_txmsg.txfields(c_pprinamt).value)
                                                                + to_number(p_txmsg.txfields(c_pintamt).value)
                                                                + to_number(p_txmsg.txfields(c_pfeeintamt).value)
                                                                + to_number(p_txmsg.txfields(c_pfeeintovdacr).value) ) )  then
           ...
           if not l_MAXDEBT >= l_totalmargin74amt + (to_number(p_txmsg.txfields(c_amt).value) - ( to_number(p_txmsg.txfields(c_pprinamt).value)
                                                                + to_number(p_txmsg.txfields(c_pintamt).value)
                                                                + to_number(p_txmsg.txfields(c_pfeeintamt).value)
                                                                + to_number(p_txmsg.txfields(c_pfeeintovdacr).value) ) ) then
               p_err_code := '-540235'; -- Pre-defined in DEFERROR table
    ...
    begin
        ...
        select  custbank into  l_custbank_olntype         from lntype where actype = p_txmsg.txfields(c_olntype).value;
    exception when others then
        l_rrtype:= 'X';
        l_custbank:= 'XXXXXXXXXXXX';
        l_custbank_olntype:='XXXXXXXXXXXX';
    end;
    select custid into l_custid from afmast where acctno = p_txmsg.txfields(c_acctno).value;
    if l_rrtype = 'B' then
        begin
        ...
        if p_txmsg.txfields(c_actype).value <> p_txmsg.txfields(c_olntype).value and l_custbank <> l_custbank_olntype then
            IF NOT l_avlamt >= TO_NUMBER(p_txmsg.txfields(c_amt).value) THEN
                p_err_code := '-100423'; -- Pre-defined in DEFERROR table
                plog.error ('ducnv1 l_avlamt : '||l_avlamt);
                plog.error ('ducnv1 TO_NUMBER(p_txmsg.txfields(c_amt).value) : '||TO_NUMBER(p_txmsg.txfields(c_amt).value));
                plog.setendsection (pkgctx, 'fn_txPreAppCheck');
                RETURN errnums.C_BIZ_RULE_INVALID;
            END IF;
        end if;
          ...
          select * from (
              select lnt.*, nvl(cf.mnemonic,cf.shortname) cfmnemonic,af.custid cfcustid, odrnum, 'Y' IsSubResource
                  ...
                  where aft.actype = afid.aftype
                      ...
                      and objname = 'LN.LNTYPE' and lnt.status <> 'N' AND lnt.lnpurpose = 'R'
              union all
              select lnt.*, nvl(cf.mnemonic,cf.shortname) cfmnemonic,af.custid cfcustid, 999 odrnum, 'N' IsSubResource
                  ...
                  where af.actype = aft.actype
                      and lnt.custbank = cf.custid(+)
                      and af.acctno = p_txmsg.txfields(c_acctno).value
                      and aft.lntype = lnt.actype and lnt.status <> 'N' AND lnt.lnpurpose = 'R'
                      )
        ...
        loop -- rec_ln
            begin
                ...
                from cimast mst, afmast af,
                    ...
                    from semast se,
                        (select s1.acctno, sum(s1.qtty-s1.aqtty) receiving
                            from
                                (select * from stschd where duetype = 'RS') s1,
                                (select * from stschd where duetype = 'SM') s2
                        ...
                        (SELECT SB.CODEID,LNB.ACTYPE,
                            ...
                            FROM (select * from lnsebasket where autoid= (
                                            select max(autoid) from lnsebasket
                        ...
                        (select codeid, sum(case when restype = 'M' then prinused else 0 end) prinused,
                            ...
                            sum(case when restype = 'M' and afacctno = p_txmsg.txfields(c_acctno).value then prinused else 0 end) prinused_af,
                            sum(case when restype = 'S' and afacctno = p_txmsg.txfields(c_acctno).value then prinused else 0 end) sys_prinused_af
                ...
                (select sts.afacctno, sum(amt+feeacr-feeamt-trft0amt) trfsecuredamt
                     ...
                     and od.afacctno = p_txmsg.txfields(c_acctno).value
                     and trfbuydt > l_currdate
            ...
            IF NOT l_afavlamt >= TO_NUMBER(p_txmsg.txfields(c_amt).value) THEN
                p_err_code := '-100423'; -- Pre-defined in DEFERROR table
                 plog.error ('ducnv2 l_avlamt : '||l_avlamt);
                  plog.error ('ducnv2 TO_NUMBER(p_txmsg.txfields(c_amt).value) : '||TO_NUMBER(p_txmsg.txfields(c_amt).value));
                plog.setendsection (pkgctx, 'fn_txPreAppCheck');
                RETURN errnums.C_BIZ_RULE_INVALID;
            END IF;
        end loop;
    ...
    if NOT l_count > 0 then
        ...
        select mrmrate into l_marginrate from afmast where acctno = p_txmsg.txfields(c_acctno).value;

        select count(1) into l_count from v_getsecmarginratio where afacctno = p_txmsg.txfields(c_acctno).value and rlsmarginrate >= l_marginrate;
        if not l_count > 0 then
        ...
        select greatest(max(af.advanceline) - nvl(sum(trft0amt),0),0)
            into l_trft0amt
        from afmast af, v_getbuyorderinfo b
        where af.acctno = b.afacctno(+)
        and af.acctno = p_txmsg.txfields(c_acctno).value;
        if not l_trft0amt >= to_number(p_txmsg.txfields(c_amt).value) then
    ...
    select chksysctrl into v_onltype_chksys from lntype where actype = p_txmsg.txfields(c_olntype).value;
    select chksysctrl, TO_NUMBER(PRINPERIOD + OPRINPERIOD) into v_retype_chksys, l_PRINPERIOD from lntype where actype = p_txmsg.txfields(c_actype).value;

    select TO_NUMBER(VARVALUE) INTO l_TOTALDAY from sysvar where varname like '%MAXTOTALDEBTDAYS%';


    if v_onltype_chksys = 'Y' and v_retype_chksys = 'Y' then
        if l_PRINPERIOD + to_number(p_txmsg.txfields('90').value) > l_TOTALDAY then
            p_txmsg.txWarningException('-5402261').value:= cspks_system.fn_get_errmsg('-540226');
            p_txmsg.txWarningException('-5402261').errlev:= '1';
        end if;
    end if;

/*    select nvl(REFOPNAUTOID,0) INTO l_REFOPNAUTOID from lnschd where autoid = p_txmsg.txfields('01').value;
    if l_REFOPNAUTOID > 0 then
        p_err_code := '-540227'; -- Pre-defined in DEFERROR table
        plog.setendsection (pkgctx, 'fn_txPreAppCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
    end if;*/

   select ROUND(nvl(INTNMLACRBANK,0)) into l_INTNMLACRBANK from lnschd where  autoid = p_txmsg.txfields('01').value;
   if to_number(p_txmsg.txfields('86').value) <> l_INTNMLACRBANK then
        p_err_code := '-530000'; -- Pre-defined in DEFERROR table
        plog.setendsection (pkgctx, 'fn_txPreAppCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
    end if;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    --- HaiLT them doan nay trong truong hop khi sua LNTYPE trong ngay va lam 5569 tren loai hinh dang sua thi se
    --- ap dung rule moi ngay cho t.k tai ky

    select count(*) into l_count from lnmast  where trfacctno = p_txmsg.txfields(c_acctno).value and actype = p_txmsg.txfields(c_actype).value;
    if l_count >0 then
        for recLN in (
            SELECT p_txmsg.txfields(c_acctno).value ACCTNO, LNT.CCYCD, LNT.LNTYPE, LNT.LNCLDR, LNT.PRINFRQ, LNT.PRINPERIOD,
            LNT.INTFRQCD, LNT.INTDAY, LNT.INTPERIOD, LNT.NINTCD, LNT.OINTCD, LNT.RATE1, LNT.RATE2, LNT.RATE3,
        ...
        LOOP
            update lnmast set rate1 = recLN.rate1, rate2 = recLN.rate2, rate3 = recLN.rate3,
                orate1 = recLN.orate1, orate2 = recLN.orate2, orate3 = recLN.orate3
            where trfacctno = p_txmsg.txfields(c_acctno).value and actype = p_txmsg.txfields(c_actype).value;
        END LOOP;
...
BEGIN
    ...
    */
    ...
    (
        select ln.trfacctno, ln.acctno, ls.autoid lnschdid,
                ...
                (case when reftype = 'P' then ls.INTNMLACRBANK else 0 end) INTNMLACRBANK
    ...
    loop
        ...
        l_txmsg.txfields ('86').VALUE     := rec_rls.INTNMLACRBANK;
    ...
    -- Cap nhap loai hinh tai ky moi cho tai khoan LN
    -- update lnmast set actype = p_txmsg.txfields(c_actype).value where acctno = p_txmsg.txfields(c_lnacctno).value;
    ...
    */
     ...
     plog.debug(pkgctx, 'c_actype' || p_txmsg.txfields(c_actype).value);
     plog.debug(pkgctx, 'trfacctno' ||  p_txmsg.txfields(c_acctno).value);

    select max(nvl(ln.acctno,'')), max(nvl(ln.rrtype,'')), max(nvl(ln.custbank,'')), max(nvl(ln.ciacctno,'')), max(nvl(cf.mnemonic,cf.shortname)) cfmnemonic
        into l_LNACCTNO, l_rrtype, l_custbank,  l_ciacctno, l_cfmnemonic
    from lnmast ln, cfmast cf where ln.custbank = cf.custid(+) and ln.actype = trim(p_txmsg.txfields(c_actype).value)
    and ln.trfacctno = trim(p_txmsg.txfields(c_acctno).value) and ln.STATUS NOT IN ('P','R','C') AND ln.FTYPE='AF';





     plog.debug(pkgctx, 'end debug 5569');
    ...
    select 'Giai ngan '||decode(l_rrtype,'C',UTF8NUMS.c_const_COMPANY_NAME,'B',nvl(l_cfmnemonic,''),'')||decode(l_count,0,'/CL/','/CL+/')||to_char(l_CURRDATE,'DD.MM.RRRR')||'/' || to_char(l_mramt+l_t0amt) || ' VND'
        into l_OrgDesc
        from dual;
    --Set cac field giao dich
    --03   C   ACCTNO
    ...
    l_txmsg.txfields ('35').VALUE     := p_txmsg.txfields(c_lnschdid).value;
