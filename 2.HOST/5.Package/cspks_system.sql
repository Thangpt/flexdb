...
IS
  ...
  Function fn_DBgen_trandesc (p_txmsg IN tx.msg_rectype,
   ...
   FUNCTION fn_get_en_errmsg (p_errnum IN varchar2)
        RETURN deferror.errdesc%TYPE; 
END;
/
...
IS
   ...
   BEGIN
      ...
      if p_tltxcd ='2670' then
        if p_apptype='CI' and p_apptxcd='0012' then
            ...
            p_txdesc:='Gi?i ngan deal ' || p_txmsg.txfields('02').value || ' ( SL: ' || p_txmsg.txfields('40').value || ', CK: ' || var1 || ', %phi: ' || var2 || ')';
      ...
      if p_tltxcd ='2678' then
        if p_apptype='CI' and p_apptxcd='0012' then
            ...
            p_txdesc:='Giải ngân deal ' || p_txmsg.txfields('02').value || ' ( SL: ' || var3 || ', CK: ' || var2 || ', %phi: ' || var1 || ')';
            /*plog.setendsection(pkgctx, 'pr_batch');
            return p_txdesc;*/
        end if;
      end if;
      if p_tltxcd ='2685' then
        if p_apptype='CI' and p_apptxcd='0012' then
            ...
            select b.rate3 into var2 from dftype a, lntype b where a.lntype = b.actype and a.actype =p_txmsg.txfields('04').value;
            plog.debug (pkgctx, 'var2:' || var2);
            p_txdesc:='Giải ngan deal ' || p_txmsg.txfields('02').value || ' ( SL: ' || p_txmsg.txfields('40').value || ', CK: ' || var1 || ', %phi: ' || var2 || ')';
        ...
        if p_apptype='CI' and p_apptxcd='0011' then
            ...
            select b.rate3 into var2 from dftype a, lntype b where a.lntype = b.actype and a.actype =p_txmsg.txfields('04').value;
            plog.debug (pkgctx, 'var2:' || var2);
            p_txdesc:='Thanh ly deal ' || p_txmsg.txfields('42').value || ' ( SL: ' || p_txmsg.txfields('64').value || ', CK: ' || var1 || ', %phi: ' || var2 || ')';
        ...
        if p_apptype='CI' and p_apptxcd='0028' then
            ...
            select b.rate3 into var2 from dftype a, lntype b where a.lntype = b.actype and a.actype =p_txmsg.txfields('04').value;
            plog.debug (pkgctx, 'var2:' || var2);
            p_txdesc:='Trả phí deal ' || p_txmsg.txfields('42').value || ' ( SL: ' || p_txmsg.txfields('64').value || ', CK: ' || var1 || ', %phi: ' || var2 || ')';
      ...
      if p_tltxcd ='2642' then
        ...
        plog.debug (pkgctx, 'var2:' || var2);
        if p_apptype='CI' and p_apptxcd='0011' then
            p_txdesc:='Thanh ly deal ' || p_txmsg.txfields('02').value || ' ( SL: ' || p_txmsg.txfields('46').value || ', CK: ' || var1 || ', %phi: ' || var2 || ')';
        end if;
        if p_apptype='CI' and p_apptxcd='0028' then
            p_txdesc:='Phi thanh ly deal ' || p_txmsg.txfields('02').value || ' ( SL: ' || p_txmsg.txfields('46').value || ', CK: ' || var1 || ', %phi: ' || var2 || ')';
        end if;
        if p_apptype='CI' and p_apptxcd='0012' then
            p_txdesc:='Thanh ly deal ' || p_txmsg.txfields('02').value || ' ( SL: ' || p_txmsg.txfields('46').value || ', CK: ' || var1 || ', %phi: ' || var2 || ')';
        end if;
      ...
      if p_tltxcd ='2686' then
        if p_apptype='CI' and p_apptxcd='0012' then
            ...
            select b.rate3 into var2 from dftype a, lntype b where a.lntype = b.actype and a.actype =p_txmsg.txfields('04').value;
            plog.debug (pkgctx, 'var2:' || var2);
            p_txdesc:='Giải ngân deal ' || p_txmsg.txfields('02').value || ' ( SL: ' || p_txmsg.txfields('40').value || ', CK: ' || var1 || ', %phi: ' || var2 || ')';
            /*plog.setendsection(pkgctx, 'pr_batch');
            return p_txdesc;*/
        end if;
      end if;
   ...
   BEGIN
      ...
      if p_tltxcd ='2643' or p_tltxcd ='2660' then
        ...
        plog.debug (pkgctx, 'var2:' || var2);
        if p_apptype='CI' and p_apptxcd='0011' then
            p_txdesc:='Thanh ly deal ' || p_txmsg.txfields('02').value || ' ( SL: ' || p_txmsg.txfields('10').value || ', CK: ' || var1 || ', %phi: ' || var2 || ')';
        end if;
        if p_apptype='CI' and p_apptxcd='0028' then
            p_txdesc:='Phi thanh ly deal ' || p_txmsg.txfields('02').value || ' ( SL: ' || p_txmsg.txfields('10').value || ', CK: ' || var1 || ', %phi: ' || var2 || ')';
        end if;
        if p_apptype='CI' and p_apptxcd='0012' then
            p_txdesc:='Thanh ly deal ' || p_txmsg.txfields('02').value || ' ( SL: ' || p_txmsg.txfields('10').value || ', CK: ' || var1 || ', %phi: ' || var2 || ')';
        end if;
   ...
   BEGIN
         ...
         begin
           ...
           if l_catype = '010' then --Co tuc bang tien
              ...
              if p_txdesc = '0001' then --So tien Goc
                 l_txdesc:='Cổ tức bằng tiền ' || var1 || ' ' || var2 || '% chốt ngày ' || var3;
              elsif p_txdesc = '0003' then --So thien thue
                 l_txdesc:='Thuế cổ tức bằng tiền ' || var1 || ' ' || var2 || '% chốt ngày ' || var3;
              --else
              ...
              where camastid = p_txmsg.txfields('02').value and ca.codeid = sb.codeid;
              if p_txdesc = '0001' then --So tien Goc
                 l_txdesc:='CP lẻ trả bằng tiền của cổ tức bằng cổ phiếu ' || var1 || ' chốt ngày ' || var3 || ', giá ' || var2 || ' đ/1CP';
              elsif p_txdesc = '0003' then --So thien thue
                 l_txdesc:='Thuế CP lẻ trả bằng tiền của cổ tức bằng cổ phiếu ' || var1 || ' chốt ngày ' || var3 || ', giá ' || var2 || ' đ/1CP';
              --else
              --   l_txdesc:=p_txmsg.txdesc;
              end if;
              ...
              where camastid = p_txmsg.txfields('02').value and ca.codeid = sb.codeid;
              if p_txdesc = '0001' then --So tien Goc
                 l_txdesc:='CP lẻ trả bằng tiền của cổ phiếu thưởng ' || var1 || ' chốt ngày ' || var3 || ', giá ' || var2 || ' đ/1CP';
              elsif p_txdesc = '0003' then --So thien thue
                 l_txdesc:='Thuế CP lẻ trả bằng tiền của cổ phiếu thưởng ' || var1 || ' chốt ngày ' || var3 || ', giá ' || var2 || ' đ/1CP';
              --else
              --   l_txdesc:=p_txmsg.txdesc;
              end if;
              ...
              elsif p_txdesc = '0002' then --So tiền lãi
           ...
           --T11/2017 CW_PhaseII
              ...
              select sb.symbol, ca.devidentrate, to_char(ca.reportdate,'DD/MM/RRRR'), CA.DEVIDENTVALUE, ca.TYPERATE
                     into var1, var2, var3, var4, l_TYPERATE
              from camast ca, sbsecurities sb--,
                   --(select * from camastdtl where deltd <> 'Y' and status ='P') cad
              where --ca.camastid = cad.camastid(+)
                    --and
                    ca.camastid = p_txmsg.txfields('02').value
                    and ca.codeid = sb.codeid;

              if p_txdesc = '0001' then --So tien Goc
                 l_txdesc:='Chi trả lợi tức chứng quyền ' || var1 || ' chốt ngày ' || var3;
              elsif p_txdesc = '0003' then --So thien thue
                 l_txdesc:='Thuế lợi tức chứng quyền ' || var1 || ' chốt ngày ' || var3||', <$DES>';
                  if (l_TYPERATE ='R') then
                   l_txdesc:= replace(l_txdesc,'<$DES>','Tỷ lệ '||var2);
                 else
                    l_txdesc:= replace(l_txdesc,'<$DES>','Chia theo giá trị/CW '||var4);
                 end if;
              end if;
              --End T11/2017 CW_PhaseII
           end if;
         exception when others then
           l_txdesc:=l_txdesc;
         end;
      ...
      if p_tltxcd = '0066' then
         if p_txdesc = '0001' then
            l_txdesc:= p_txmsg.txfields('30').value || ' (CK quy�?n)';
         end if;
      end if;
      if p_tltxcd = '5566' then
            if p_apptype = 'CI' then
                begin
                    select nvl(nvl(cf.mnemonic,cf.shortname),UTF8NUMS.c_const_COMPANY_NAME)
                        into var1
      ...
      if p_tltxcd ='5540' then
       ...
       if p_tltxcd ='1113' then
            if p_apptype='CI' then
                begin
                    select to_char(getcurrdate,'MM/RRRR')
                        into var1
                    from dual;
                exception when others then
                    var1:='';
                ...
                l_txdesc:= replace(l_txdesc,'<$TXDATE>',var1);
                l_txdesc:= replace(l_txdesc,'<$WARAMT>',TO_CHAR(p_txmsg.txfields('11').value));
                l_txdesc:= replace(l_txdesc,'<$SMSAMT>',TO_CHAR(p_txmsg.txfields('12').value));
            end if;
      end if;

      if p_tltxcd IN ('2674','2624') then
            if p_apptype='CI' then
                begin
...
41  FORMAT:[0001]>>Giai ngan <$BANKNAME>/DF/<$TXDATE>/<$AMT>
      ...
      if p_tltxcd in ('2646','2648','2664','2665','2636','2635') then
            if p_apptype='CI' then
                ...
                begin
                    SELECT nvl(nvl(cf.mnemonic,cf.shortname),UTF8NUMS.c_const_COMPANY_NAME), lnm.rlsdate, lnm.rlsamt
   ...
   BEGIN
	...
	--SELECT upper(dbms_random.string('U', 10)) str INTO l_Password from dual;
      Select to_char(round(dbms_random.value(100000,999999))) str INTO l_Password from dual;
      RETURN l_Password;
   END;
