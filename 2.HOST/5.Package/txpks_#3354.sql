...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
    ...
    END IF;
      ...
      UPDATE CIMAST
         SET
           BALANCE = BALANCE + (ROUND((p_txmsg.txfields('10').value-p_txmsg.txfields('13').value-p_txmsg.txfields('20').value*p_txmsg.txfields('62').value)*p_txmsg.txfields('60').value,0)),
    ...
    IF  NOT to_char(p_txmsg.txfields('62').value) <> '0' THEN
      ...
      UPDATE CIMAST
         SET
           BALANCE = BALANCE + (ROUND((p_txmsg.txfields('13').value-p_txmsg.txfields('20').value)*p_txmsg.txfields('60').value,0)), LAST_CHANGE = SYSTIMESTAMP
        WHERE ACCTNO=p_txmsg.txfields('03').value;
    END IF;
    IF to_char(p_txmsg.txfields('61').value) <> '0' THEN
