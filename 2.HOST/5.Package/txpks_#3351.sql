...
plog.setbeginsection (pkgctx, 'pr_txlog');
   ...
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
      VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'25',TO_NUMBER(p_txmsg.txfields('25').value),NULL,'Blocked quantity');
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
...
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
     ...
     IF NOT ( INSTR('SGHJ',l_STATUS) > 0) THEN
        p_err_code := '-300014';
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
...
*/
      ...
      l_txdesc:= cspks_system.fn_DBgen_trandesc_with_format(p_txmsg,'3351','SE','0044','0002');
      INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('09').value,'0044',ROUND(p_txmsg.txfields('25').value,0),NULL,p_txmsg.txfields ('01').value,p_txmsg.deltd,p_txmsg.txfields ('01').value,seq_SETRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || l_txdesc || '');
    ...
    END IF;
      INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('09').value,'0068',ROUND(p_txmsg.txfields('13').value+p_txmsg.txfields('25').value,0),NULL,p_txmsg.txfields ('01').value,p_txmsg.deltd,p_txmsg.txfields ('01').value,seq_SETRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');
*/
      ...
      UPDATE SEMAST
         SET
           BLOCKED = BLOCKED - (ROUND(p_txmsg.txfields('25').value,0)),
           /*DDROUTQTTY = DDROUTQTTY + (ROUND(p_txmsg.txfields('13').value+p_txmsg.txfields('25').value,0)),*/
