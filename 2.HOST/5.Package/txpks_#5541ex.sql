    ...
    loop -- rec_ln
        begin
            ...
            from cimast mst, afmast af,
                ...
                from semast se,
                    ...
                    (select codeid, sum(case when restype = 'M' then prinused else 0 end) prinused,
                        ...
                        sum(case when restype = 'M' and afacctno = p_txmsg.txfields(c_afacctno).value then prinused else 0 end) prinused_af,
                        sum(case when restype = 'S' and afacctno = p_txmsg.txfields(c_afacctno).value then prinused else 0 end) sys_prinused_af
            ...
            (select sts.afacctno, sum(amt+feeacr-feeamt-trft0amt) trfsecuredamt
                 ...
                 and od.afacctno = p_txmsg.txfields(c_afacctno).value
...
BEGIN
    ...
    */
    ...
    select 'Giai ngan '||decode(l_rrtype,'C',UTF8NUMS.c_const_COMPANY_NAME,'B',nvl(l_cfmnemonic,''),'')||'/CL/'||to_char(l_CURRDATE,'DD.MM.RRRR')||'/' || to_char(l_exec_MRamount) || ' VND'
        into l_OrgDesc
        from dual;
    --Set cac field giao dich
    --03   C   ACCTNO
