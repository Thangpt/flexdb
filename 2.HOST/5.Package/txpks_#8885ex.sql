...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    INTO l_oldOrderqtty, v_dblEXECQTTY,v_dblREMAINQTTY,v_ORGPRICE,v_strpriceType,v_strORSTATUS FROM odmast
    ...
    when no_data_found then
      ...
      plog.error(pkgctx,'Phuong Test1: '|| l_oldOrderqtty || ' , ' || v_dblQTTY ||' , ' || v_ADPRICE || ' , ' || v_ORGPRICE );
...
/*  IF NOT (greatest(to_number(l_TRADE),0) >= to_number(p_txmsg.txfields('96').value*(p_txmsg.txfields('12').value-p_txmsg.txfields('60').value*p_txmsg.txfields('12').value))) THEN
        ...
        RETURN errnums.C_BIZ_RULE_INVALID;
    END IF;*/
    IF NOT (to_number(l_TRADE)+to_number(l_dfmortage) >= to_number(p_txmsg.txfields('12').value) -to_number(p_txmsg.txfields('16').value) ) THEN
       p_err_code := '-900017';
       RETURN errnums.C_BIZ_RULE_INVALID;
    END IF;
    ...
    ***************************************************************************************************/
    ...
    If Not v_blnREVERSAL Then
        ...
        INSERT INTO ODMAST (ORDERID,CUSTID,ACTYPE,CODEID,AFACCTNO,SEACCTNO,CIACCTNO,
                                        ...
                                        EXECQTTY,STANDQTTY,CANCELQTTY,ADJUSTQTTY,REJECTQTTY,REJECTCD,VOUCHER,CONSULTANT,REFORDERID,EDSTATUS, TLID,FOACCTNO,QUOTEQTTY,IsFO_Order)
                        VALUES ( v_strORDERID , v_strCUSTID , v_strACTYPE , v_strCODEID , v_strAFACCTNO , v_strSEACCTNO , v_strCIACCTNO
                                        ...
                                        v_strCancelOrderID , v_strEDSTATUS, v_strTLID, l_strFOACCTNO,v_dblQuoteqtty , l_IsFO_Order );
