CREATE OR REPLACE PACKAGE txpks_#1805ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#1805EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#1805ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    IF p_txmsg.deltd = 'N' THEN
       BEGIN
           SELECT nvl(COUNT(*),0) INTO v_count             FROM VW_ACCOUNT_ADVT0
...
IS
       ...
       l_ACCTNO VARCHAR2(30);
       l_TLID   VARCHAR2(30);
       l_advanceline NUMBER(30);
       v_symbol VARCHAR2(200);
BEGIN
    ...
    ***************************************************************************************************/
    ...
    IF l_advanceline <= 0 THEN
        FOR rec IN
        (
            SELECT afse.autoid, sb.symbol FROM afserule afse, sbsecurities sb WHERE afse.codeid = sb.codeid and afse.refid = p_txmsg.txfields('03').value AND afseruletype = 'BL' AND afse.status = 'A'
        )
        LOOP
            v_symbol := v_symbol||rec.symbol||',';
            --update trang thai = C khi thu hoi het han muc BL
            UPDATE afserule SET
                status = 'C'
            WHERE afseruletype = 'BL' AND autoid = rec.autoid;
        END LOOP;
        IF v_symbol = ',' OR v_symbol IS NOT NULL OR v_symbol = ' ' OR v_symbol = '' THEN
            v_symbol := rtrim(v_symbol,',');

            --log DS CK co the dc mua
            INSERT INTO afseruledetail (REFID,SYMBOL,AFSERULETYPE,MAKER,APPROVE,ACTION,TXDATE,TXTIME)
            VALUES(p_txmsg.txfields('03').value,v_symbol,'BL',p_txmsg.tlid,NULL,'GD1811',getcurrdate,to_char(SYSTIMESTAMP,'hh24:mi:ss'));
            --end log DS CK
        END IF;
    END IF;
