...
BEGIN
   ...
   select count(*) into i from semastdtl where acctno =   p_txmsg.txfields('05').VALUE and  QTTYTYPE=   p_txmsg.txfields('19').VALUE;
...
ELSE
  ...
  select  nvl(qtty-dfqtty,0) into v_qtty from semastdtl where acctno =   p_txmsg.txfields('05').VALUE and  QTTYTYPE=   p_txmsg.txfields('19').VALUE;

if v_qtty is not null then
     IF nvl( v_qtty,0)-p_txmsg.txfields('22').VALUE < 0  THEN
          p_err_code:= -900040;
          RETURN -900040;
    END IF;
    end if;
...
if v_strDEALTYPE='P' then
    v_dblCARCVQTTY:=v_dblQTTY;
        ...
        SELECT COUNT(*) into l_count FROM DFMAST WHERE DFREF = p_txmsg.txfields('56').value AND DFRATE = v_dbDFRATE and groupid=p_txmsg.txfields('20').value AND CODEID =p_txmsg.txfields('01').value;

        if l_count >0 then
            SELECT MAX(AFACCTNO) INTO v_tmpDFACCTNO FROM DFMAST WHERE DFREF = p_txmsg.txfields('56').value AND DFRATE = v_dbDFRATE and groupid=p_txmsg.txfields('20').value AND CODEID =p_txmsg.txfields('01').value;
            ...
            INSERT INTO DFMAST (
                 ...
                 INTAMTACR, FEEAMT, RLSFEEAMT, STATUS, DFREF,DESCRIPTION,CISVRFEE,GROUPID,CALLTYPE,LIMITCHK,DEALTYPE,CACASHQTTY)
            VALUES
                 ...
                 0, 0, 0, 'A', p_txmsg.txfields('56').value,p_txmsg.txfields('30').value,v_dblCISVRFEE,v_strgroupid,p_txmsg.txfields('17').value,p_txmsg.txfields('99').value,v_strDEALTYPE,v_dblCACASHQTTY);
        ...
        if  v_dblBLOCKQTTY>0 then
            update semastdtl set dfqtty =dfqtty + v_dblBLOCKQTTY where acctno =   p_txmsg.txfields('05').VALUE and QTTYTYPE=   p_txmsg.txfields('19').VALUE;
        end if ;
