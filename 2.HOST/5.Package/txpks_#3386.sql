...
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Hu? dang k? mua CP ph?t h?nh th?m
...
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('25').value,'SEMAST','25','3386')<>'TRUE' then
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
         RETURN errnums.C_BIZ_RULE_INVALID;
     End if;
...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      ...
      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0029',ROUND(p_txmsg.txfields('21').value*p_txmsg.txfields('05').value*p_txmsg.txfields('60').value,0),NULL,p_txmsg.txfields ('01').value,p_txmsg.deltd,p_txmsg.txfields ('01').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');
      ...
      UPDATE CIMAST
         SET
           BALANCE = BALANCE + (ROUND(p_txmsg.txfields('21').value*p_txmsg.txfields('05').value*p_txmsg.txfields('60').value,0)),
