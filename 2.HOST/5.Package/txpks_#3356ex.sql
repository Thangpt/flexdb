...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    (
        ...
        SELECT  camast.camastid, camast.description, '001' type,
        ...
        where camast.camastid = ca.camastid
              ...
              and sbwft.tradeplace='006' and camast.status in('C','S','G','H','J')
...
*/
------------chaunh comment : cancel costprice calculator
            --INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
...
------------chaunh comment: recalcular costprice
...
------------end chaunh
   ...
   /* AND status <> 'C'*/ AND camastid =v_camastid AND acctno=rec.seacctnodr;
    ...
    p_err_code:=0;
    plog.debug(pkgctx, 'pck_3355 call');
    END IF;
