...
BEGIN
   plog.setbeginsection (pkgctx, 'pr_PrintInfo');
    ...
    END IF;
    BEGIN
         SELECT '' custname,'' address,'' license,'' custody,'' bankac,0 value
         INTO p_txmsg.txPrintInfo('01').custname,p_txmsg.txPrintInfo('01').address,p_txmsg.txPrintInfo('01').license,
         p_txmsg.txPrintInfo('01').custody,p_txmsg.txPrintInfo('01').bankac,p_txmsg.txPrintInfo('01').value
         FROM CRBTRFLOG
         WHERE AUTOID=l_acctno;
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
       ...
       SELECT  ERRSTS, STATUS
      INTO l_ERRSTS,l_STATUS
        FROM CRBTRFLOG
