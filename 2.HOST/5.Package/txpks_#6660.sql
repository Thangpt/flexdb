...
BEGIN
   plog.setbeginsection (pkgctx, 'pr_PrintInfo');
    ...
    END IF;
    BEGIN
         SELECT CF.FULLNAME,CF.ADDRESS,CF.IDCODE LICENSE, CF.CUSTODYCD,
         AF.BANKACCTNO BANKAC,CRB.BANKNAME,AF.BANKNAME BANKQUE,CI.HOLDBALANCE HOLDAMT
         INTO p_txmsg.txPrintInfo('03').custname,p_txmsg.txPrintInfo('03').address,p_txmsg.txPrintInfo('03').license,
         p_txmsg.txPrintInfo('03').custody,p_txmsg.txPrintInfo('03').bankac,p_txmsg.txPrintInfo('03').bankname,
         p_txmsg.txPrintInfo('03').bankque,p_txmsg.txPrintInfo('03').holdamt
         FROM CIMAST CI,AFMAST AF,CFMAST CF,CRBDEFBANK CRB
         WHERE CI.AFACCTNO=AF.ACCTNO AND AF.CUSTID=CF.CUSTID
         AND AF.BANKNAME=CRB.BANKCODE AND CI.ACCTNO=l_acctno;
