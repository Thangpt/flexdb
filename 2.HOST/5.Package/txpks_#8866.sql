...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      ...
      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('05').value,'0029',ROUND(p_txmsg.txfields('10').value-p_txmsg.txfields('10').value*p_txmsg.txfields('31').value,0),NULL,p_txmsg.txfields ('03').value,p_txmsg.deltd,p_txmsg.txfields ('03').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || p_txmsg.txdesc || '');
      ...
      UPDATE CIMAST
         SET
           BALANCE = BALANCE + (ROUND(p_txmsg.txfields('10').value-p_txmsg.txfields('10').value*p_txmsg.txfields('31').value,0)),
