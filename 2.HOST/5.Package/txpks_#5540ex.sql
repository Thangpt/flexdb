...
IS
   ...
   c_intnmlacrbank    CONSTANT CHAR(2) := '86';
    ...
    ***************************************************************************************************/
    ...
    loop
        ...
        END IF;
         ...
         IF round(to_number(p_txmsg.txfields(c_prinamt).value),0) + round(to_number(p_txmsg.txfields(c_intamt).value),0) <=0  THEN
            p_err_code := '-541113'; -- Pre-defined in DEFERROR table
            plog.setendsection (pkgctx, 'fn_txAftAppUpdate');
            RETURN errnums.C_BIZ_RULE_INVALID;
        END IF;

         IF round( rec_lnschd.intnmlacrbank ,0)<> round(to_number(p_txmsg.txfields(c_intnmlacrbank).value),0)   THEN
            p_err_code := '-541114'; -- Pre-defined in DEFERROR table
            plog.setendsection (pkgctx, 'fn_txAftAppUpdate');
            RETURN errnums.C_BIZ_RULE_INVALID;
        END IF;
    ...
    ***************************************************************************************************/
    ...
    loop
        UPDATE LNSCHD
            ...
            INTNMLACRBANK = INTNMLACRBANK - (ROUND(p_txmsg.txfields(c_intnmlacrbank).value,0))
        ...
        INSERT INTO LNSCHDLOG(AUTOID, TXNUM, TXDATE, NML, OVD, PAID, FEE, FEEOVD, FEEDUE, INTNMLACR, INTDUE, INTOVD, INTOVDPRIN,
                    ...
                    FEEINTPAID, INTPAID,INTNMLACRBANK)
        VALUES(rec_lnschd.AUTOID,
            ...
            round(to_number(p_txmsg.txfields(c_pt0intnmlacr).value),0) + round(to_number(p_txmsg.txfields(c_pintnmlacr).value),0)
                        ...
                        + round(to_number(p_txmsg.txfields(c_pt0intnmlovd).value),0) + round(to_number(p_txmsg.txfields(c_pintnmlovd).value),0)
                        + round(to_number(p_txmsg.txfields(c_pt0intovdacr).value),0) + round(to_number(p_txmsg.txfields(c_pintovdacr).value),0),
           - round(to_number(p_txmsg.txfields(c_intnmlacrbank).value),0)
                                                );
        ...
        loop
            ...
            INSERT INTO LNSCHDLOG(AUTOID, TXNUM, TXDATE, NML, OVD, PAID,INTNMLACRBANK)
            ...
            round(to_number(p_txmsg.txfields(c_pfeeovd).value),0)+ round(to_number(p_txmsg.txfields(c_pfeedue).value),0)
                + round(to_number(p_txmsg.txfields(c_pfee).value),0),0);
        end loop;

        for rec_int in
            (SELECT * FROM LNSCHD WHERE REFAUTOID = p_txmsg.txfields(c_autoid).value AND REFTYPE in( 'I','GI'))
        loop
            UPDATE LNSCHD
            ...
            INSERT INTO LNSCHDLOG(AUTOID, TXNUM, TXDATE, NML, OVD, PAID, NMLFEEINT, OVDFEEINT, PAIDFEEINT,INTNMLACRBANK)
        ...
        --Cap nhat lai lai va phi ve 0 neu abs < 0.5
        UPDATE lnschd SET intdue = 0 WHERE abs(intdue) < 0.5 and autoid=rec_lnschd.autoid;
        UPDATE lnschd SET intnmlacr = 0 WHERE abs(intnmlacr) <  0.5 and autoid=rec_lnschd.autoid;
        UPDATE lnschd SET intovd = 0 WHERE abs(intovd) <  0.5 and autoid=rec_lnschd.autoid;
        UPDATE lnschd SET intovdprin = 0 WHERE abs(intovdprin) < 0.5 and autoid=rec_lnschd.autoid;

        UPDATE lnmast SET intnmlacr = 0 WHERE abs(intnmlacr) <  0.5 and acctno = p_txmsg.txfields(c_acctno).value;
        UPDATE lnmast SET intdue = 0 WHERE abs(intdue)  <  0.5 and acctno = p_txmsg.txfields(c_acctno).value;
        UPDATE lnmast SET intovdacr = 0 WHERE abs(intovdacr) <  0.5 and acctno = p_txmsg.txfields(c_acctno).value;

        UPDATE lnschd SET feeintdue = 0 WHERE abs(feeintdue) <  0.5 and autoid=rec_lnschd.autoid;
        UPDATE lnschd SET feeintnmlacr = 0 WHERE abs(feeintnmlacr) <  0.5 and autoid=rec_lnschd.autoid;
        UPDATE lnschd SET FEEINTOVDACR = 0 WHERE abs(FEEINTOVDACR) <  0.5 and autoid=rec_lnschd.autoid;

        UPDATE lnmast SET feeintnmlacr = 0 WHERE abs(feeintnmlacr) <  0.5 and acctno = p_txmsg.txfields(c_acctno).value;
        UPDATE lnmast SET feeintdue = 0 WHERE abs(feeintdue) <  0.5 and acctno = p_txmsg.txfields(c_acctno).value;
        UPDATE lnmast SET feeintovdacr = 0 WHERE abs(feeintovdacr) <  0.5 and acctno = p_txmsg.txfields(c_acctno).value;

        UPDATE lnmast SET ointnmlacr = 0 WHERE abs(ointnmlacr) <  0.5 and acctno = p_txmsg.txfields(c_acctno).value;
        UPDATE lnmast SET ointdue = 0 WHERE abs(ointdue) <  0.5 and acctno = p_txmsg.txfields(c_acctno).value;
        UPDATE lnmast SET ointovdacr = 0 WHERE abs(ointovdacr) <  0.5 and acctno = p_txmsg.txfields(c_acctno).value;
        --End Cap nhat lai lai va phi ve 0 neu abs < 0.5
