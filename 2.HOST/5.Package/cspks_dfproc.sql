...
l_CONTRACTCHK := substr(V_STRXML,instr(V_STRXML,'|',1,24)+1,instr(V_STRXML,'|',1,25)-instr(V_STRXML,'|',1,24)-1 ) ;
l_AFACCTNODRD  := substr(V_STRXML,instr(V_STRXML,'|',1,25)+1,instr(V_STRXML,'|',1,26)-instr(V_STRXML,'|',1,25)-1 ) ;
l_ref := substr(V_STRXML,instr(V_STRXML,'|',1,26)+1 ) ;
...
plog.error (pkgctx, 'LINHLNB:l_GROUPID:'||l_GROUPID);
select count (groupid) into L_countgrpid  from  dfgrpdtllog where groupid=l_GROUPID ;
if L_countgrpid = 0  then
    plog.error (pkgctx, 'LINHLNB:L_countgrpid:'||L_countgrpid);
    insert into DFGRPLOG (autoid,groupid,actype,afacctno, dftype, limitchk , custbank, rrtype ,ciacctno,pstatus,status,orgamt,amt,calltype,lrate,mrate,irate,amtmin,tax,feemin,fee,lntype,txtime,txdate,autopaid,maker,checker,autodrawndown,isapprove,description,AFACCTNODRD,CONTRACTCHK)
    VALUES (seq_dfgrplog.NEXTVAL , l_GROUPID,l_ACTYPE,l_AFACCTNO,l_DFTYPE,L_limitchk,l_custbank,l_rrtype,L_CIACCTNO,'',DECODE(l_ISAPPROVE,'N','N','P'),l_orgamt,0,'P',l_lrate,l_mrate,l_irate,0,l_TAX,l_FEEMIN,l_FEE,l_LNTYPE,l_TXTIME,l_TXDATE,l_AUTOPAID,l_MAKER,'',l_AUTODRAWNDOWN,l_isapprove,l_DESCRIPTION,l_AFACCTNODRD,l_CONTRACTCHK );
end if;
...
plog.error (pkgctx, 'LINHLNB:SUCCESS:');
    p_err_code:=0;
...
l_GROUPID:=p_groupid;
 ...
 for rec in
    (
          select  df.groupid , cf.custodycd , DF.AFACCTNODRD , af.acctno afacctno,cf.fullname CUSTNAME,cf.address , cf.idcode LICENSE ,df.autodrawndown,df.autopaid,df.limitchk LIMITCHECK,
...
*/
   ...
   plog.debug (pkgctx,'pr_AddCIToReleaseSecu'||V_STRXML || ' ' || l_countN);
    ...
    for l_count in 1.. l_countN loop
        ...
        l_ORGAMT := substr( V_STRXML,instr(V_STRXML,'|',1,10) + 1,instr (V_STRXML,'@',1,1) - instr(V_STRXML,'|',1,10) -1 );
...
*/
        ...
        UPDATE DFMAST
           SET
             ...
             RLSQTTY = RLSQTTY + l_ReleaseQTTY, LAST_CHANGE = SYSTIMESTAMP
  ...
  BEGIN
   ...
   plog.debug(pkgctx,'pr_AddSEToGRDeal HAILT: ' || V_STRXML);
    ...
    for l_count in 1.. l_countN loop
        ...
        for rec in
        (
            select V.amtdf,V.amtln,V.AMTEXT,v.amtwit, al1.cdcontent DEALFLAGTRIGGER,DF.GROUPID,CF.CUSTODYCD,CF.FULLNAME,AF.ACCTNO AFACCTNO,CF.ADDRESS,CF.IDCODE,nvl(DF.CONTRACTCHK,'N') CONTRACTCHK,DF.LIMITCHK LIMITCHECK ,
            ...
            decode (df.RRTYPE,'C',1,0) CMPDRAWNDOWN,dftype.AUTODRAWNDOWN,df.calltype, DFB.DEALTYPE,
        ...
        loop
            ...
            --86  CONTRACTCHK     C   20CONTRACTCHK
            l_txmsg.txfields ('86').defname   := 'CONTRACTCHK';
            l_txmsg.txfields ('86').TYPE      := 'C';
    ...
    */
            ...
            ELSE
                ...
                INSERT INTO DFMAST (
                   ...
                   INTAMTACR, FEEAMT, RLSFEEAMT, STATUS, DFREF,DESCRIPTION,LIMITCHK,CISVRFEE,GROUPID,DEALTYPE,CACASHQTTY)
                SELECT l_DFACCTNO, AFACCTNO, (SELECT LNACCTNO FROM DFGROUP WHERE GROUPID=l_TOGROUPID), TXDATE, '', '',
                   ...
                   0, 0, 0, STATUS, DFREF,DESCRIPTION,LIMITCHK,CISVRFEE,l_TOGROUPID,DEALTYPE,0 FROM DFMAST WHERE ACCTNO=l_REFID;
            ...
            */
            ...
            ELSE
                ...
                SELECT l_DFACCTNO, AFACCTNO, (SELECT LNACCTNO FROM DFGROUP WHERE GROUPID=l_TOGROUPID), TXDATE, '', '',
                   ...
                   0, 0, 0, STATUS, DFREF,DESCRIPTION,LIMITCHK,CISVRFEE,l_TOGROUPID,DEALTYPE,v_dblCACASHQTTY FROM DFMAST WHERE ACCTNO=l_ACCTNO;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    v_strCONTRACTCHK:=p_txmsg.txfields('86').value;
...
select lntype into v_strLNTYPE from dftype  where actype =v_stractype;
    ...
    if not v_blnREVERSAL then
      ...
      --  if v_strACCTNO is null or length(v_strACCTNO)=0 then
       ...
       -- end if;
        begin
          SELECT LNT.RRTYPE,DFT.DFTYPE,LNT.CUSTBANK,LNT.CIACCTNO,DFT.LNTYPE,DFT.FEE,DFT.FEEMIN,DFT.TAX,DFT.AMTMIN,DFT.CISVRFEE, DFT.LRATE, DFT.MRATE, DFT.IRATE
            into    v_strRRTYPE,v_strDFTYPE,v_strCUSTBANK,v_strCIACCTNO,v_strLNTYPE,v_dblFEE,v_dblFEEMIN,v_dblTAX,v_dblAMTMIN,v_dblCISVRFEE,v_dblLRATE, v_dblMRATE, v_dblIRATE
        ...
        insert into dfgroup (groupid,actype,afacctno,lnacctno,dftype,flagtrigger,limitchk,custbank,rrtype,ciacctno,status,rlsfeeamt,feeamt,intamtacr,orgamt,amt,rlsamt,dfamt,calltype,lrate,mrate,irate,amtmin,tax,feemin,fee,lntype,txtime,txdate,autopaid,description)
        values (v_strgroupid,v_stractype,v_strAFACCTNODRD,v_strLNACCTNO,v_strDFTYPE,'N',v_strLIMITCHECK, v_strCUSTBANK,v_strRRTYPE,v_strCIACCTNO,'N',0,0,0,v_dblORGAMT,0,0,0,'P',v_dblLRATE,v_dblMRATE,v_dblIRATE,v_dblAMTMIN,v_dblTAX ,v_dblFEEMIN,v_dblFEE,v_strLNTYPE,p_txmsg.txnum,p_txmsg.txdate ,v_strAUTOPAID, v_strdec );
...
-- cho nhan ve
     ...
     /*  If v_dblRCVQTTY > 0 Then
        ...
        If p_txmsg.txfields('22').value > 0 Then
            UPDATE SEMASTDTL SET DFQTTY= DFQTTY + p_txmsg.txfields('22').value WHERE TXNUM || to_char(TXDATE,'dd/mm/rrrr')=p_txmsg.txfields('29').value;
        End If;
        ...
        begin
            ...
            from
                (select sts.qtty,sts.amt-od.feeacr -
                       (case when aft.vat='Y' then to_number(sys.varvalue) else 0 end)/100 *  sts.amt advamt,
                ...
                from stschd sts, odmast od, afmast af, aftype aft,sysvar sys, adtype adt, afidtype map
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    if not v_blnREVERSAL then
        ...
        begin
            SELECT LNT.RRTYPE,DFT.DFTYPE,LNT.CUSTBANK,LNT.CIACCTNO,DFT.LNTYPE,DFT.FEE,DFT.FEEMIN,DFT.TAX,DFT.AMTMIN,DFT.CISVRFEE
            into    v_strRRTYPE,v_strDFTYPE,v_strCUSTBANK,v_strCIACCTNO,v_strLNTYPE,v_dblFEE,v_dblFEEMIN,v_dblTAX,v_dblAMTMIN,v_dblCISVRFEE
        ...
        INSERT INTO DFMAST (
                     ...
                     TRIGGERPRICE, DFRATE, IRATE, MRATE, LRATE, CALLTYPE,
                     DFQTTY, BQTTY,RCVQTTY,BLOCKQTTY,CARCVQTTY, RLSQTTY, DFAMT, RLSAMT, AMT,
                     INTAMTACR, FEEAMT, RLSFEEAMT, STATUS, DFREF,DESCRIPTION,LIMITCHK,CISVRFEE,TLID)
              VALUES
                     ( v_strACCTNO , p_txmsg.txfields('03').value, v_strLNACCTNO, to_date(p_txmsg.txdate,systemnums.c_date_format), p_txmsg.txnum, p_txmsg.txtime,
                      p_txmsg.txfields('04').value, v_strRRTYPE, v_strDFTYPE, v_strCUSTBANK,v_strCIACCTNO, v_strLNTYPE,  v_dblFEE ,
                      v_dblFEEMIN ,  v_dblTAX ,  v_dblAMTMIN , p_txmsg.txfields('01').value,  p_txmsg.txfields('06').value ,  p_txmsg.txfields('10').value ,
                      p_txmsg.txfields('11').value ,  p_txmsg.txfields('07').value ,  p_txmsg.txfields('14').value ,  p_txmsg.txfields('08').value ,  p_txmsg.txfields('09').value , p_txmsg.txfields('15').value,
                      p_txmsg.txfields('12').value ,0, p_txmsg.txfields('13').value , p_txmsg.txfields('22').value , p_txmsg.txfields('23').value , 0, 0, 0,  v_dblAMT ,
                     0, 0, 0, v_strSTATUS, p_txmsg.txfields('29').value,p_txmsg.txfields('30').value,decode(p_txmsg.txfields('99').value,'1','Y','N'),v_dblCISVRFEE, '0000');
        If p_txmsg.txfields('13').value > 0 Then
            v_strORDERID:='orderid';
            begin
                select orgorderid into v_strORDERID from stschd where autoid=p_txmsg.txfields('29').value;
