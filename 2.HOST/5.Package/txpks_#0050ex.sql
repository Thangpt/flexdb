CREATE OR REPLACE PACKAGE TXPKS_#0050EX
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#0050EX
...
END;
/
CREATE OR REPLACE PACKAGE BODY TXPKS_#0050EX
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    IF l_count > 0 THEN
        p_err_code := '-200407'; -- Invalid status
        plog.setendsection (pkgctx, 'fn_txPreAppCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
    END IF;
    ...
    IF l_count > 0 THEN
        p_err_code := '-200410'; -- Invalid status
        plog.setendsection (pkgctx, 'fn_txPreAppCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
    END IF;

    IF REGEXP_LIKE(p_txmsg.txfields (c_blacctno).value, specialPartern) THEN
        p_err_code := '-200409'; -- Invalid status
        plog.setendsection (pkgctx, 'fn_txPreAppCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
    END IF;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    INSERT INTO bl_register (id,blacctno, afacctno,regdate, status)
    VALUES (seq_bl_register.nextval,p_txmsg.txfields (c_blacctno).value,p_txmsg.txfields (c_acctno).value, getcurrdate,'A');
