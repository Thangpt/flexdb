...
plog.debug(pkgctx,'Begin fn_txAppAutoCheck.fn_aftxmapcheck ');
     ...
     End if;
plog.debug(pkgctx,'Begin fn_txAppAutoCheck.fn_CIMASTcheck ');
     l_CIMASTcheck_arr := txpks_check.fn_CIMASTcheck(p_txmsg.txfields('05').value,'CIMAST','ACCTNO');

     l_STATUS := l_CIMASTcheck_arr(0).STATUS;
plog.debug(pkgctx,'End of fn_txAppAutoCheck.fn_CIMASTcheck ');
     IF NOT ( INSTR('A',l_STATUS) > 0) THEN
        p_err_code := '-400100';
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;

plog.debug(pkgctx,'BEgin fn_txAppAutoCheck. select  ' || p_txmsg.txfields('03').value);
...
plog.debug(pkgctx,'BEgin fn_txAppAutoCheck. select  ' || p_txmsg.txfields('03').value);
     ...
     END IF;
     plog.debug(pkgctx,'BEgin fn_txAppAutoCheck. select  ' || p_txmsg.txfields('03').value);
     -- PhuongHT comment: ko thay fill truong 10 vao jao dich
     /*IF NOT (to_number(l_BALANCE) >= to_number(p_txmsg.txfields('10').value)) THEN
        p_err_code := '-570005';
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;*/
     -- enf of PhuongHT comment
plog.debug(pkgctx,'end fn_txAppAutoCheck. select  ' || p_txmsg.txfields('03').value);
    END IF;
