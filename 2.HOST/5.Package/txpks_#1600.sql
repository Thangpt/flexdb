...
CREATE OR REPLACE PACKAGE BODY txpks_#1600
IS
   pkgctx   plog.log_ctx;
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('05').value,'CIMAST','05','1600')<>'TRUE' then
...
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
     ...
     END IF;
       ...
       SELECT  STATUS, BALANCE, MORTGAGE
      INTO l_STATUS,l_BALANCE,l_MORTGAGE
        FROM TDMAST
...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
     ...
     INSERT INTO TDTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
      ...
      INSERT INTO TDTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0026',ROUND(p_txmsg.txfields('11').value,0),NULL,'',p_txmsg.deltd,'',seq_TDTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'Nh?n lãi ti?t ki?m, g?i ngày ' || p_txmsg.txfields ('17').VALUE || ', s? ti?n g?i ban d?u ' || p_txmsg.txfields ('16').VALUE || '');
      ...
      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('05').value,'0029',ROUND(p_txmsg.txfields('10').value,0),NULL,p_txmsg.txfields ('03').value,p_txmsg.deltd,p_txmsg.txfields ('03').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'Rút g?c ti?t ki?m, g?i ngày ' || p_txmsg.txfields ('17').VALUE || ', s? ti?n g?i ban d?u ' || p_txmsg.txfields ('16').VALUE || '');

      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('05').value,'0012',ROUND(p_txmsg.txfields('11').value,0),NULL,p_txmsg.txfields ('03').value,p_txmsg.deltd,p_txmsg.txfields ('03').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'Nh?n lãi ti?t ki?m, g?i ngày ' || p_txmsg.txfields ('17').VALUE || ', s? ti?n g?i ban d?u ' || p_txmsg.txfields ('16').VALUE || '');
...
-- Run Pre Update
   IF txpks_#1600EX.fn_txPreAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto Update
   ...
   END IF;
-- Run After Update
   IF txpks_#1600EX.fn_txAftAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
-- Run Pre check
   IF txpks_#1600EX.fn_txPreAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto check
   ...
   END IF;
-- Run After check
   IF txpks_#1600EX.fn_txAftAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
