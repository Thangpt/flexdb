  ...
  END fn_GetAvailableTrade;
    ...
    BEGIN
        ...
        ELSE
            begin
                select
                    round ((sec.MRIRATIO/100 - nvl(sec.MARGINRATIO,0))
                                    / (1-sec.MRIRATIO/100) * (nvl(sec.SEMRASS,0) + greatest(sec.outstanding,0))

                    / least(nvl(sb.marginprice,0),nvl(rsk.mrpricerate,0)) ,0)
                into l_sedeposit

                from afmast af, aftype aft, mrtype mrt, afserisk rsk, securities_info sb, (select * from v_getsecmarginratio where afacctno = p_afacctno) sec
