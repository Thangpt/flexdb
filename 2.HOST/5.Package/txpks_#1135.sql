...
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Complete transfer to other Bank - Cash bank
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('03').value,'CIMAST','03','1135')<>'TRUE' then
   ...
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      ...
      UPDATE CIMAST
         SET
           ...
           FLOATAMT = FLOATAMT - (ROUND(p_txmsg.txfields('10').value,0)), LAST_CHANGE = SYSTIMESTAMP
