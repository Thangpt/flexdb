...
RETURN NUMBER;
FUNCTION fn_AutoGenWhenCancelOD(p_txmsg in out tx.msg_rectype,p_err_code in out varchar2,p_err_param out varchar2)
RETURN NUMBER;
...
--TungNT add - dung khi huy lenh, can gen giao dich
FUNCTION fn_AutoGenWhenCancelOD(p_txmsg in out tx.msg_rectype,p_err_code in out varchar2,p_err_param out varchar2)
RETURN NUMBER
IS
   l_return_code VARCHAR2(30) := systemnums.C_SUCCESS;
...
BEGIN
   plog.setbeginsection (pkgctx, 'fn_AutoGenWhenCancelOD');
   /*Gen transaction thi khong can phai check j ca
   IF fn_txAppCheck(p_txmsg, p_err_code) <> systemnums.C_SUCCESS THEN
        RAISE errnums.E_BIZ_RULE_INVALID;
   END IF;
   */
   ...
   ELSE    -- Delete transaction
       txpks_txlog.pr_txdellog(p_txmsg,p_err_code);
   END IF;
   plog.setendsection (pkgctx, 'fn_AutoGenWhenCancelOD');
   RETURN l_return_code;
EXCEPTION
   WHEN errnums.E_BIZ_RULE_INVALID
   THEN
