...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      ...
      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0012',ROUND(p_txmsg.txfields('10').value-p_txmsg.txfields('07').value*p_txmsg.txfields('10').value,0),NULL,'',p_txmsg.deltd,'',seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0014',ROUND(p_txmsg.txfields('10').value-p_txmsg.txfields('07').value*p_txmsg.txfields('10').value,0),NULL,'',p_txmsg.deltd,'',seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');



      UPDATE CIMAST
         SET
           BALANCE = BALANCE + (ROUND(p_txmsg.txfields('10').value-p_txmsg.txfields('07').value*p_txmsg.txfields('10').value,0)),
           LASTDATE = TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),
           CRAMT = CRAMT + (ROUND(p_txmsg.txfields('10').value-p_txmsg.txfields('07').value*p_txmsg.txfields('10').value,0)), LAST_CHANGE = SYSTIMESTAMP
