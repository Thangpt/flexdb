...
plog.setbeginsection (pkgctx, 'pr_txlog');
   ...
   plog.debug(pkgctx, 'abt to insert into tllogfld');
    BEGIN
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
      VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'82',TO_NUMBER(p_txmsg.txfields('82').value),NULL,'Top old Quantity');
      EXCEPTION
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
     ...
     END IF;
     /*
     IF NOT (to_number(l_PP) >= to_number(ROUND(p_txmsg.txfields('18').value*p_txmsg.txfields('98').value,0))) THEN
        p_err_code := '-400116';
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
     IF NOT (to_number(l_AVLLIMIT) >= to_number(p_txmsg.txfields('18').value*p_txmsg.txfields('98').value)) THEN
        p_err_code := '-400117';
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
     */
