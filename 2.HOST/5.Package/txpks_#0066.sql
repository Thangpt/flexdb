...
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Thu? TNCN thanh toán ngày
...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0011',ROUND(p_txmsg.txfields('10').value-p_txmsg.txfields('10').value*p_txmsg.txfields('31').value,0),NULL,'',p_txmsg.deltd,'',seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || p_txmsg.txdesc || '');

      l_txdesc:= cspks_system.fn_DBgen_trandesc_with_format(p_txmsg,'0066','CI','0028','0001');
      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0028',ROUND(p_txmsg.txfields('12').value-p_txmsg.txfields('12').value*p_txmsg.txfields('31').value,0),NULL,'',p_txmsg.deltd,'',seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || l_txdesc || '');


      UPDATE CIMAST
         SET
           BALANCE = BALANCE - (ROUND(p_txmsg.txfields('12').value-p_txmsg.txfields('12').value*p_txmsg.txfields('31').value,0)) - (ROUND(p_txmsg.txfields('10').value-p_txmsg.txfields('10').value*p_txmsg.txfields('31').value,0)), LAST_CHANGE = SYSTIMESTAMP
