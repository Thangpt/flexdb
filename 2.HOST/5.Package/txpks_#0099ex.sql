...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    SELECT cf.address, cf.phone
    INTO v_address, v_phone
    FROM cfmast cf
    ...
    UPDATE CFMAST SET
        --ADDRESS = p_txmsg.txfields(c_address).value,
        PHONE = p_txmsg.txfields(c_phone).value,
        email = p_txmsg.txfields(c_email).value
    WHERE CUSTID = p_txmsg.txfields(c_custid).value;
    UPDATE afmast set email =  p_txmsg.txfields(c_email).value
    where custid = p_txmsg.txfields(c_custid).value;
    IF v_count >0 THEN
        -- CAP NHAT CFCONTACT
    ...
    ELSE
        v_udaddress := p_txmsg.txfields(c_address).value;
    END IF;
    IF v_phone <> p_txmsg.txfields(c_phone).value THEN
