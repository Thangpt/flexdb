...
BEGIN
   ...
   plog.debug(pkgctx,'BEGIN OF fn_txPreAppCheck');
     ...
     select ceil(DEPOFEEAMT) into l_feeacr
     from cimast where acctno = p_txmsg.txfields('03').value;

     IF NOT (l_feeacr >= to_number(p_txmsg.txfields('10').value)) THEN
        p_err_code := '-401180';
    ...
    if(v_dblAMT>0) THEN
       if( p_txmsg.deltd <> 'Y') THEN
           ...
           if(L_count >0) THEN
                ...
                WHERE  DELTD<>'Y' AND AFACCTNO=v_strAfAcctno
                AND TODATE=TO_DATE(v_strTODATE,'DD/MM/RRRR');
                If v_dblFEEAMT <> v_dblAMT THEN -- phai thu tron ki
                  p_err_code:='-100430';
                  plog.setendsection (pkgctx, 'fn_txPreAppCheck');
                  RETURN errnums.C_BIZ_RULE_INVALID;
                END IF;
           ELSE
            --Thông báo không tìm th?y ti?u kho?n d? thu phí
                  p_err_code:='-400014';
                  plog.setendsection (pkgctx, 'fn_txPreAppCheck');
                  RETURN errnums.C_BIZ_RULE_INVALID;
           END IF;
           UPDATE CIFEESCHD SET PAIDAMT=NMLAMT ,
           paidtxdate=p_txmsg.busdate,
           paidtxnum=p_txmsg.txnum
           WHERE DELTD<>'Y' AND AFACCTNO=v_strAfAcctno
           AND TODATE=TO_DATE(v_strTODATE,'DD/MM/RRRR') ;
       ELSE -- xoa jao dich
          UPDATE CIFEESCHD SET PAIDAMT=0,
