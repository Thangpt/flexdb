...
plog.setbeginsection (pkgctx, 'pr_txlog');
   ...
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
      VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'18',0,p_txmsg.txfields('18').value,'DEAL type');
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   ...
   IF p_txmsg.txfields('18').value <> 'T' THEN
   ...
   plog.debug(pkgctx,'MATRIX04');
     ...
     IF NOT ( INSTR('A',l_STATUS) > 0) THEN
        ...
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
   plog.debug(pkgctx,'MATRIX05 FIELD 51: ' || p_txmsg.txfields('51').value || ' FIELD 50: ' || p_txmsg.txfields('50').value );
    IF to_char(p_txmsg.txfields('51').value) <> '0' THEN
...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
    plog.debug(pkgctx, 'DTYPE fn_txAppAutoUpdate:');
      ...
      IF  p_txmsg.txfields ('18').value = 'T' THEN
          UPDATE caschd set dfamt= dfamt + (ROUND(p_txmsg.txfields('12').value,0)) where autoid=p_txmsg.txfields('29').value;
      ELSE
          UPDATE SEMAST
             SET
