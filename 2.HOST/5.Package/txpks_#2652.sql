...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
    ...
    plog.debug(pkgctx,'begin TXPKS_#2652.fn_txAppAutoCheck.fn_aftxmapcheck ' || p_txmsg.txfields('05').value );
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('05').value,'SEMAST','05','2652')<>'TRUE' then
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
         RETURN errnums.C_BIZ_RULE_INVALID;
     End if;
    plog.debug(pkgctx,'begin TXPKS_#2652.fn_txAppAutoCheck.fn_aftxmapcheck ' || p_txmsg.txfields('03').value );
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('03').value,'CIMAST','03','2652')<>'TRUE' then
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
         RETURN errnums.C_BIZ_RULE_INVALID;
     End if;

    plog.debug(pkgctx,'begin TXPKS_#2652.fn_txAppAutoCheck.fn_SEMASTcheck ' || p_txmsg.txfields('05').value );
  /*
