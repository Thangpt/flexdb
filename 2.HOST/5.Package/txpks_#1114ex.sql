...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    if p_txmsg.deltd <> 'Y' then
        UPDATE CIREMITTANCE SET RMSTATUS='R' WHERE TXDATE= TO_DATE(p_txmsg.txfields('06').value,systemnums.c_date_format) AND TXNUM=p_txmsg.txfields('07').value;
    ELSE
        UPDATE CIREMITTANCE SET RMSTATUS='P' WHERE TXDATE= TO_DATE(p_txmsg.txfields('06').value,systemnums.c_date_format) AND TXNUM=p_txmsg.txfields('07').value;
    END IF;
