...
BEGIN
    ...
    ***************************************************************************************************/
    IF p_txmsg.deltd = 'N' THEN
       ...
       BEGIN
            SELECT TRADEPLACE, sectype INTO l_strTRADEPLACE, l_sectype FROM SBSECURITIES WHERE CODEID= l_strCODEID;
       EXCEPTION
       ...
       IF l_strTRADEPLACE = errnums.gc_TRADEPLACE_HNCSTC OR  l_strTRADEPLACE = errnums.gc_TRADEPLACE_UPCOM  THEN
           --Lay tham so max order trung nhau trong ordersys.
            ...
            AND AFACCTNO IN (SELECT ACCTNO FROM AFMAST WHERE CUSTID=(SELECT CUSTID FROM AFMAST WHERE ACCTNO= l_strAFACCTNO ))
             ...
             plog.error(pkgctx,'l_count '||l_count || ' v_MaxSameOrd '|| v_MaxSameOrd );
    ...
    begin
            SELECT mr.mrtype, af.actype, mst.groupleader, nvl(mst.deal,'N'), cf.t0loanrate
            ...
            WHERE mst.actype = af.actype
              ...
              from stschd sts
              where sts.afacctno = p_txmsg.txfields('03').value and sts.duetype = 'SM'  and deltd <> 'Y' and trfbuysts <> 'Y' and trfbuyrate * trfbuyext > 0;
        ...
        if l_margintype not in ('S','T') THEN
           /* --00 Ktra Co dung BL phe duyet mua CK blacklist khong.
            ...
            IF NOT (ceil(to_number(l_PP)) >= to_number(ROUND(p_txmsg.txfields('96').value*p_txmsg.txfields('12').value*p_txmsg.txfields('11').value*p_txmsg.txfields('13').value*p_txmsg.txfields('98').value/p_txmsg.txfields('99').value,0))) THEN
                ...
                PLOG.error (pkgctx, 'p_txmsg.txfields(96).value' || p_txmsg.txfields('96').value);
                PLOG.error (pkgctx, 'p_txmsg.txfields(12).value' || p_txmsg.txfields('12').value);
                PLOG.error (pkgctx, 'p_txmsg.txfields(11).value' || p_txmsg.txfields('11').value);
                PLOG.error (pkgctx, 'p_txmsg.txfields(13).value' || p_txmsg.txfields('13').value);
                PLOG.error (pkgctx, 'p_txmsg.txfields(98).value' || p_txmsg.txfields('98').value);
                PLOG.error (pkgctx, 'p_txmsg.txfields(99).value' || p_txmsg.txfields('99').value);
                PLOG.error (pkgctx, 'L_PP' || L_PP);
                plog.setendsection (pkgctx, 'fn_txPreAppCheck');
                RETURN errnums.C_BIZ_RULE_INVALID;
            END IF;
        else
               ...
               -- l_PPSE_BL:=l_PP + (l_advanceline-l_aprT0) - l_T0SecureAmt;
                ...
                if l_chksysctrl = 'Y' then
                  --  if l_PP > 0 then
                        ...
                        --HaiLT sua theo y/c MR008 Phase 2 cua MSBS
                ...
                /*plog.error('LINHLNB:l_mrratiorate:'||l_mrratiorate);
                plog.error('LINHLNB:l_marginprice:'||l_marginprice);
                plog.error('LINHLNB:l_mrpriceloan:'||l_mrpriceloan);
                plog.error('LINHLNB:Price:'||p_txmsg.txfields('11').value);
                plog.error('LINHLNB:l_advanceline:'||l_advanceline);
                plog.error('LINHLNB:l_deffeerate:'||l_deffeerate);
                plog.error('LINHLNBl_PPse:'||l_PPse);
                plog.error('LINHLNBl_PP00:'||l_PP);*/
                ...
                if l_roomchk ='N' then
                    ...
                    --Neu co checkroom thi moi kiem soat Room, neu khong thi bo qua
                -- Room
                begin
           ...
           /*  plog.error (pkgctx, 'l_aprT0'||l_aprT0||'l_PPSE_BL'||to_number(l_PPSE_BL)||'l_PPSE_BL'||l_aprT0);
            --00 Ktra Co dung BL phe duyet mua CK blacklist khong.
            IF l_aprT0>0 THEN
...
IS
    ...
    Pl_REFCURSOR   PKG_REPORT.REF_CURSOR;
    l_strSETYPE    setype.actype%TYPE;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    SELECT count(1) into l_count FROM SEMAST WHERE ACCTNO=l_strAFACCTNO||l_strCODEID;
    if l_count<=0 THEN
      SELECT custid INTO l_strCUSTID  FROM afmast WHERE ACCTNO= l_strAFACCTNO;
      --Neu khong co thi tu dong mo tai khoan
      SELECT TYP.SETYPE into l_strSETYPE FROM AFMAST AF, AFTYPE TYP WHERE AF.ACTYPE=TYP.ACTYPE AND AF.ACCTNO= l_strAFACCTNO;
      INSERT INTO SEMAST (ACTYPE,CUSTID,ACCTNO,CODEID,AFACCTNO,
                        OPNDATE,LASTDATE,STATUS,IRTIED,IRCD,
                        COSTPRICE,TRADE,MORTAGE,MARGIN,NETTING,
                        STANDING,WITHDRAW,DEPOSIT,LOAN)
                  VALUES (l_strSETYPE, l_strCUSTID, l_strAFACCTNO||l_strCODEID , l_strCODEID , l_strAFACCTNO ,
                  p_txmsg.txdate,p_txmsg.txdate,'A','Y','001',
                  0,0,0,0,0,0,0,0,0);
    end IF;
    ...
    if l_blnReversal then
        ...
        INSERT INTO ODMAST (ORDERID,CUSTID,ACTYPE,CODEID,AFACCTNO,SEACCTNO,CIACCTNO,
                                         ...
                                         CONSULTANT,CONTRAFIRM, TRADERID,CLIENTID,FOACCTNO,PUTTYPE,CONTRAORDERID,CONTRAFRM, TLID,SSAFACCTNO,QUOTEQTTY,PTDEAL, ISFO_ORDER)
                         VALUES ( l_strORDERID , l_strCUSTID , l_strACTYPE , l_strCODEID , l_strAFACCTNO , l_strSEACCTNO , l_strCIACCTNO
                                         ...
                                         , l_strCONSULTANT , l_strContrafirm , l_strTraderid , l_strClientID , l_strFOACCTNO , l_strPutType , l_strContraCus , l_strContrafirm2, l_strTLID,l_strSSAFACCTNO, l_dblQuoteQtty,l_strPtDeal,l_IsFO_Order);
