...
BEGIN
   plog.setbeginsection (pkgctx, 'pr_PrintInfo');
    ...
    --<<BEGIN OF PROCESS SEWITHDRAWDTL>>
    l_acctno := p_txmsg.txfields('07').value;
    SELECT count(*) INTO l_count
    FROM SEWITHDRAWDTL
    WHERE TXDATETXNUM= l_acctno;
    ...
    BEGIN
         ...
         INTO p_txmsg.txPrintInfo('07').custname,p_txmsg.txPrintInfo('07').address,p_txmsg.txPrintInfo('07').license,p_txmsg.txPrintInfo('07').custody,p_txmsg.txPrintInfo('07').bankac,p_txmsg.txPrintInfo('07').value
         FROM CFMAST A, AFMAST  B
         WHERE A.CUSTID = B.CUSTID
         AND EXISTS (
             SELECT 1 FROM SEWITHDRAWDTL
             WHERE afacctno = b.acctno
             AND TXDATETXNUM=l_acctno
         );
    EXCEPTION WHEN NO_DATA_FOUND THEN
        p_err_code := errnums.C_CF_CUSTOM_NOTFOUND;
        RAISE errnums.E_PRINTINFO_ACCTNOTFOUND;
    END;
    --<<END OF PROCESS SEWITHDRAWDTL>>
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('12').value,'SEMAST','12','2201')<>'TRUE' then
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
         RETURN errnums.C_BIZ_RULE_INVALID;
     End if;
...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      ...
      INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('12').value,'0067',ROUND(p_txmsg.txfields('10').value*p_txmsg.txfields('09').value,0),NULL,p_txmsg.txfields ('03').value,p_txmsg.deltd,p_txmsg.txfields ('03').value,seq_SETRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('12').value,'0068',ROUND(p_txmsg.txfields('10').value,0),NULL,p_txmsg.txfields ('03').value,p_txmsg.deltd,p_txmsg.txfields ('03').value,seq_SETRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');
