...
v_countCA               NUMBER;
v_autoid                NUMBER;
L_SEACCTNO_UPDATECOST   VARCHAR2(20);
 ...
 */   l_dbl_left_rightoffrate := nvl(to_number(substr(l_rightoffrate,0,instr(l_rightoffrate,'/') - 1)),1);
    ...
    SELECT COUNT(*) INTO l_count
    FROM semast WHERE acctno=L_SEACCTNO_UPDATECOST;
    if(l_count=0) THEN
    BEGIN
         ...
         VALUES(
         l_sectype, l_custid, L_SEACCTNO_UPDATECOST,substr(L_SEACCTNO_UPDATECOST,11),l_afacctno,
    ...
    ELSE-- chua pbo CK thi pai cong RECEIVING cho ck se dc nhan ve o buoc 3351
         ...
         plog.debug (pkgctx, '<<BEGIN OF HAILT 1 ' || l_autoid );
          ...
          INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
                VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),L_SEACCTNO_UPDATECOST,'0051',l_receiving*l_exprice,NULL,l_afacctno,p_txmsg.deltd,l_strCAMASTID,seq_SETRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

          INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
                VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),L_SEACCTNO_UPDATECOST,'0052',l_receiving,NULL,l_afacctno,p_txmsg.deltd,l_strCAMASTID,seq_SETRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');


          UPDATE SEMAST
             SET
               DCRQTTY = DCRQTTY + l_receiving,
               DCRAMT = DCRAMT + l_receiving*l_exprice
            WHERE ACCTNO=L_SEACCTNO_UPDATECOST;
    ...
    ELSE
        if(l_status ='S') THEN
            ...
            plog.debug (pkgctx, '<<BEGIN OF HAILT 2 ' || l_autoid );
    ...
    plog.debug (pkgctx, '<<BEGIN OF HAILT 3' || l_autoid || ' ' || SEQ_CASCHD.NEXTVAL);
    --Kiem tra neu da co quyen thuc hien update , chua co insert
    SELECT nvl(COUNT(*),0), MAX(autoid) INTO v_countCA, v_autoid from caschd  WHERE camastid =l_strCAMASTID AND afacctno =l_afacctno;
    IF v_countCA =0 THEN
         INSERT INTO caschd
        ...
        (l_autoid, l_strCAMASTID, l_balance,(l_qtty+l_qttyreceive), l_amt,
         ...
         l_codeid, 'N',l_pstatus , 0, '', '', 0, 0, 0, l_pbalance, l_pqtty, l_paamt, 'N', '',
          0, l_isci, l_isse,
