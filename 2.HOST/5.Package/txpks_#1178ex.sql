...
BEGIN
  ...
  /*  if p_txmsg.deltd <>'Y' then
        ...
        loop
            ...
            if greatest (v_dbladfminfee,  v_days * rec.amt*v_dbladvrate/36000/(1-v_days * v_dbladvrate/36000))
               > rec.feeamt then
...
BEGIN
    ...
    ***************************************************************************************************/
    if p_txmsg.deltd <>'Y' then
        ...
        for rec in (
            select *  from adsource where id = p_txmsg.txfields('01').value
        )
        loop
            --Lay thong tin ve loai hinh UT moi
            ...
            --Cap nhat vao ADSCHD
            v_days:=to_number(rec.cleardt- rec.txdate);
            v_dblNewFeeamt:=greatest (v_dbladfminfee, v_days * rec.amt*v_dbladvrate/36000/(1-v_days*v_dbladvrate/36000));
            v_dblNewFeebank:=greatest (v_dbladvminfeebank, v_days * rec.amt*v_dbladvbankrate/36000/(1-v_days*v_dbladvbankrate/36000));
            update adsource set    rrtype = v_strrrtype, ciacctno =v_strciacctno, custbank=v_strcustbank,
